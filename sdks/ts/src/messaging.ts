...
export type PublishResult = { partition: number; offset: bigint; deduplicated: boolean };
...
export type Filter = { path: string; op: "equals"; value: string | number | boolean | null };
export type Message = { topic: string; partition: number; offset: bigint; value: Uint8Array; headers: Header[] };

export type StoredQueueDelivery = {
  state: "available" | "acquired" | "accepted" | "rejected";
  sessionId?: string;
...
export type StoredMessage = {
  ...
  partition: number;
  offset: bigint;
  queueDeliveries: Map<string, StoredQueueDelivery>;
};
export type MockStore = {
  messages: StoredMessage[];
  nextQueueSessionId: number;
  queueSessions: Map<string, { topic: string; group: string; max: number }>;
};
...
export function createMockStore(): MockStore {
  return { messages: [], nextQueueSessionId: 1, queueSessions: new Map() };
}
...
async function publishRecord(
  ...
  try {
    ...
    return { partition: result.partition, offset: result.offset, deduplicated: result.deduplicated };
  } catch (error) {
...
function publishMockRecord(store: MockStore, record: PublishRecord): PublishResult {
  const offset = BigInt(store.messages.filter((message) => message.record.topic === record.topic).length);
  store.messages.push({ record: cloneRecord(record), partition: 0, offset, queueDeliveries: new Map() });
...
function subscribe(
  ...
  if (subscribeOptions.filter) {
    toProtoFilter(subscribeOptions.filter);
  }
  if (options.mockStore) {
...
function toProtoFilter(filter: Filter | undefined): string {
  ...
  if (!filter.path.startsWith("$.")) {
    throw new InvalidArgumentError("filter path must start with $.");
  }
  const field = filter.path.slice(2);
  if (!field || !field.split(".").every((segment) => /^[A-Za-z_][A-Za-z0-9_]*$/.test(segment))) {
    throw new InvalidArgumentError("filter path must contain identifier segments");
  }
  ...
  if (typeof filter.value !== "boolean" && (typeof filter.value !== "number" || !Number.isFinite(filter.value))) {
    throw new InvalidArgumentError("filter value must be a string, number, boolean, or null");
  }
