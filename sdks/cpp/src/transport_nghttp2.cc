...
constexpr std::string_view kQueueAcquirePath = "/crabka.gateway.v1.Gateway/QueueAcquire";
constexpr std::string_view kQueueAcknowledgePath = "/crabka.gateway.v1.Gateway/QueueAcknowledge";
constexpr std::string_view kQueueRenewPath = "/crabka.gateway.v1.Gateway/QueueRenew";
...
struct QueueMessageCoordinates {
  std::string topic;
...
bool parse_integer(std::string_view value, Integer& parsed) {
  const auto [end, error] = std::from_chars(value.data(), value.data() + value.size(), parsed);
  return error == std::errc{} && end == value.data() + value.size();
}

QueueMessageCoordinates parse_queue_message_id(std::string_view message_id) {
  const auto first_separator = message_id.find(':');
  const auto second_separator = first_separator == std::string_view::npos
                                    ? std::string_view::npos
                                    : message_id.find(':', first_separator + 1);
  const auto extra_separator = second_separator == std::string_view::npos
                                   ? std::string_view::npos
                                   : message_id.find(':', second_separator + 1);
  QueueMessageCoordinates parsed;
  if (first_separator == std::string_view::npos || first_separator == 0 ||
      second_separator == std::string_view::npos || extra_separator != std::string_view::npos ||
      !parse_integer(message_id.substr(first_separator + 1,
                                       second_separator - first_separator - 1),
                     parsed.partition) ||
      !parse_integer(message_id.substr(second_separator + 1), parsed.offset)) {
    throw SdkError(error_with_message(
        ErrorKind::InvalidArgument,
        "queue message_id must be <topic>:<partition>:<offset>"));
  }
  parsed.topic.assign(message_id.substr(0, first_separator));
  return parsed;
}

std::string gateway_filter_literal(const json::Value& value);

bool valid_filter_segment(std::string_view segment) {
  if (segment.empty() || !(segment.front() == '_' ||
                           std::isalpha(static_cast<unsigned char>(segment.front())) != 0)) {
    return false;
  }
...
std::string filter_expression(const std::optional<Filter>& filter) {
  ...
  while (begin <= field.size()) {
    ...
    if (!valid_filter_segment(field.substr(begin, end - begin))) {
      throw SdkError(error_with_message(
  ...
  return field + " = " + gateway_filter_literal(filter->value);
}
...
std::vector<std::uint8_t> encode_queue_acquire_request(const std::string& topic,
                                                       const std::string& group,
                                                       std::uint32_t max,
                                                       std::uint64_t lock_duration_ms,
...
std::uint64_t queue_ack_type_value(QueueAckType ack_type) {
  if (ack_type == QueueAckType::Release) return 2;
  if (ack_type == QueueAckType::Reject) return 3;
  return 1;
}

std::vector<std::uint8_t> encode_queue_ack_entry(std::string_view message_id,
                                                 QueueAckType ack_type) {
  const auto coordinates = parse_queue_message_id(message_id);
  std::vector<std::uint8_t> out;
  append_bytes(out, 1, coordinates.topic);
  if (coordinates.partition != 0) {
    append_int(out, 2, static_cast<std::uint64_t>(static_cast<std::int64_t>(coordinates.partition)));
  }
  if (coordinates.offset != 0) append_int(out, 3, static_cast<std::uint64_t>(coordinates.offset));
  append_int(out, 4, queue_ack_type_value(ack_type));
  return out;
}

std::vector<std::uint8_t> encode_queue_acknowledge_request(
    const std::string& session_id, const std::vector<QueueAckEntry>& entries) {
  std::vector<std::uint8_t> out;
  append_bytes(out, 1, session_id);
  for (const auto& entry : entries) {
    append_bytes(out, 2, encode_queue_ack_entry(entry.message_id, entry.ack_type));
  }
  return out;
}

std::vector<std::uint8_t> encode_queue_renew_request(
    const std::string& session_id, const std::vector<QueueRenewEntry>& entries) {
  std::vector<std::uint8_t> out;
  append_bytes(out, 1, session_id);
  for (const auto& entry : entries) {
    append_bytes(out, 2, encode_queue_ack_entry(entry.message_id, QueueAckType::Accept));
  }
  return out;
}
...
LiveTransportRequestForTest make_queue_acquire_http_request(
    const std::string& topic, const std::string& group, std::uint32_t max,
    std::uint64_t lock_duration_ms, const std::string& session_id,
    const std::optional<std::string>& bearer) {
  return make_unary_http_request(
      kQueueAcquirePath,
      encode_queue_acquire_request(topic, group, max, lock_duration_ms, session_id), bearer);
}

LiveTransportRequestForTest make_queue_acknowledge_http_request(
    const std::string& session_id, const std::vector<QueueAckEntry>& entries,
    const std::optional<std::string>& bearer) {
  return make_unary_http_request(
      kQueueAcknowledgePath, encode_queue_acknowledge_request(session_id, entries), bearer);
}

LiveTransportRequestForTest make_queue_renew_http_request(
    const std::string& session_id, const std::vector<QueueRenewEntry>& entries,
    const std::optional<std::string>& bearer) {
  return make_unary_http_request(
      kQueueRenewPath, encode_queue_renew_request(session_id, entries), bearer);
}
...
QueueMessage decode_queued_message(std::string_view bytes) {
  FieldReader reader{.bytes = bytes};
  ...
  while (reader.cursor < reader.bytes.size()) {
    ...
    if (field == 1 && wire_type == 2) {
      const auto value = read_length_delimited(reader);
      message.topic.assign(value.begin(), value.end());
    } else if (field == 2 && wire_type == 0) {
      message.partition = static_cast<std::int32_t>(read_varint(reader));
    } else if (field == 3 && wire_type == 0) {
    ...
    } else if (field == 8 && wire_type == 0) {
      message.delivery_count = static_cast<std::int32_t>(read_varint(reader));
    } else {
      skip_field(reader, wire_type);
    }
  }
  message.message_id = message.topic + ":" + std::to_string(message.partition) + ":" +
                       std::to_string(message.offset);
  return message;
}

QueueAcquireResult decode_queue_acquire_response(const std::vector<std::uint8_t>& bytes) {
  FieldReader reader{
      .bytes = std::string_view(reinterpret_cast<const char*>(bytes.data()), bytes.size())};
  QueueAcquireResult result;
  while (reader.cursor < reader.bytes.size()) {
    const auto key = read_varint(reader);
    const auto field = static_cast<std::uint32_t>(key >> 3U);
    const auto wire_type = static_cast<std::uint8_t>(key & 0x07U);
    if (field == 1 && wire_type == 2) {
      const auto value = read_length_delimited(reader);
      result.session_id.assign(value.begin(), value.end());
    } else if (field == 2 && wire_type == 2) {
      result.messages.push_back(decode_queued_message(read_length_delimited(reader)));
    } else {
      skip_field(reader, wire_type);
    }
  }
...
ErrorKind queue_error_kind(std::int32_t code, bool retriable) {
  if (retriable) return ErrorKind::Transport;
  if (code == 3 || code == 9 || code == 11) return ErrorKind::InvalidArgument;
  if (code == 5) return ErrorKind::NotFound;
  if (code == 7 || code == 16) return ErrorKind::Unauthenticated;
  if (code == 12) return ErrorKind::Unimplemented;
  if (code == 1 || code == 4 || code == 14) return ErrorKind::Transport;
  return ErrorKind::ServerError;
}

QueueOperationError decode_queue_operation_error(std::string_view bytes) {
  FieldReader reader{.bytes = bytes};
  std::int32_t code = 0;
  ...
  while (reader.cursor < reader.bytes.size()) {
    ...
    if (field == 1 && wire_type == 0) {
      code = static_cast<std::int32_t>(read_varint(reader));
    } else if (field == 2 && wire_type == 2) {
      const auto value = read_length_delimited(reader);
      message.assign(value.begin(), value.end());
    } else if (field == 3 && wire_type == 0) {
      retriable = read_varint(reader) != 0;
    } else {
      skip_field(reader, wire_type);
    }
  }
  const auto kind = queue_error_kind(code, retriable);
  if (message == "record is not acquired by this session") {
    message = "queue message is not acquired";
  }
  return QueueOperationError{.kind = kind, .message = std::move(message)};
}

QueueResult decode_queue_result(std::string_view bytes, std::string message_id) {
  FieldReader reader{.bytes = bytes};
  ...
  while (reader.cursor < reader.bytes.size()) {
    ...
    const auto wire_type = static_cast<std::uint8_t>(key & 0x07U);
    if (field == 2 && wire_type == 2) {
      error = decode_queue_operation_error(read_length_delimited(reader));
    } else {
      skip_field(reader, wire_type);
    }
  }
  return QueueResult{.message_id = std::move(message_id), .error = std::move(error)};
}

QueueBatchResult decode_queue_batch_response(const std::vector<std::uint8_t>& bytes,
  ...
  while (reader.cursor < reader.bytes.size()) {
    ...
    const auto wire_type = static_cast<std::uint8_t>(key & 0x07U);
    if (field == 1 && wire_type == 2) {
      const auto message_id = result_index < message_ids.size() ? message_ids[result_index] : "";
      batch.results.push_back(
          decode_queue_result(read_length_delimited(reader), std::move(message_id)));
      ++result_index;
    } else {
      skip_field(reader, wire_type);
    }
...
std::vector<std::string> queue_message_ids(const std::vector<QueueAckEntry>& entries) {
  std::vector<std::string> ids;
  ids.reserve(entries.size());
  for (const auto& entry : entries) ids.push_back(entry.message_id);
  return ids;
}

std::vector<std::string> queue_message_ids(const std::vector<QueueRenewEntry>& entries) {
  std::vector<std::string> ids;
  ids.reserve(entries.size());
  for (const auto& entry : entries) ids.push_back(entry.message_id);
  return ids;
...
ErrorKind connect_error_kind(std::string_view code) {
  if (code == "unauthenticated") return ErrorKind::Unauthenticated;
  if (code == "invalid_argument" || code == "failed_precondition" || code == "out_of_range") {
    return ErrorKind::InvalidArgument;
  }
  if (code == "not_found") return ErrorKind::NotFound;
  if (code == "unimplemented") return ErrorKind::Unimplemented;
  if (code == "unavailable" || code == "deadline_exceeded" || code == "canceled") {
    return ErrorKind::Transport;
  }
  return ErrorKind::ServerError;
}

ErrorKind http_status_error_kind(std::string_view status) {
  if (status == "400") return ErrorKind::InvalidArgument;
...
std::optional<Error> decode_connect_error_body(const std::vector<std::uint8_t>& body) {
  try {
    const std::string text(body.begin(), body.end());
    const auto parsed = json::parse(text);
    const auto& object = json::as_object(parsed);
    const auto code = json::get_string(object, "code");
    if (code.empty()) return std::nullopt;
    return error_with_message(connect_error_kind(code), json::get_string(object, "message"));
  } catch (const SdkError&) {
    return std::nullopt;
  }
}

Error bad_status_error(const StreamState& state) {
  if (auto error = decode_connect_error_body(state.response); error.has_value()) return *error;
  const std::string body(state.response.begin(), state.response.end());
  const auto message = body.empty() ? "gateway returned HTTP status " + state.http_status : body;
  return error_with_message(http_status_error_kind(state.http_status), message);
}

void fail_bad_status(const StreamState& state) {
  if (state.http_status.empty() || state.http_status == "200") return;
  throw SdkError(bad_status_error(state));
}

Error end_stream_error(const envelope::EndStream& end) {
  return error_with_message(connect_error_kind(end.code.value_or("unknown")),
                            end.message.value_or("Connect stream failed"));
}
...
QueueAcquireResult unavailable_queue_acquire() { throw SdkError(live_transport_unavailable_error()); }

QueueBatchResult unavailable_queue_batch() { throw SdkError(live_transport_unavailable_error()); }
#else
...
private:
  ...
  static std::optional<std::vector<std::uint8_t>> take_message(std::vector<std::uint8_t>& bytes) {
    ...
    if (auto end = std::get_if<envelope::EndStream>(&decoded); end != nullptr) {
      if (end->code.has_value()) throw SdkError(end_stream_error(*end));
      bytes.clear();
      return std::nullopt;
    }
...
QueueAcquireResult live_transport_queue_acquire(
  ...
  (void)group;
  (void)max;
  (void)lock_duration_ms;
  (void)session_id;
  return unavailable_queue_acquire();
#else
  Http2Connection connection(parse_endpoint(endpoint));
  const auto stream_id = connection.submit(
      make_queue_acquire_http_request(topic, group, max, lock_duration_ms, session_id, bearer));
  return decode_queue_acquire_response(connection.read_unary(stream_id));
#endif
}

QueueBatchResult live_transport_queue_acknowledge(
  ...
  (void)session_id;
  (void)entries;
  return unavailable_queue_batch();
#else
  auto request = make_queue_acknowledge_http_request(session_id, entries, bearer);
  Http2Connection connection(parse_endpoint(endpoint));
  const auto stream_id = connection.submit(std::move(request));
  return decode_queue_batch_response(connection.read_unary(stream_id), queue_message_ids(entries));
#endif
}

QueueBatchResult live_transport_queue_renew(
    const std::string& endpoint, const std::optional<std::string>& bearer,
    const std::string& session_id, const std::vector<QueueRenewEntry>& entries) {
#ifndef CRABKA_CPP_HAS_NGHTTP2
  (void)endpoint;
  ...
  return unavailable_queue_batch();
#else
  auto request = make_queue_renew_http_request(session_id, entries, bearer);
  Http2Connection connection(parse_endpoint(endpoint));
  const auto stream_id = connection.submit(std::move(request));
  return decode_queue_batch_response(connection.read_unary(stream_id), queue_message_ids(entries));
#endif
...
LiveTransportRequestForTest live_transport_queue_acquire_http_request_for_test(
    const std::string& topic, const std::string& group, std::uint32_t max,
    std::uint64_t lock_duration_ms, const std::string& session_id,
    const std::optional<std::string>& bearer) {
  return make_queue_acquire_http_request(topic, group, max, lock_duration_ms, session_id, bearer);
}

LiveTransportRequestForTest live_transport_queue_acknowledge_http_request_for_test(
    const std::string& session_id, const std::vector<QueueAckEntry>& entries,
    const std::optional<std::string>& bearer) {
  return make_queue_acknowledge_http_request(session_id, entries, bearer);
}

LiveTransportRequestForTest live_transport_queue_renew_http_request_for_test(
    const std::string& session_id, const std::vector<QueueRenewEntry>& entries,
    const std::optional<std::string>& bearer) {
  return make_queue_renew_http_request(session_id, entries, bearer);
}

QueueAcquireResult live_transport_queue_acquire_response_for_test(
    const std::vector<std::uint8_t>& response) {
  return decode_queue_acquire_response(response);
}

QueueBatchResult live_transport_queue_acknowledge_response_for_test(
    const std::vector<std::uint8_t>& response, const std::vector<QueueAckEntry>& entries) {
  return decode_queue_batch_response(response, queue_message_ids(entries));
}

QueueBatchResult live_transport_queue_renew_response_for_test(
...
LiveTransportResponseLifecycleForTest live_transport_response_lifecycle_for_test() {
  ...
  record_response_status(bad_status, "400");
  const std::string connect_http_body =
      R"({"code":"invalid_argument","message":"bad request"})";
  append_response_body(bad_status,
                       reinterpret_cast<const std::uint8_t*>(connect_http_body.data()),
                       connect_http_body.size());
  ...
  try {
    ...
    connect_http_error = error.error();
  }

  const std::string end_stream_body =
      R"({"error":{"code":"not_found","message":"missing"}})";
  const auto end_stream_frame = envelope::encode(
      0x02, std::vector<std::uint8_t>(end_stream_body.begin(), end_stream_body.end()));
  const auto decoded_end_stream = envelope::decode_one(end_stream_frame);
  std::optional<Error> connect_end_stream_error;
  if (const auto* end = std::get_if<envelope::EndStream>(&decoded_end_stream); end != nullptr) {
    connect_end_stream_error = end_stream_error(*end);
  }
  ...
  return LiveTransportResponseLifecycleForTest{
      ...
      .connect_http_error = std::move(connect_http_error),
      .connect_end_stream_error = std::move(connect_end_stream_error)};
}
