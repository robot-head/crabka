...
type mockMessage struct {
	...
	partition       int32
	offset          int64
	queueDeliveries map[string]*mockQueueDelivery
}

type mockQueueDelivery struct {
	state         mockQueueMessageState
	sessionID     string
	deliveryCount int32
}

type mockQueueSession struct {
	topic string
	group string
	max   uint32
}
...
type mockStore struct {
	...
	queueSessions      map[string]mockQueueSession
}
...
func newMockStore() *mockStore {
	return &mockStore{nextQueueSessionID: 1, queueSessions: make(map[string]mockQueueSession)}
}
...
func (s *mockStore) publish(endpoint string, record Record) (PublishResult, error) {
	...
	stored := mockMessage{record: cloneRecord(record), partition: 0, offset: offset}
	s.messages = append(s.messages, stored)
	return PublishResult{Partition: 0, Offset: offset, Deduplicated: false}, nil
}

func (m *mockMessage) queueDelivery(group string) *mockQueueDelivery {
	if m.queueDeliveries == nil {
		m.queueDeliveries = make(map[string]*mockQueueDelivery)
	}
	delivery := m.queueDeliveries[group]
	if delivery == nil {
		delivery = &mockQueueDelivery{state: mockQueueMessageAvailable}
		m.queueDeliveries[group] = delivery
	}
	return delivery
}
