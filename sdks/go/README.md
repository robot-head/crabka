...
- `Queues()` implements queue v1.1 acquire / acknowledge / renew. Pass a returned session ID to `AcquireWithSession` for subsequent polls.
...
- Manual per-offset subscribe ack is not a public SDK mode yet; subscribe uses
  `auto_commit=true` even though the gateway now enforces the MSG-3 contiguous
  explicit-ack contract for lower-level clients.
- Topic auto-provision is not exposed.
- CloudEvents consume is not exposed as a typed SDK helper; callers receive the
  ordered raw headers and can apply the gateway's now-defined MSG-1 mapping.
