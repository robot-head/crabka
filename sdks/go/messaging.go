...
func toProtoFilter(filter *Filter) (string, error) {
	...
	for _, segment := range strings.Split(field, ".") {
		if !validFilterSegment(segment) {
			return "", errorWithMessage(InvalidArgument, "filter path must contain identifier segments")
		}
	}
	...
	default:
		return "", errorWithMessage(InvalidArgument, "filter value must be a string, number, boolean, or null")
	}
}

func validFilterSegment(segment string) bool {
	for index, character := range segment {
		if index == 0 && character != '_' && !isASCIILetter(character) {
			return false
		}
		if index > 0 && character != '_' && !isASCIILetter(character) && (character < '0' || character > '9') {
			return false
		}
	}
	return segment != ""
