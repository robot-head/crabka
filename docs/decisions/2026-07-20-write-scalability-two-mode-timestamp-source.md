...
- **Distributed mode — HLC (HA / geo)** (service, proposed) — Hybrid Logical Clock per node/region. No central RTT floor, no single failure domain. Cross-shard commit_ts *ticks strictly after* max(start_ts, participant HLCs) — observe-then-allocate, never a bare max, so commit_ts > start_ts always holds. Each node persists/recovers its max issued stamp (or waits out physical time) so a backward wall-clock step never mints below a committed version, and refuses service if NTP error approaches max_offset. Reads handle an uncertainty window. The shared physical-ish clock is what lets a Kafka partition and a SQL row share a cross-domain order. Accept slightly less efficiency for HA+geo. (Multi-node identity — a node discriminator in the stamp, or minting txn identity separately from commit order — is a follow-up the multi-node stamping work must settle.)
...
**Decision: Live promotion: seed the HLC from the LogicalTso horizon ★ agent-recommended** — Every stamp is one packed `u64` (physical ms in the high bits, logical counter in the low), so a LogicalTso stamp is exactly a packed HLC with physical = 0 and sorts below every real HLC stamp under the same numeric/big-endian order MVCC already uses. On promotion each node folds the persisted TSO horizon into its HLC (an `observe`) before serving its first stamp. No data migration and no rewrite of existing versions/intents — legacy stamps compare correctly against new HLC stamps by construction.

- Pros: Monotone by construction — no timestamp can go backwards across the switch; No dump/reload; existing intents and versions stay valid unchanged (the packed encoding makes them valid HLC values); Turns the two impls into a continuum rather than a cliff
...
**Decision: Stamp is an additional internal coordinate; offsets untouched ★ agent-recommended** — The per-partition offset stays the sole on-wire ordering and record format is byte-exact. The TimestampSource stamp is stored internally alongside, used only for cross-domain txn ordering and snapshot reads. The stamp is **assigned by the partition leader at append** (folded/clamped into offset order at the same seam that assigns offsets), not obtained independently by producers — so within a partition stamp order can never contradict offset order, and there is no way to write the "offset 0 = stamp 100, offset 1 = stamp 90" inversion that a producer-chosen stamp would allow.
