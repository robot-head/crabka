...
Add `TenantState::ResumeRequested`; helper transitions `request_resume(&mut self, tenant)` (upsert only if current state is `Suspended` — read-modify-write with version bump; a lost race with another activator's identical write is harmless by fold semantics) and `mark_active` / `mark_suspended` for the compute's use. *(Scaling-review amendments:)* the record also gains `wal_generation: u64` (bumped by the controller when it parks a suspended tenant's topic — recovery compares it against the manifest's generation) and `suspend_max_checkpoint_bytes: Option<u64>` (the size gate: tenants whose last checkpoint exceeds it stay warm; fleet default + per-tenant override). Steps: failing fold/transition unit tests (resume-request on Active is a no-op; duplicate resume-requests collapse; suspend→resume→active sequences fold correctly under reordering of distinct versions; generation only ever increases under fold), implement, nextest/clippy/fmt, commit `feat(gres): tenant lifecycle states in the registry`.
...
**Files:** Modify `crates/operator/src/controller/gres_tenant.rs` (+ its test file), `src/controller/gres.rs` (+ its test file), the registry seam on `Context` if the watch surface needs extending; **plus** `crates/gres-substrate/src/recover.rs` and `crates/gres-control/src/registry.rs` for the generation-aware recovery below.

`GresTenant`: reconcile now also derives desired replicas from the registry state (`Active`/`ResumeRequested` → 1, `Suspended` → 0) — the registry watch (from G-4's `RegistryLike` seam) feeds a requeue channel so state flips reconcile promptly; status surfaces the lifecycle phase. *(Scaling-review amendments:)* on `Suspended` the controller **parks the topic** — verifies the final manifest exists (registry record carries its offset/generation), deletes `__gres_wal.<t>` via `delete_topics`, and bumps `wal_generation` in the registry; on `ResumeRequested` it recreates the topic (ensure at the current generation) before scaling to 1. **Generation-aware recovery:** `recover(...)` takes the registry record's `wal_generation`; when the newest manifest's generation is older, the topic is fresh — tail replay runs from offset 0 (to the compute's own barrier, which is immediately the first record) instead of `covered_offset + 1`, and the torn-truncation check is skipped for the fresh-topic case (pinned by a test: park → resume → exact state; and the negative: a missing manifest with a parked topic still refuses). `Gres`: the render input marks suspended tenants → activator endpoint (Task 3's Service), active tenants → compute Service; the activator Deployment/Service joins the fleet's rendered workloads (image from config, replicas from spec); **the resume-side flip back to direct routing is lazy** — the fleet controller batches re-renders on its ordinary reconcile cadence, and nothing on the wake path waits for it (the activator pipes directly; mock test pins that ResumeRequested→Active produces no reconcile-blocking render). Steps: mock-harness tests first (suspend flip → scale-to-0 + park + re-render sequence; ResumeRequested → topic-ensure + scale-to-1 with no render in the sequence; activator workload present), implement, nextest/clippy/fmt, commit `feat(operator): registry-driven suspend/resume orchestration with topic parking`.
