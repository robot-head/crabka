...
**Latest review update:** The polymorphic multirange wave keeps the exact pass count at 8 / 231 with 223 semantic failures and zero infrastructure failures. The complete serial diff shrinks from 260012 changed lines / 4519 hunks to 259629 / 4544, while focused `multirangetypes` falls from 874 / 34 to 725 / 41 as valid routine calls execute deeper. Traditional and compatible multirange pseudo-types now participate in argument matching, subtype unification, overload selection, and result derivation; generic `multirange(anyrange)`, multirange bound accessors, SQL OUT/INOUT/TABLE inference, and PL/pgSQL positional polymorphic calls match the upstream block. Review fixes the next dependency as explicit companion naming and collision rules, multirange arrays/catalog rows, domains over and beneath multiranges, and exact diagnostics, followed by exclusion constraints/expression indexes and catalog-visible tablespace/operator-class lifecycle. The monotone baseline is not ratcheted because the fixture-enabled schedule still contains worsened and changed-fingerprint tests.
...
- [ ] Implement explicit multirange companion naming/collision rules, multirange arrays and catalog rows, domains over and beneath multiranges, and remaining exact diagnostics.
