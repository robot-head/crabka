...
**Latest review update:** Multirange arrays now reuse the native array cast, schema/value storage, wire, and parameter-OID paths, expose PostgreSQL 18.4's six built-in array OIDs and scalar/array `pg_type` links, and preserve dynamic user companion identity without changing the four-OID user-type allocation stride. The complete upstream arrays-of-multiranges block now matches exactly; focused `multirangetypes` falls from 706 changed lines / 41 hunks to 686 / 41, and the complete serial diff falls from 259608 / 4544 to 246540 / 4476. This run completed all 231 tests, but passed 7 rather than the prior infrastructure-clean 8 because `test_setup` hit the blocking-query memory budget while copying `tenk2`; the prior exact `test_setup` result and unchanged statement isolate that as a run-specific resource regression to recheck, not an arrays semantic change. Review fixes the next dependency as range/multirange subtypes over arrays and composites, domains over and beneath multiranges, then exact diagnostics, exclusion constraints/expression indexes, and catalog-visible tablespace/operator-class lifecycle. The monotone baseline is not ratcheted because the fixture-enabled schedule still contains worsened and changed-fingerprint tests.
...
- [ ] Implement ranges/multiranges over array and composite subtypes, domains over and beneath multiranges, and remaining exact diagnostics.
