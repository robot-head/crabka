...
**Latest review update:** Range-family operator resolution now tries nominal types first, preserving ranges whose subtype is a domain, then falls back through domain storage types, allowing domains over ranges and multiranges to use the base operators. Existing cast-time domain checks remain authoritative, so valid values execute and invalid multiranges still raise the declared check constraint. Both upstream domain blocks now match exactly; focused `multirangetypes` falls from 660 / 41 to 646 / 41 and the complete serial diff falls from 246480 / 4476 to 246452 / 4476. All 231 tests complete, while the recurring `test_setup` memory-budget error keeps the exact count at 7 rather than the prior infrastructure-clean 8. Review fixes the next dependency as `varbit` range subtypes, then exact diagnostics, exclusion constraints/expression indexes, and catalog-visible tablespace/operator-class lifecycle. The monotone baseline is not ratcheted because the fixture-enabled schedule still contains worsened and changed-fingerprint tests.
...
- [ ] Implement `varbit` range subtypes and remaining exact diagnostics.
