...
**Latest review update:** `DROP TYPE … CASCADE` now follows transitive user-type dependencies and removes a composite subtype's range and multirange companion from both durable catalog and live registry. The existing record-to-JSON conversion now serves `row_to_json(record [, pretty])`, retaining compact `json` output rather than `jsonb` spacing. This clears the stale `two_ints_range` collision and makes both range/composite JSON result blocks exact: focused `rangetypes` falls from 459 / 18 to 447 / 18, `multirangetypes` from 686 / 41 to 663 / 41, and `json` from 2513 / 35 to 2498 / 35. The complete serial diff falls from 246540 / 4476 to 246491 / 4476. All 231 tests complete, while the same run-specific `test_setup` memory-budget error keeps the exact count at 7 rather than the prior infrastructure-clean 8. Review fixes the next dependency as `ALTER TYPE … ADD ATTRIBUTE` with composite self-inclusion rejection, domains over and beneath multiranges, then exact diagnostics, exclusion constraints/expression indexes, and catalog-visible tablespace/operator-class lifecycle. The monotone baseline is not ratcheted because the fixture-enabled schedule still contains worsened and changed-fingerprint tests.
...
- [ ] Implement `ALTER TYPE … ADD ATTRIBUTE` with composite self-inclusion rejection, domains over and beneath multiranges, and remaining exact diagnostics.
