...
**Latest review update:** `varbit` now resolves through the existing ordered variable-width scalar storage, making both upstream `varbitrange` and `varbitmultirange` blocks exact. Range-to-multirange adjacency now reuses the existing external-boundary helper instead of treating internal component edges as adjacent, fixing all five wrong counts. Across the two complete serial runs, the raw `rangetypes` diff falls to 390 / 15, `multirangetypes` to 617 / 39, `bit` to 998 / 3, and the complete serial diff from 246452 / 4476 to 246390 / 4473. PostgreSQL's serial and parallel self-checks pass and all 231 Gres tests complete; the recurring `test_setup` memory-budget error keeps the exact count at 7 rather than the prior infrastructure-clean 8. The wire error model now supports PostgreSQL's optional one-based `P` source-position field. Review rejected attaching every parser offset: that improved 48 tests but worsened 139 because many current parse errors stand in for SQL PostgreSQL accepts. Exact caret diagnostics therefore require expression/source spans and targeted attachment after the semantic parser gaps are removed. Exclusion constraints/expression indexes and catalog-visible tablespace/operator-class lifecycle remain next. The monotone baseline is not ratcheted because the fixture-enabled schedule still contains worsened and changed-fingerprint tests.
