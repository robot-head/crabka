...
- [ ] Complete tablespace lifecycle. Durable create/alter/drop rows, option/path validation, owner/rename changes, `pg_tablespace`, and `pg_tablespace_location` are implemented; relation/index placement plus dependency-safe drop remain. Focused `tablespace` improves from 797 / 7 to 780 / 5 and the complete serial artifact to 167418 / 4471.
