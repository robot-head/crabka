...
- [ ] Complete operator class/family lifecycle. Durable create metadata, implicit family creation, OID linkage, and `pg_opclass`/`pg_opfamily` visibility are implemented. Alter/drop/member semantics and built-in rows remain. The complete serial artifact improves to 167370 / 4476; `alter_generic` is 477 / 16 and `opr_sanity` 1190 / 79.
