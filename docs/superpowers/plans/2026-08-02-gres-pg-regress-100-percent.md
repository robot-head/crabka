...
**Latest review update:** The explicit companion naming wave keeps the exact pass count at 8 / 231 with 223 semantic failures and zero infrastructure failures. The complete serial diff shrinks from 259629 changed lines / 4544 hunks to 259608 / 4544, while focused `multirangetypes` falls from 725 / 41 to 706 / 41. `multirange_type_name` now survives parsing, durable catalog serialization, hydration, and schema-relative resolution; built-in, user-type, automatic-companion, and wrong-range-constructor collisions are rejected through the shared type namespace. Review fixes the next dependency as multirange arrays/catalog rows and domains over and beneath multiranges, then exact diagnostics, exclusion constraints/expression indexes, and catalog-visible tablespace/operator-class lifecycle. The monotone baseline is not ratcheted because the fixture-enabled schedule still contains worsened and changed-fingerprint tests.
...
- [ ] Implement multirange arrays and catalog rows, domains over and beneath multiranges, and remaining exact diagnostics.
