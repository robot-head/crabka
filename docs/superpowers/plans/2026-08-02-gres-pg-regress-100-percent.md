...
**Latest review update:** `ALTER TYPE … ADD ATTRIBUTE` now preserves field type/collation, updates durable composite metadata, rejects duplicate/pseudo-type fields, and walks arrays, domains, ranges, multiranges, and named composites to reject direct or indirect recursive containment with PostgreSQL's `42P16` diagnostic. The previously failing self-inclusion statements now match in all three owning files: focused `rangetypes` falls from 447 / 18 to 444 / 18, `multirangetypes` from 663 / 41 to 660 / 41, and `alter_table` from 5226 / 107 to 5223 / 107. The complete serial diff falls from 246491 / 4476 to 246480 / 4476. All 231 tests complete, while the recurring `test_setup` memory-budget error keeps the exact count at 7 rather than the prior infrastructure-clean 8. Review fixes the next dependency as domains over multiranges (operator type unwrapping plus constraint enforcement), then `varbit` range subtypes, exact diagnostics, exclusion constraints/expression indexes, and catalog-visible tablespace/operator-class lifecycle. The monotone baseline is not ratcheted because the fixture-enabled schedule still contains worsened and changed-fingerprint tests.
...
- [ ] Complete domains over multiranges, `varbit` range subtypes, and remaining exact diagnostics.
