...
**Latest review update:** The native multirange wave keeps the exact pass count at 8 / 231 with 223 semantic failures and zero infrastructure failures. It improves 96 recorded failures, including `rangetypes` and `multirangetypes`, while deeper execution expands the aggregate mismatch to 261849 changed lines / 4535 hunks. Built-in multiranges now have native type/value identity, canonicalization, storage, wire encoding, constructors, singleton range casts, comparison, containment, overlap, and range-left directional operators; focused `rangetypes` is 185 changed lines / 15 hunks. Review fixes the next dependency order as durable user-range companion multiranges; subtype literal coercion and reverse/multirange directional operators; `unnest` and named support functions/operators; then polymorphic multirange resolution. The monotone baseline is not ratcheted because 17 tests worsen and 38 retain changed fingerprints.
