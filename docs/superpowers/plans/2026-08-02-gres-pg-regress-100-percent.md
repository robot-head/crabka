...
**Latest review update:** The multirange aggregate wave keeps the exact pass count at 8 / 231 with 223 semantic failures and zero infrastructure failures. The complete serial diff shrinks from 260190 changed lines / 4521 hunks to 260012 / 4519, while focused `multirangetypes` falls from 1053 / 36 to 874 / 34. `range_agg` now unions both range and multirange inputs into canonical multiranges, including user-defined range companions; `range_intersect_agg` now intersects either range or multirange inputs and preserves the polymorphic result type. Review fixes the next dependency as polymorphic multirange resolution, followed by exact input diagnostics, explicit companion naming, array/catalog rows, exclusion constraints/expression indexes, and catalog-visible tablespace/operator-class lifecycle. The monotone baseline is not ratcheted because 17 tests worsen and 38 retain changed fingerprints.
...
- [ ] Complete polymorphic multirange argument/result resolution and generic multirange constructors before broadening into exact diagnostics and catalog lifecycle.
