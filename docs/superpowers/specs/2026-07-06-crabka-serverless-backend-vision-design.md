...
**What is spec-only, not landed:** the diskless quorum WAL — the "safekeeper" this whole vision stands on. The consensus *kernel* is real code (the payload-blind `QuorumStateMachine`, `crates/kraft-core/src/core.rs:30`, which advances a durable-offset watermark rather than reducing state). But the `WalStore` seam, `QuorumWalStore`, `LocalFsyncWal`, the object-store flush worker, and the KRaft offset sequencer (slices 1–6) are **approved designs, not merged code** — `crates/broker/src/wal/` does not exist. Any sentence that implies the safekeeper is built is an over-claim.
...
- Record headers end-to-end and a transport-agnostic `GatewayRecord.headers` seam (`crates/grpc-gateway/src/types.rs:23`).

**The net-new.**
- A thin polyglot serverless SDK (idiomatic publish/subscribe/queue-consume, at-least-once ack helpers, topic auto-provisioning) — the proto exists, the packaged BaaS SDK does not.
- A **CloudEvents** Kafka + HTTP protocol binding. Today the gateway binds *no* CloudEvents semantics, and worse, it **drops record headers on multiple paths**: `webhook.rs` emits `headers: vec![]` (`:198`, `:249`), `outbound.rs` `render_envelope` emits a proprietary envelope that omits headers (`:285`), and `DecodedConsumerRecord` has no `headers` field (`consume.rs:16`). The CloudEvents binding must first *fix the header drop*, since binary-mode CloudEvents rides in exactly those headers.
- Per-offset commit / explicit ack in the `Subscribe` stream (today `SubscribeAck` offsets are advisory, `gateway.proto:108`).
- A share-group-backlog → KEDA/autoscaler bridge. **Correctness landmine to flag before shipping:** `describe_share_group_offsets.rs` returns `lag = −1` when the data partition is not local (`:231`), which a naive KEDA bridge reads as a false scale-*down*. The bridge must aggregate fleet-wide and treat `−1` as "unknown," never "zero."
...
**Key risk.** Low, and mostly packaging. The engine is ~90% built; the risks are (1) DX/SDK ergonomics, (2) the CloudEvents binding correctly restoring the headers the gateway currently drops, and (3) the `lag = −1` false-scale-down trap in any backlog autoscaler. This is the face to ship first and use to demonstrate the wedge; do not let its readiness inflate the others.
...
- A unified thin SDK spanning all faces behind one project credential, and **polyglot TS/Python bindings** (only Rust crates + one `gateway.proto` exist today — a from-scratch build or codegen off the proto).
...
**Thesis.** Crabka already carries the two ingredients nobody else combines: a **Schema Registry** (Avro/Protobuf/JSON) and an **Arrow/DataFusion** engine. Make them a first-class **column type system** — a column can be a protobuf message, an Avro record, or an Arrow struct (nested/repeated) — and turn realtime subscription filtering from today's *advisory JSONPath equality on flat columns* into a **server-enforced, complex-predicate engine over those nested types, evaluated by DataFusion.** This is a wedge Supabase structurally cannot match: its `postgres_changes` filter is advisory equality on top-level columns; Crabka's is a real query engine over nested, schema-typed data on the same substrate. It also **closes the security landmine in Chapter D** — the advisory filter that leaks rows becomes an enforced one.
...
- The `Subscribe` stream + `FieldPredicate` — the advisory JSONPath-equality filter this chapter *replaces* (`crates/grpc-gateway/proto/…/gateway.proto:34`, `crates/grpc-gateway/src/streaming.rs:89`).

**The net-new.**
- **Arrow as the unifying decode target** for protobuf/avro/json column values — extend `RowBridge` so all three schema formats resolve, via the registry, to one Arrow representation the filter and the lakehouse both consume.
- A **complex predicate engine**: compile a subscription's filter to a DataFusion expression (or a predicate AST) evaluated over the decoded Arrow row — boolean combinators, nested/dotted field access, repeated/map element predicates (`items[*].price > 100`), comparisons, `LIKE`/regex, `IN`, null-handling, optional UDFs. Far beyond `FieldPredicate`'s single-field equality.
- **Server-enforced** filtering — the filter *decides delivery*, not just hints it — composed with RLS (Chapter E) so a predicate can never widen what a subscriber is authorized to read.
...
**Key risk.** Filter correctness + **per-change-per-subscriber evaluation cost at fan-out scale** (a complex predicate over a decoded nested row, for every subscriber, on every change) — pushdown and shared evaluation are essential, not optional. Entanglement: server-enforcement must compose with RLS (Chapter E), and the **DB-column-type half depends on the unbuilt Postgres compute (Chapter C)**. The **topic-side half — complex filters over schema-typed *message* records — is near-term** (registry + `RowBridge` + DataFusion are all landed), so G ships in two halves: the messaging-record filter engine now, the DB-column types on the Postgres keystone's track.
...
```
   [ Diskless quorum WAL — slices 1-6 ]   ← spec-only; unblocks everything below the messaging line
             │
   ┌─────────┴─────────────────────────────────────────────┐
   │                                                        │
...
**Recommended first sub-service to design next: Messaging — including G's topic-side filter engine.** It is the most-built, highest-leverage, lowest-risk face — a thin DX/CloudEvents/SDK wrapper over landed, JVM-validated plumbing — and it is the one face shippable in the near term without waiting on the diskless WAL. Designing it next lets us prove the integration wedge concretely (one topic simultaneously serving pub/sub + KIP-932 queue + CDC feed + observability WAL, all on one bucket) and ship the native queue-backlog autoscaling signal, while the deep net-new IP — the pageserver keystone and the WAL it stands on — proceeds on its own longer track. **Do the three messaging fixes as part of that design:** restore the dropped record headers before binding CloudEvents, and treat the `lag = −1` non-local-partition case as "unknown," never "zero," in any autoscaler bridge. **Design G's topic-side filter engine as part of the same cycle:** a schema-typed, DataFusion-backed, *server-enforced* complex predicate over message records — the concrete proof of the schema-plane wedge and the fix for D's advisory filter, reusing only landed code (Schema Registry + `RowBridge` + DataFusion). Blob is explicitly *not* a candidate for "next" — it should be sequenced last, after Ch.2's lakehouse gives it its only reason to exist.
