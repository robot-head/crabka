...
- **pgbench parity claims** — pgbench requires primary keys, which the engine does not have yet; the chapter's gates use the conformance corpus and a deterministic synthetic OLTP workload instead. *(Now scheduled, not abandoned: the SQL-Parity Program's milestone M1 is stock pgbench green.)*
- **SQL-surface completeness as a chapter gate** — the chapter's slices gate on durability and lifecycle, not breadth. *(Breadth itself is now fully scheduled by the SQL-Parity Program below.)*
...
crabka-gres compute   (per tenant, single writer, disposable, pure Rust)
   ...
   └─ substrate durability (SqlEngine::replicated + SubstrateCommitter — G-2)
        ├─ local working store: fjall LSM on ephemeral disk (MemKv for tiny tenants)
        ├─ WAL: every Committer batch → __gres_wal.<tenant> (single partition,
        │        group-commit Kafka txns; transactional id __gres.<tenant> —
        │        epoch fences zombies)
...
spin-up (fence FIRST — the load-bearing order): InitProducerId (fences any
  predecessor) → produce barrier → restore newest same-generation manifest →
  replay tail to own barrier → serve
```
...
*(This section was the chapter's original sketch; it is edited to match the G-2 design's resolved shape after the PR panel review found the sketch still taught the superseded mechanism.)* The engine is constructed via `SqlEngine::replicated` and every durable mutation flows through its `Committer` seam — **not** a `Kv` wrapper — because Replicated mode is what forces the xid/rowid counter allocations into the batch stream. A single WAL-writer task owns a transactional producer and group-commits: each `Committer` batch becomes one `GRW1` record (`u8 version | u64 journal_seq | ops`; oversized batches chunk across records, atomic via the enclosing Kafka transaction) on the tenant's single-partition topic, one Kafka transaction per commit-group, waiters acked only after `EndTxn(commit)` and local apply. Statement batches await durability in v1 (async pipelining is a named optimization, deliberately deferred). A deterministic produce/EndTxn failure aborts the waiting SQL transactions with a clear error; an *indeterminate* outcome is writer-fatal — the process exits without answering waiters, and recovery resolves the truth from the log — never ack-then-hope, and never report-failure-then-discover-durable.
...
Two computes for one tenant must never interleave writes. Rather than inventing a lease service, the compute produces with `transactional.id = __gres.<tenant>`: a successor's `InitProducerId` bumps the producer epoch and the broker fences the predecessor, which gets hard produce errors and self-terminates. Crabka already implements transactional produce with KIP-faithful semantics, so correct failover costs zero new broker machinery. **The recovery ordering is fence-first and it is load-bearing** *(corrected after the PR panel review — an earlier sketch here had it reversed)*: `InitProducerId` (the fence) → produce the recovery barrier → restore the newest checkpoint → replay the tail to the compute's own barrier → serve. Fencing before reading the log's end is what guarantees the replayed state contains every append any predecessor ever acked; the G-7 model corpus carries a dedicated action (`ZombieAppendAfterEndRead`) whose counterexamples are exactly what the reversed order admits.

### Checkpoints bound both spin-up and log growth

A background checkpointer snapshots the working store at a point where the applied state equals the durable log prefix (so the snapshot corresponds exactly to a WAL offset), uploads it under the tenant's bucket prefix using the existing object-store plumbing, and writes the manifest last so a torn upload is invisible. After a checkpoint lands, the WAL topic is truncated up to the covered offset via DeleteRecords, so the replay tail stays bounded by checkpoint cadence. The refusal rule, stated correctly *(the PR panel review found an inverted, availability-breaking version here)*: recovery refuses to serve when the **log start has advanced past the newest same-generation manifest's covered offset** (truncation outran every usable checkpoint — bucket loss or interference); checkpoints and manifests carry a `wal_generation`, selection and pruning order by `(generation, offset)`, and a manifest older than the topic's current generation means a parked-and-resumed topic whose tail correctly replays from offset 0 (G-3/G-5 define the mechanics). Spin-up cost scales with live database size (the G-3 garbage horizon keeps "live" honest); removing that scaling is exactly the named follow-on's job.
...
- **Broker:** zero code changes. The WAL path is ordinary transactional produce + DeleteRecords over the Kafka wire; internal-topic naming (`__gres_wal.<tenant>`) follows the `__diskless_wal_index` precedent. Broker *configuration* is required for tenant isolation, though *(added after the PR panel review)*: per-tenant Kafka principals with ACLs scoping each compute to its own topics/transactional ids — G-4's "Substrate authorization" decision, using the broker's existing ACL + KIP-554 machinery.
