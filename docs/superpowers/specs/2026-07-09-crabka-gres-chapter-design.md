...
**The chapter scales on one axis: tenant count.** The product is a fleet of many small, mostly-idle Postgres databases. It does not — and structurally cannot, under approach A or approach B — scale a *single* database's writes horizontally: one writer process, one WAL partition, one commit path per tenant. The donor's range-partitioning/2PC layer (the one mechanism that could shard a single database) is deliberately not vendored; write scale-out would be its own future chapter.

**Per-tenant envelope (v1, approach A, order-of-magnitude):** the vendored executor is a full-scanning, materializing tree-walker with no indexes, so hot tables are comfortable to ~10⁴ rows and degrade visibly by ~10⁵–10⁶ row-versions; nested-loop joins constrain multi-table queries well below that. The commit path awaits durable produce per statement (group-commit amortized), putting a few-ms floor per write statement and an absolute best of low-thousands of commits/s per tenant. Checkpoint, suspend, and cold-start costs scale with store size (approach A's stated trade) and — until the G-3 garbage horizon runs — with write history. Tenants that outgrow this envelope are the graduation path's problem: the PG chapter's real-Postgres tier, or approach B for size/read-replicas.
...
**What breaks first:** for one growing tenant — the engine itself (full scans, then unvacuumed MVCC growth), long before Kafka. For the fleet — the config/RELOAD pipeline around ~10³, then broker per-topic overhead around ~10⁴. Kafka-as-log is the last thing to break on both axes.
