...
*(Added after the scaling review, which found both undecided — and both headline serverless behaviors break under the wrong defaults.)* **`pooler_mode = "transaction"` is the rendered default.** Session pooling would hold one backend session per client connection for the connection's lifetime — with app-side poolers that means forever, `sessions > 0` at the compute permanently, and G-5's scale-to-zero never fires. Transaction pooling has the standard Postgres-pooler semantics caveats — session-level `SET` (the engine faithfully implements PostgreSQL's session/`SET LOCAL` split), prepared statements, and advisory session state don't survive transaction boundaries across multiplexed backends — which are documented in the product docs and, more importantly, **tested**: the conformance corpus runs *through PgDog in transaction mode* as a CI leg, so any engine/pooler semantic interaction is caught by the differential harness rather than by users. The render also pins PgDog's backend idle disconnects (`idle_timeout`/`server_lifetime`) so pooled server connections drain and suspend can fire. And the timeout budget is owned by the renderer, not left to PgDog defaults: rendered `connect_timeout`/`connect_attempts`/`checkout_timeout` must jointly exceed the G-5 cold-start ceiling (the renderer takes the ceiling as an input and enforces the floor), so a wake never loses a race against the front door's own patience.
...
*(Added after the PR panel review, which found east-west authorization silently unaddressed — with per-tenant topics and a shared registry, any compute could otherwise fetch another tenant's entire database, and the cluster-scoped registry made every tenant's verifier co-readable. The broker already ships the mechanism: ACLs and KIP-554 SCRAM, both implemented and unused by this chapter until now.)* Every tenant compute gets its **own Kafka principal** `gres-<tenant>` (a SCRAM user provisioned through `AlterUserScramCredentials`, exactly the operator's existing KafkaUser path), and provisioning applies **ACLs** scoping that principal to: READ/WRITE/DESCRIBE on the `__gres_wal.<tenant>` topic prefix (all ranges), WRITE on the `__gres.<tenant>` TransactionalId prefix (fencing rides transactional ids, which are ACL-able resources), DeleteRecords rights on its own topics (checkpoint truncation), and READ on `__gres_cfg.<tenant>` — a new **per-tenant config topic** (compacted, one partition) into which the control plane writes that tenant's record. Computes **no longer tail the global registry at all**: `__gres_tenants` is ACL'd to the control-plane principal (operator, CLI, balancer, activator — fleet components), which closes the verifier-co-readability hole without splitting the control plane's own view. Object-storage isolation follows the same posture: per-tenant bucket prefixes with **scoped credentials per compute** where the backend supports prefix-scoped IAM (stated per-backend; a shared-credential deployment is a documented degraded mode, named in Risks, never a silent default). The e2e gate grows the negative assertions: a tenant principal's fetch of another tenant's WAL topic and of the registry must be DENIED.
...
- **A single all-privileged SQL role per tenant until the parity program's D8 wave** (roles/GRANT/RLS) — an explicit, documented interim posture, with the additional note that `SET ROLE` under transaction pooling is unsafe-by-construction and stays refused until D8 designs it *(named after the PR panel review — silence here was the bug)*.
- **Shared object-store credentials as a degraded mode:** where the backend cannot scope credentials to a tenant prefix, computes can read other tenants' checkpoints — deploy-time documented, mitigated by backend choice, never silent.
...
- Pooling: `pooler_mode = "transaction"` rendered by default, with backend idle disconnects pinned and the fleet timeout budget floored at the cold-start ceiling; conformance-through-PgDog is the semantics gate; ~10³ tenants per fleet cell.
