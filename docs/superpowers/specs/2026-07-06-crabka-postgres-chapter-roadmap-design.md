...
Materialization: (latest image ≤ LSN) ⊕ (deltas ≤ LSN via redo), plus image-layer creation and GC (both need redo; re-homed from PG-3). **Both crux decisions resolved in PG-4's cycle:** redo is **full native Rust** (bounded v1 rmgr set — XLOG-FPI/HEAP/HEAP2/BTREE/SEQ — with loud `UnsupportedRmgr` refusal; the correctness mechanism is a byte-exact differential against a **WAL-replayed standby**, redo-vs-redo); the page service is a **Crabka-native Connect RPC** (`GetPage`/`GetRelSize`), not Neon-pagestream-compatible. **PG-4b** (follow-on, now designed): SLRU/CLOG + multixact materialization through the layer store (the tagged `Key::{Rel, Slru, RelMeta}` enum — an explicit pre-build amendment to PG-3's key space; ingest-light/redo-full interpretation split), relation lifecycle (`SMGR` create/truncate + commit-carried drops + DBASE WAL_LOG passthrough / FILE_COPY refusal) making `GetRelSize` **exact** with `BlockBeyondEof`/`NotFound` semantics, and the five index-rmgr arm families (BRIN → hash → GiST → SP-GiST → GIN), each fixture-gated by its own standby differential; the extended gate adds byte-exact `pg_xact`/`pg_multixact` comparison — **unblocks PG-5's boot gate**.
