...
The patched-Postgres compute image (`smgr` → the Connect page service), one maintained patch set per supported PG major (containerized — operationally an OCI image). **Consequence of PG-4's protocol decision:** the smgr↔Connect client is bespoke (Neon's compute pieces can't be vendored) — likely a small Rust cdylib behind a C ABI, with the `unsafe`-boundary tension resolved in PG-5's own cycle. **The keystone conformance gate:** pgbench boots and runs against the disaggregated stack (PG-1 ingest live, PG-4b landed), with differential page-image checks vs stock Postgres — the PG-differential analogue of the JVM-differential culture.
