...
… commit group N+1 …                               gres/<tenant>/ckpt/<offset>-<epoch>/part-00000
                                                 ...
                                                 → DeleteRecords(covered_offset + 1)
                                                 → prune old ckpt prefixes (retain 2)

recovery (G-3 shape):
  fence (init_transactions) → produce barrier
  → list gres/<tenant>/ckpt/*/MANIFEST, pick highest (generation, offset)
  → restore parts (fjall ingestion / write_batch chunks) → verify counts+checksums
  → replay WAL from covered_offset + 1 until own barrier (merge rules, seq
    tripwire; manifest generation < topic generation ⇒ fresh topic, replay from 0)
...
- Layout: `gres/<tenant>/ckpt/<generation>-<offset>-<epoch>/part-NNNNN` + `MANIFEST` last; immutable; `(generation, offset)`-ordered selection and prune (retain 2, current generation's newest always kept); no CAS.
