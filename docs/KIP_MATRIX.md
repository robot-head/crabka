...
| **KIP-939** — 2PC participation | **Broker side complete behind opt-in `transaction.version=3`:** `enable2Pc` and `keepPreparedTxn` share the cluster gate + `TWO_PHASE_COMMIT` ACL; prepared state is never reaped; recovery identities persist/replay, fence older recovery clients, handle both epoch boundaries, and complete through `EndTxn`; abort markers reach local and remote leaders. Safety proven by the exhaustive `txn::two_pc_model` plus live broker recovery tests. | Native client `prepareTransaction` / `completeTransaction` APIs and Admin `forceTerminateTransaction` tooling. |
