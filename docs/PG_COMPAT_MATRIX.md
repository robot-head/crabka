...
| ALTER TABLE | Implemented | Bounded subset: `ALTER TABLE name RENAME TO new_name` atomically moves table and sharding metadata, rewrites affected index/ACL metadata, and preserves immutable row/index storage IDs; index names are unchanged. Quoted identifiers and duplicate/missing/wrong-object SQLSTATEs are supported. A rename fails clear with `0A000` while any stored view exists because view definitions are stored as SQL text without rewriteable dependency identities. `RENAME COLUMN` is parsed but fails clear with `0A000` for the same unresolved view/index dependency rewrite. `ADD [CONSTRAINT name] PRIMARY KEY (col, …)` desugars onto the local unique-index machinery: existing rows are back-validated all-or-nothing (NULL key → `23502` with PostgreSQL's "contains null values" message, checked before duplicates → `23505`), the key columns become NOT NULL for future writes, and the constraint-backed `<table>_pkey` index blocks a second primary key (`42P16`) and `DROP INDEX` (`2BP01`); sharded tables fail clear `0A000` until global enforcement exists. |
...
| INSERT | Implemented | Baseline DML; column defaults fire for omitted columns and explicit `DEFAULT`, `NOT NULL` is enforced, and Q1 starter `RETURNING` supports `*`, direct columns, simple scalar expressions, and aliases on local MVCC tables. `ON CONFLICT DO NOTHING`/`DO UPDATE` is implemented on local MVCC tables — see the `INSERT ... ON CONFLICT` major-feature row for the arbiter, `excluded`, RETURNING, and SQLSTATE contract, and for the sharded and inference-predicate refusals. |
...
| Item | Disposition | Notes |
|---|---|---|
| ARRAY expressions and operators | Implemented | One-dimensional arrays over the existing scalar types: `ARRAY[...]` constructors, `'{...}'` literals with PostgreSQL quoting/NULL rules in both directions, subscripting, `= ANY(expr)` / `<> ALL(expr)`, `unnest(...)` in FROM, `array_agg`, `array_length`, `cardinality`, `array_append`/`array_prepend`/`array_cat`, `array_to_string`, `string_to_array`, and the `\|\|`, `@>`, `<@`, `&&` operators. `int4[]`/`text[]` columns store and index array values through the canonical encoding, and `pg_type` carries real `typelem`/`typarray`/`typdelim` plus the `_`-prefixed array rows. Deferred: multidimensional arrays, slices (`a[1:2]` fails clear with 0A000), non-default lower bounds, `varchar[]`/`char(n)[]`/`regclass[]`/nested-array element types (0A000), `ARRAY(subquery)`, and LATERAL `unnest`. Divergence: `ARRAY` is now a reserved keyword, matching PostgreSQL, so it is no longer accepted as a bare identifier. |
...
| Scalar type `jsonb` | Implemented | `jsonb` values report OID 3802 and PostgreSQL's canonical text: object keys sorted by length then bytes, duplicate keys last-wins at parse time, `", "` / `": "` separators, and preserved numeric scale (`'1.00'::jsonb` prints `1.00`). Operators `->`, `->>`, `#>`, `#>>`, `@>`, `<@`, `?`, `?\|`, `?&`, `\|\|`, and `-`; functions `jsonb_build_object`/`jsonb_build_array`, `jsonb_array_length`, `jsonb_typeof`, `jsonb_extract_path`(`_text`), `jsonb_set`, and `to_jsonb`; aggregates `jsonb_agg` and `jsonb_object_agg`; text↔jsonb casts; storage plus unique indexes over the canonical encoding, so `{"a": 1, "b": 2}` and `{"b": 2, "a": 1}` (and `1.0` versus `1.00`) collide with 23505. An untyped string literal is resolved against a jsonb operand as PostgreSQL resolves `unknown`, so `body @> '{"name": "ada"}'`, `j = '{"a":1}'`, and `j \|\| '{"b":2}'` need no explicit `::jsonb`; adoption is jsonb-only, so `ARRAY['a'] \|\| 'b'` still appends an element. Divergences: jsonb string comparison is bytewise rather than collation-aware; function *arguments* are not adopted, so `jsonb_set(j, '{a}', '2')` still needs a spelled-out `text[]` path (42883 otherwise). jsonpath, `jsonb_each`, and `jsonb_path_query` stay deferred with the SQL/JSON rows. |
| `json` type as a `jsonb` input alias | Mapped(jsonb) | `json` is accepted as a column type, in casts, and as a parameter OID (both 114 and 3802), but every value is stored and returned as `jsonb`: output reports OID 3802 and canonical jsonb text, so PostgreSQL's whitespace-, key-order-, and duplicate-key-preserving `json` semantics are deliberately not reproduced. `pg_type` still carries real `json`/`_json` rows. |
...
| JSON_TABLE and SQL/JSON expressions | Wave-assigned(T3) | PG18 SQL/JSON surface. The `jsonb` type, its operators, and the `jsonb_*` functions are implemented (see the `jsonb` row); the SQL/JSON standard syntax — `JSON_TABLE`, jsonpath, `jsonb_path_query` — is still parser-rejected and owned by T3. |
...
| SQL/JSON constructors and aggregates | Wave-assigned(T3) | JSON type/function wave. The `jsonb_build_object`/`jsonb_build_array` constructors and the `jsonb_agg`/`jsonb_object_agg` aggregates are implemented (see the `jsonb` row); the SQL/JSON standard constructor syntax (`JSON_OBJECT(... VALUE ...)`, `JSON_ARRAY`, `JSON_OBJECTAGG`) is still parser-rejected and owned by T3. |
