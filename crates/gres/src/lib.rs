...
async fn attach_range0_read_barrier(
    ...
    let range0_follower_poll_interval = config.range0_follower_poll_interval;
    tokio::spawn(async move {
        loop {
            ...
            wait_for_range0_follower_refresh(&catalog_refresh_poke, range0_follower_poll_interval)
                .await;
        }
    });
    Ok(tenant_config)
...
mod tests {
    ...
    fn set_wal_policy_option(args: &mut ServeArgs, option: usize) {
        match option {
            ...
            10 => args.wal_producer_request_timeout_ms = PositiveMillis::new(1).ok(),
            11 => args.wal_producer_retries = NonNegativeI32::new(0).ok(),
            12 => args.wal_producer_retry_backoff_ms = PositiveMillis::new(1).ok(),
            13 => args.wal_producer_routing_retry_budget_ms = PositiveMillis::new(1).ok(),
    ...
    async fn configured_range0_follower_poll_and_poke_control_refresh() {
        ...
        let notified = tokio::spawn(async move {
            wait_for_range0_follower_refresh(&notified_poke, Duration::from_mins(1)).await;
        });
        tokio::task::yield_now().await;
        poke.notify_one();
