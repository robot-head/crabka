//! The backend id announced in `BackendKeyData` is more than a session label.
//! It names `pg_temp_<backend id>` in a catalog that every gateway of a
//! cluster shares. It must therefore be a cluster-wide name, and it must stay
//! the `int4` a client sends back in a `CancelRequest`.
...
/// small set of temporary namespaces. Ids above this range cannot.
