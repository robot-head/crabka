...
async fn wal_head_delegates_metadata_cardinality_stats_and_blocks() {
    ...
    assert_eq!(stats.head_stats.num_series, 3);
    assert_eq!(stats.head_stats.num_samples, 3);
    assert_eq!(
        stats.series_count_by_metric_name,
        expected_metric_name_stats()
    );
...
async fn prune_drops_old_samples() {
    ...
    assert_eq!(stats.series_dropped, 0);
...
fn offsets_track_low_and_high_water() {
    ...
    assert_eq!(head.high_water_offset(PartitionIndex(0)), None);
    assert_eq!(head.low_water_offset(PartitionIndex(0)), None);
    ...
    head.prune(i64::MAX);
    assert_eq!(head.high_water_offset(PartitionIndex(0)), Some(Offset(6)));
    assert_eq!(head.low_water_offset(PartitionIndex(0)), Some(Offset(5)));
}
