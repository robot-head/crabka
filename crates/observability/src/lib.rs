...
pub struct ServiceConfig {
    ...
    #[arg(long, value_parser = crabka_units::parse::time)]
    ...
    /// `512MiB`.
    #[arg(long, value_parser = crabka_units::parse::byte_size)]
    pub max_query_read: Option<ByteSize>,

    /// Ceiling on the length of the `LogQL` query *string* in bytes. A string
    /// length, not a data volume.
    #[arg(long)]
...
pub struct QuerierState {
    ...
    /// The length of the `LogQL` query *string* in bytes, not a data volume, so
    /// it stays a plain integer.
