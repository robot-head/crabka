...
fn format_with_add_scram_writes_credential_record() {
    ...
    // KIP-778 + group.version KIP-848) + SCRAM = 4 records.
    assert!(
        manifest.contains("\"record_count\": 4"),
        "manifest must list KRaftVersion + metadata.version + group.version + one SCRAM record, got: {manifest}",
    );
    assert!(
