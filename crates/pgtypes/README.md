...
Implements the PostgreSQL-faithful value semantics the Gres engine executes over: the `Datum` and `ColumnType` enums, numeric (arbitrary precision) and date/time arithmetic, cast rules, operator semantics, and both wire encodings. It is the root of the engine crate graph and has no sibling dependencies.
