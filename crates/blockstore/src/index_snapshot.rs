...
impl IndexSnapshotMaxBytes {
    /// Validate a maximum index-snapshot read size.
    ///
    /// # Errors
    ///
...
impl Default for IndexSnapshotMaxBytes {
    fn default() -> Self {
        Self::new(DEFAULT_INDEX_SNAPSHOT_MAX_BYTES)
            .expect("default index snapshot maximum is positive")
    }
}

impl fmt::Display for IndexSnapshotMaxBytes {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        self.0.fmt(formatter)
    }
}

impl FromStr for IndexSnapshotMaxBytes {
    type Err = String;
...
#[cfg(test)]
mod tests {
    use super::{IndexSnapshotMaxBytes, IndexSnapshotRetain};

    #[test]
    fn index_snapshot_settings_preserve_defaults_and_validate_input() {
        assert_eq!(
            IndexSnapshotMaxBytes::default().into_value(),
            256 * 1024 * 1024
        );
        assert_eq!(IndexSnapshotRetain::default().into_value(), 8);
        assert_eq!(
            "1".parse::<IndexSnapshotMaxBytes>()
        ...
        for invalid in ["0", "not-a-number", "-1", "18446744073709551616"] {
            assert!(
                invalid.parse::<IndexSnapshotMaxBytes>().is_err(),
