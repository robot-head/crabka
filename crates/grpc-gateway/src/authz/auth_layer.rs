...
mod tests {
    ...
    /// No Authorization header → no principal injected by this layer.
    #[tokio::test]
    async fn no_bearer_header_passes_through_without_principal() {
        let app = test_router();
    ...
    /// Valid bearer token → principal name injected.
    #[tokio::test]
    async fn valid_bearer_resolves_principal() {
        let app = test_router();
    ...
    /// Expired token → falls through, no principal injected.
    #[tokio::test]
    async fn expired_bearer_falls_through() {
        let app = test_router();
        let req = Request::builder()
            .uri("/")
            .header("Authorization", format!("Bearer {TOKEN_BOB_EXPIRED}"))
            .body(Body::empty())
            .unwrap();
        let resp = app.oneshot(req).await.unwrap();
        let body = axum::body::to_bytes(resp.into_body(), 1024).await.unwrap();
