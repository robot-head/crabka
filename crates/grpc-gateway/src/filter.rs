...
///
/// JSON decoding, Arrow dictionary decoding, and the `DataFusion` bridge all
...
impl ArrowCompiledFilter {
    fn compile(
        ...
        let expr = if let Ok(predicates) = parse_legacy_predicates(&sql) {
            datafusion_predicate_for_schema(&predicates, df_schema.as_arrow())?
        } else {
            let logical = ctx
                .parse_sql_expr(&sql, &df_schema)
                .map_err(datafusion_error)?;
            ctx.create_physical_expr(logical, &df_schema)
                .map_err(datafusion_error)?
        };
...
impl FieldPath {
    fn parse(input: &str) -> Result<Self, FilterCompileError> {
        ...
        if segments.len() != 1 {
            return Err(FilterCompileError::DataFusionUnavailable(
                "nested field predicates require the grpc-gateway arrow feature".to_string(),
            ));
        }
        ...
        if segments.len() != 1 {
            return Err(FilterCompileError::UnsupportedSql(
                "nested field predicates require Arrow/DataFusion schema compilation".to_string(),
            ));
        }
...
impl FilterValue {
    fn parse(input: &str) -> Result<Self, FilterCompileError> {
        ...
        if let Some(value) = NumericValue::parse(literal) {
    ...
    fn matches(&self, operator: ComparisonOperator, actual: &FieldValue) -> bool {
        match (self, actual) {
            ...
            (Self::Number(expected), FieldValue::Number(actual)) => {
                operator.compare_order(actual.compare(*expected))
            }
            (Self::Number(expected), FieldValue::String(actual)) => NumericValue::parse(actual)
                .is_some_and(|actual| operator.compare_order(actual.compare(*expected))),
...
impl NumericValue {
    fn parse(input: &str) -> Option<Self> {
        if !input.bytes().any(|byte| matches!(byte, b'.' | b'e' | b'E')) {
    ...
    fn compare(self, other: Self) -> std::cmp::Ordering {
        match (self, other) {
            ...
            (left, right) => left
                ...
                .expect("filter numbers are finite"),
        }
    }
    ...
    fn as_integer(self) -> Option<i128> {
        match self {
            ...
            Self::Float(value) if value.fract() == 0.0 => value.to_string().parse().ok(),
            Self::Float(_) => None,
        }
    }
    ...
    fn as_f64(self) -> f64 {
        match self {
            Self::Integer(value) => value
                .to_string()
                .parse()
                .expect("signed and unsigned 64-bit integers fit in finite f64"),
...
fn numeric_datafusion_literal(
    ...
    match data_type {
        DataType::Int8 => integer_datafusion_literal::<i8>(value, data_type),
        DataType::Int16 => integer_datafusion_literal::<i16>(value, data_type),
        DataType::Int32 => integer_datafusion_literal::<i32>(value, data_type),
        DataType::Int64 => integer_datafusion_literal::<i64>(value, data_type),
        DataType::UInt8 => integer_datafusion_literal::<u8>(value, data_type),
        DataType::UInt16 => integer_datafusion_literal::<u16>(value, data_type),
        DataType::UInt32 => integer_datafusion_literal::<u32>(value, data_type),
        DataType::UInt64 => integer_datafusion_literal::<u64>(value, data_type),
        DataType::Float32 => value
            ...
            .filter(|value| value.is_finite())
            .map(lit)
            .ok_or_else(|| numeric_literal_type_error(value, data_type)),
        DataType::Float64 => Ok(lit(value.as_f64())),
        DataType::Dictionary(_, value_type) => numeric_datafusion_literal(value, value_type),
        _ => Err(numeric_literal_type_error(value, data_type)),
    }
}
...
fn integer_datafusion_literal<T>(
    value: NumericValue,
    data_type: &arrow::datatypes::DataType,
) -> Result<std::sync::Arc<dyn datafusion::physical_expr::PhysicalExpr>, FilterCompileError>
where
    T: TryFrom<i128> + datafusion::logical_expr::Literal,
{
    let integer = value
        .as_integer()
        .ok_or_else(|| numeric_literal_type_error(value, data_type))?;
    let converted =
        T::try_from(integer).map_err(|_| numeric_literal_type_error(value, data_type))?;
    Ok(datafusion::physical_expr::expressions::lit(converted))
}
...
fn numeric_literal_type_error(
    value: NumericValue,
    data_type: &arrow::datatypes::DataType,
) -> FilterCompileError {
    FilterCompileError::UnsupportedSql(format!(
        "numeric literal {value} cannot be represented as {data_type}"
    ))
}
...
mod tests {
    ...
    #[test]
    fn raw_json_integer_filters_distinguish_adjacent_signed_and_unsigned_values() {
        let cases = [
            (
                "signed values above the f64 exact range",
                "id = 9007199254740993",
                br#"{"id":9007199254740993}"# as &'static [u8],
                br#"{"id":9007199254740992}"# as &'static [u8],
            ),
            (
                "unsigned values above the i64 range",
                "id = 18446744073709551615",
                br#"{"id":18446744073709551615}"#,
                br#"{"id":18446744073709551614}"#,
            ),
        ];

        for (name, sql, matching, adjacent) in cases {
            let filter = CompiledFilter::compile(sql).expect("integer filter compiles");

            assert2::assert!(
                filter.matches_structured_json(Some(&Bytes::from_static(matching))),
                "case {name} must match the exact integer"
            );
            assert2::assert!(
                !filter.matches_structured_json(Some(&Bytes::from_static(adjacent))),
                "case {name} must reject the adjacent integer"
            );
        }
    }
    ...
    fn typed_row_batch_seam_evaluates_arrow_like_enum_dictionary_values() {
        ...
        let batch = TypedRowBatch::from_rows(vec![
            BTreeMap::from([
                ...
                (
                    "priority".to_string(),
                    FieldValue::Number(NumericValue::Float(9.0)),
                ),
            ...
            BTreeMap::from([
                ...
                (
                    "priority".to_string(),
                    FieldValue::Number(NumericValue::Float(4.0)),
                ),
            ...
            BTreeMap::from([
                ...
                (
                    "priority".to_string(),
                    FieldValue::Number(NumericValue::Float(2.0)),
                ),
                ("deleted".to_string(), FieldValue::Bool(false)),
            ]),
        ]);
    ...
    #[test]
    #[cfg(feature = "arrow")]
    fn legacy_json_and_datafusion_paths_have_null_and_large_integer_parity() {
        use std::sync::Arc;
        ...
        .expect("parity batch builds");
        let json_rows = [
            Bytes::from_static(br#"{"id":9007199254740993,"deleted":null}"#),
            Bytes::from_static(br#"{"id":9007199254740992,"deleted":false}"#),
        ];
        let cases = [
            ("deleted = null", vec![true, false]),
            ("deleted != null", vec![false, true]),
            ("id = 9007199254740993", vec![true, false]),
        ];

        for (sql, expected) in cases {
            let filter = CompiledFilter::compile(sql).expect("legacy filter compiles");
            let arrow_mask = filter
                .evaluate_arrow_batch(&batch)
                .expect("legacy filter evaluates through DataFusion");

            assert2::assert!(arrow_mask == BooleanArray::from(expected.clone()));
            for (row, expected) in json_rows.iter().zip(expected) {
                assert2::assert!(
                    filter.matches_structured_json(Some(row)) == expected,
                    "raw JSON and DataFusion must agree for {sql}"
                );
            }
        }
    }
    ...
    fn datafusion_arrow_filter_reports_type_errors() {
        ...
        assert2::assert!(
            error
                == FilterCompileError::UnsupportedSql(
                    "numeric literal 0 cannot be represented as Boolean".to_string()
                )
        );
        assert!(!filter.matches_arrow_batch(&batch, 0));
    }
    ...
    #[test]
    fn quoted_literals_use_sql_apostrophe_doubling_and_preserve_backslashes() {
        assert!(filter_matches(
            "path = 'C:\\tmp\\O''Brien'",
            br#"{"path":"C:\\tmp\\O'Brien"}"#,
        ));
    }
}
