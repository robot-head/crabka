...
async fn init_producer_id_without_coordinator_bootstrap_returns_not_coordinator() {
    ...
    let r = p
        ...
        .await
        .expect("InitProducerId");
    assert_eq!(r.error_code, 16); // NOT_COORDINATOR
    p.broker.shutdown().await;
}
...
async fn init_producer_id_with_same_tid_bumps_epoch() {
    ...
    assert_eq!(r1.error_code, 0, "r1 error_code");
    ...
    assert_eq!(r2.error_code, 0, "r2 error_code");
    assert_eq!(r1.producer_id, r2.producer_id, "same pid for same tid");
    assert_eq!(
        r2.producer_epoch,
        r1.producer_epoch + 1,
