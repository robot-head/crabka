...
mod tests {
    ...
    fn http_statuses_map_to_taxonomy_when_connect_body_is_unusable() {
        let invalid = CrabkaError::from(crate::connect_client::ConnectClientError::HttpStatus {
            status: 400,
            message: "bad request".into(),
        });
        let unavailable =
            CrabkaError::from(crate::connect_client::ConnectClientError::HttpStatus {
                status: 503,
                message: "down".into(),
            });

        assert2::assert!(let CrabkaError::InvalidArgument(message) = invalid);
        assert_eq!(message, "bad request");
        assert2::assert!(let CrabkaError::Transport(message) = unavailable);
