...
pub(crate) struct MockState {
    ...
    pub(crate) queue_states: Vec<MockQueueMessageState>,
}
...
/// Per-group queue state for one mock message.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct MockQueueMessageState {
    pub(crate) group: String,
    pub(crate) delivery: MockQueueDelivery,
    pub(crate) disposition: MockQueueDisposition,
    pub(crate) delivery_count: i32,
}

/// Delivery disposition within one mock consumer group.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) enum MockQueueDisposition {
    Available,
    Acquired(String),
    Accepted,
    Rejected,
...
impl CrabkaClient {
    ...
    /// # Errors
    ///
    /// Returns [`CrabkaError::InvalidArgument`] when required client
...
impl CrabkaClientBuilder {
    ...
    /// # Errors
    ///
    /// Returns [`CrabkaError::InvalidArgument`] when no endpoint was supplied.
