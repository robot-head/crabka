...
fn environment_is_used_and_cli_wins_before_external_io() {
    ...
    assert!(
        String::from_utf8_lossy(&environment.stderr)
            .contains("--streams-join-retry-backoff-ms (37 ms) is only valid with --role stream")
    );

    let cli = demo()
        .args(["--role", "produce", "--streams-join-retry-backoff-ms", "41"])
        .env("CRABKA_DEMO_STREAMS_JOIN_RETRY_BACKOFF_MS", "37")
        .output()
        .expect("run demo");
    ...
    assert!(
        String::from_utf8_lossy(&cli.stderr)
            .contains("--streams-join-retry-backoff-ms (41 ms) is only valid with --role stream")
    );
}
