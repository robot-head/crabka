...
impl Encode for ApiVersionsResponse {
        ...
        if flex {
            ...
            if !(crate::codegen_helpers::is_default(&self.supported_features)) {
                let payload = encode_to_bytes(
                    {
                        let prefix = crate::primitives::array::array_len_prefix_len((self.supported_features).len(), flex);
                        let body: usize = (self.supported_features).iter().map(|it| it.encoded_len(version)).sum();
                        prefix + body
                    },
                    |b| {
                        {
                            crate::primitives::array::put_array_len(b, (self.supported_features).len(), flex);
            ...
            if !(crate::codegen_helpers::is_default(&self.finalized_features)) {
                let payload = encode_to_bytes(
                    {
                        let prefix = crate::primitives::array::array_len_prefix_len((self.finalized_features).len(), flex);
                        let body: usize = (self.finalized_features).iter().map(|it| it.encoded_len(version)).sum();
                        prefix + body
                    },
                    |b| {
                        {
                            crate::primitives::array::put_array_len(b, (self.finalized_features).len(), flex);
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.api_keys).len(), flex);
                let body: usize = (self.api_keys).iter().map(|it| it.encoded_len(version)).sum();
                prefix + body
            };
        }
        if version >= 1 {
...
impl ApiVersionsResponse {
    ...
    pub fn populated(version: i16) -> Self {
        ...
        if version >= 3 {
            m.supported_features = vec![crate::owned::api_versions_response::SupportedFeatureKey::populated(version)];
        }
        if version >= 3 {
            m.finalized_features_epoch = 1i64;
        }
        if version >= 3 {
            m.finalized_features = vec![crate::owned::api_versions_response::FinalizedFeatureKey::populated(version)];
        }
        if version >= 3 {
