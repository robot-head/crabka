                ...
                += if version >= 2 {
                    {
                        ...
                        let body: usize = opt
                            .map_or(
                                0,
                                |v| v.iter().map(|it| it.encoded_len(version)).sum(),
                            );
                        prefix + body
                    }
                } else {
    ...
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 && version <= 7 {
            out.topics = if version >= 2 {
                {
                    let opt = crate::primitives::array::get_nullable_array_len(
                        buf,
                        flex,
                    )?;
                    match opt {
                        ...
                        Some(n) => {
                            ...
                            for _ in 0..n {
                                v.push(
                                    OffsetFetchRequestTopic::decode_borrow(buf, version)?,
                                );
                            }
                            Some(v)
                        }
        ...
        if version >= 8 {
            n
                += {
                    ...
                    let body: usize = opt
                        .map_or(0, |v| v.iter().map(|it| it.encoded_len(version)).sum());
                    prefix + body
                };
        }
        if flex {
            ...
            out.topics = {
                ...
                match opt {
                    ...
                    Some(n) => {
                        ...
                        for _ in 0..n {
                            v.push(
                                OffsetFetchRequestTopics::decode_borrow(buf, version)?,
                            );
                        }
                        Some(v)
                    }
