...
impl<'de> DecodeBorrow<'de> for ApiVersionsRequest<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 3 { out.client_software_name = if flex { get_compact_string_borrowed(buf)? } else { get_string_borrowed(buf)? }; }
        if version >= 3 { out.client_software_version = if flex { get_compact_string_borrowed(buf)? } else { get_string_borrowed(buf)? }; }
        if flex {
