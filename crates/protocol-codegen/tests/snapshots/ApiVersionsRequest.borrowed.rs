...
impl<'a> Encode for ApiVersionsRequest<'a> {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        let mut n: usize = 0;
        if version >= 3 {
            n += if flex { compact_string_len(self.client_software_name) } else { string_len(self.client_software_name) };
        }
        if version >= 3 {
            n += if flex { compact_string_len(self.client_software_version) } else { string_len(self.client_software_version) };
        }
        if version >= 5 {
...
impl<'de> DecodeBorrow<'de> for ApiVersionsRequest<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 3 {
            out.client_software_name = if flex { get_compact_string_borrowed(buf)? } else { get_string_borrowed(buf)? };
        }
        if version >= 3 {
            out.client_software_version = if flex { get_compact_string_borrowed(buf)? } else { get_string_borrowed(buf)? };
        }
        if version >= 5 {
