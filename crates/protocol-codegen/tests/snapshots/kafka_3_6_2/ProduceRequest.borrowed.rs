...
impl<'a> Encode for ProduceRequest<'a> {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        let mut n: usize = 0;
        if version >= 3 {
            n += if flex { compact_nullable_string_len(self.transactional_id) } else { nullable_string_len(self.transactional_id) };
        }
        if version >= 0 {
            n += 2;
        }
...
impl<'de> DecodeBorrow<'de> for ProduceRequest<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 3 {
            out.transactional_id = if flex { get_compact_nullable_string_borrowed(buf)? } else { get_nullable_string_borrowed(buf)? };
        }
        if version >= 0 {
