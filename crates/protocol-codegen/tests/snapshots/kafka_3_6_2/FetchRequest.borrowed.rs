    ...
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        ...
        if flex {
            ...
            if !(crate::codegen_helpers::is_default(&self.replica_state)) {
                let payload = encode_to_bytes(
                    self.replica_state.encoded_len(version),
                    |b| {
...
impl<'de> DecodeBorrow<'de> for FetchRequest<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if flex {
            ...
            out.unknown_tagged_fields = read_tagged_fields(
                buf,
                |tag, payload| {
                    match tag {
                        0 => {
                            tag_cluster_id = Some({
                                ...
                                if flex {
                                    crate::primitives::string_bytes::get_compact_nullable_string_owned(
                                        b,
                                    )?
                                } else {
                                    crate::primitives::string_bytes::get_nullable_string_owned(
                                        b,
                                    )?
                                }
                            });
