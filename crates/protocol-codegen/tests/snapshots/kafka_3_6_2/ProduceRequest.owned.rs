...
use crate::primitives::string_bytes::{
    get_compact_nullable_bytes_owned, get_nullable_bytes_owned, put_bytes,
    put_compact_bytes, put_compact_nullable_bytes, put_nullable_bytes,
};
    ...
    fn decode<B: Buf>(buf: &mut B, version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 {
            out.topic_data = {
                ...
                for _ in 0..n {
                    v.push(TopicProduceData::decode(buf, version)?);
                }
                v
            };
        }
    ...
    fn decode<B: Buf>(buf: &mut B, version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 {
            out.partition_data = {
                ...
                let mut v = Vec::with_capacity(n);
                for _ in 0..n {
                    v.push(PartitionProduceData::decode(buf, version)?);
                }
                v
            };
        }
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n
                += match &self.records {
                    None => {
                        if flex { crate::primitives::varint::uvarint_len(0) } else { 4 }
                    }
                    Some(__rb) => {
            ...
            out.records = {
                ...
                match __rb_opt {
                    ...
                    Some(__rb_bytes) => {
                        ...
                        Some(
                            <crate::records::RecordsPayload as crate::Decode>::decode(
                                &mut __rb_cur,
                                version,
                            )?,
                        )
