                        ...
                        0 => {
                            tag_node_endpoints = Some({
                                let b: &mut &[u8] = payload;
                                {
                                    ...
                                    for _ in 0..n {
                                        v.push(
                                            crate::owned::produce_response::NodeEndpoint::decode(
                                                b,
                                                version,
                                            )?,
                                        );
...
impl<'a> ProduceResponse<'a> {
    ...
    pub fn populated(version: i16) -> Self {
        ...
        if version >= 10 {
            m.node_endpoints = vec![
                crate ::owned::produce_response::NodeEndpoint::populated(version)
            ];
        }
        m
    }
    ...
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        ...
        if flex {
            ...
            if !(crate::codegen_helpers::is_default(&self.current_leader)) {
                let payload = encode_to_bytes(
                    self.current_leader.encoded_len(version),
                    |b| {
