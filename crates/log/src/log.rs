...
impl Log {
    ...
    pub fn read(&self, offset: Offset, max_size: ByteSize) -> Result<ReadOutput, LogError> {
        ...
        {
            let bs = active.read(current_offset, remaining.max(bytes(1)))?;
            batches.extend(bs);
        }
    ...
    pub fn read_raw(
        ...
        for seg in &self.segments {
            ...
            let r: RawSegmentRead =
                seg.read_raw(current, limit_offset, remaining.max(batch_header()))?;
            if !r.is_empty() {
                if !got_first {
        ...
        {
            let r = active.read_raw(current, limit_offset, remaining.max(batch_header()))?;
            if !r.is_empty() {
                if !got_first {
                    start_offset = r.start_offset;
                }
