...
fn record_variant(rec: &MetadataRecord) -> &'static str {
    match rec {
        ...
        MetadataRecord::V1ProducerIds(_) => "V1ProducerIds",
...
impl MetadataImage {
    ...
    /// Dynamic KIP-1071 overrides for one GROUP config resource.
    #[must_use]
    pub fn group_config(&self, group_id: &str) -> Option<&BTreeMap<String, String>> {
        self.group_configs.get(group_id)
    }

    /// All configured GROUP resources.
    pub fn group_configs(&self) -> impl Iterator<Item = (&String, &BTreeMap<String, String>)> {
        self.group_configs.iter()
    }
    ...
    /// First producer ID not covered by any committed allocation block.
    #[must_use]
    pub fn next_producer_id(&self) -> i64 {
        self.producer_ids.as_ref().map_or(0, |r| r.next_producer_id)
    }
    ...
    pub fn apply(&mut self, rec: &MetadataRecord) {
        match rec {
            ...
            MetadataRecord::V1ProducerIds(rec) => {
                if rec.next_producer_id > self.next_producer_id() {
                    self.producer_ids = Some(*rec);
                } else {
                    tracing::error!(
                        current = self.next_producer_id(),
                        proposed = rec.next_producer_id,
                        "ignoring non-monotonic producer ID allocation record"
                    );
                }
            }
            ...
            MetadataRecord::V1GroupConfig(c) => {
                if c.configs.is_empty() {
                    self.group_configs.remove(&c.group_id);
                } else {
                    self.group_configs
                        .insert(c.group_id.clone(), c.configs.clone());
                }
            }
    ...
    pub fn to_records(&self) -> Vec<MetadataRecord> {
        ...
        if let Some(record) = self.producer_ids {
            out.push(MetadataRecord::V1ProducerIds(record));
        }
        ...
        // KIP-1071 dynamic GROUP resources.
        for (group_id, configs) in &self.group_configs {
            out.push(MetadataRecord::V1GroupConfig(GroupConfigRecord {
    ...
    pub fn validate(&self, rec: &MetadataRecord) -> Result<(), MetadataError> {
        match rec {
            ...
            // KIP-1071: GROUP config keys and values are validated by the
            // IncrementalAlterConfigs handler before this whole-map upsert.
            | MetadataRecord::V1GroupConfig(_)
            ...
            MetadataRecord::V1ProducerIds(record) => {
                if record.next_producer_id <= self.next_producer_id() {
                    return Err(MetadataError::InvalidRecord(
                        "next producer ID must advance monotonically",
                    ));
                }
                if self.broker_epoch(record.broker_id) != Some(record.broker_epoch) {
                    return Err(MetadataError::InvalidRecord(
                        "producer ID allocation has a stale broker epoch",
                    ));
                }
                Ok(())
            }
...
mod tests {
    ...
    fn record_variant_returns_exact_discriminant_names() {
        ...
        assert2::assert!(record_variant(&topic) == "V1Topic");

        let feature = MetadataRecord::V1FeatureLevel(crate::records::FeatureLevelRecord {
            name: "metadata.version".into(),
            level: 1,
        });
        assert2::assert!(record_variant(&feature) == "V1FeatureLevel");
    }
    ...
    fn fresh_image_has_no_features_and_unknown_epoch() {
        let m = img();
        assert2::assert!(m.finalized_features().is_empty());
        assert2::assert!(m.finalized_features_epoch() == -1);
    }

    #[test]
    fn finalized_feature_reads_arbitrary_name() {
        let mut m = MetadataImage::new(uuid::Uuid::nil());
        assert2::assert!(m.finalized_feature("transaction.version").is_none());
        m.apply(&MetadataRecord::V1FeatureLevel(FeatureLevelRecord {
            name: "transaction.version".into(),
            level: 2,
        }));
        assert2::assert!(m.finalized_feature("transaction.version") == Some(2));
    }
    ...
    fn apply_feature_level_sets_level_and_bumps_epoch() {
        ...
        assert2::assert!(m.finalized_features().get("metadata.version") == Some(&1));
        assert2::assert!(m.finalized_features_epoch() == 0);
    ...
    fn offset_advance_applies_as_monotonic_delta() {
        ...
        image.apply(&advance(3));
        image.apply(&advance(2));
        ...
        assert2::assert!(image.partition_next_offset("t", 0) == Some(5));
        assert2::assert!(image.partition_next_offset("t", 1) == Some(7));
    }
    ...
    fn offset_advance_survives_snapshot_round_trip() {
        ...
        assert2::assert!(restored.partition_next_offset("t", 0) == Some(5));
    }
    ...
    fn to_records_round_trips_all_variants() {
        ...
        assert2::assert!(image.finalized_features_epoch() == 2);
    ...
    fn apply_topic_inserts() {
        let mut m = img();
        m.apply(&topic("t", 3));
        assert2::assert!(m.topic("t").is_some());
    }
    ...
    fn apply_delete_clears_partitions() {
        ...
        assert2::assert!(m.partitions_of("t").count() == 2);
        m.apply(&MetadataRecord::V1DeleteTopic(DeleteTopicRecord {
            name: "t".into(),
        }));
        assert2::assert!(m.topic("t").is_none());
        assert2::assert!(m.partitions_of("t").count() == 0);
    }
    ...
    fn validate_topic_partition_count_increase_allowed() {
        ...
        assert2::assert!(m.validate(&updated).is_ok());
    }
    ...
    fn validate_delete_unknown_topic_rejected() {
        ...
        let err = m
            ...
            .unwrap_err();
        assert2::assert!(matches!(err, MetadataError::UnknownTopic(_)));
    }
    ...
    fn broker_registration_is_idempotent() {
        ...
        m.apply(&b);
        m.apply(&b);
        assert2::assert!(m.brokers().count() == 1);
    }
    ...
    fn apply_topic_config_inserts() {
        ...
        assert2::assert!(m.topic_config("t") == Some(&overrides));
    }
    ...
    fn apply_topic_config_replaces_previous() {
        ...
        assert2::assert!(m.topic_config("t") == Some(&second));
    }
    ...
    fn delete_topic_clears_configs() {
        ...
        assert2::assert!(m.topic_config("t").is_none());
    }
    ...
    fn apply_scram_credential_last_write_wins() {
        ...
        assert2::assert!((got.iterations, &got.salt) == (8192, &vec![9; 16]));
    }
    ...
    fn delete_scram_credential_removes() {
        ...
        assert2::assert!(m.scram_credential("alice", mech).is_none());
    }
    ...
    fn matching_acls_combines_literal_and_prefixed() {
        ...
        assert2::assert!((hits_foo.len(), hits_team.len()) == (1, 1));
    }
    ...
    fn matching_acls_literal_wildcard_deny_matches_arbitrary_topic() {
        ...
        let hits: Vec<_> = m
            ...
            .collect();
        assert2::assert!(hits == vec![&topic_wildcard_deny()]);
    }
    ...
    fn apply_v1_delete_access_control_entry_removes_matching() {
        ...
        let hits_foo: Vec<_> = m.matching_acls(ResourceType::Topic, "foo").collect();
        let hits_team: Vec<_> = m.matching_acls(ResourceType::Topic, "team-x").collect();
        assert2::assert!((hits_foo.len(), hits_team.len()) == (0, 1));
    }
    ...
    fn all_acls_returns_every_entry() {
        ...
        m.apply(&MetadataRecord::V1AccessControlEntry(topic_read_for_alice()));
        m.apply(&MetadataRecord::V1AccessControlEntry(topic_prefixed_team()));
        assert2::assert!(m.all_acls().count() == 2);
    }
    ...
    fn all_partitions_count_matches_map_size() {
        ...
        assert2::assert!(m.all_partitions().count() == 4);
        assert2::assert!(
    ...
    fn reassignments_in_flight_excludes_idle_partitions() {
        ...
        assert2::assert!(img.reassignments_in_flight().count() == 0);
    }
    ...
    fn reassignments_in_flight_covers_multiple_topics() {
        ...
        assert2::assert!(img.reassignments_in_flight().count() == 2);
    }
    ...
    fn broker_config_set_inserts_into_image() {
        ...
        assert2::assert!(bc.get("leader.replication.throttled.rate") == Some(&"2048".to_string()));
    }
    ...
    fn broker_config_delete_removes_from_image() {
        ...
        assert2::assert!(bc.get("leader.replication.throttled.rate").is_none());
    }
    ...
    fn broker_throttle_rate_parses_positive_value() {
        ...
        assert2::assert!(
            img.broker_throttle_rate(NodeId(1), ThrottleKind::AlterLogDirs)
                == Some(bytes_per_sec(4096))
        );
    }
    ...
    fn client_quota_apply_inserts_canonicalized() {
        ...
        assert2::assert!(configs.get("producer_byte_rate") == Some(&1024.0));
    }
    ...
    fn client_quota_apply_delete_removes_key() {
        ...
        assert2::assert!(configs.get("producer_byte_rate").is_none());
    }
    ...
    fn client_quota_default_entity_uses_none_name() {
        ...
        assert2::assert!(img.client_quotas().contains_key(&key));
    }
    ...
    fn scram_credentials_users_returns_distinct_users() {
        ...
        users.sort();
        assert2::assert!(users == vec!["alice".to_string(), "bob".to_string()]);
    }
    ...
    fn apply_delegation_token_insert_and_replace() {
        ...
        assert2::assert!(got.expiry_timestamp_ms == 5_000);
        assert2::assert!(&got.owner == &alice);

        // Same token_id, different expiry — replace, not duplicate.
        img.apply(&dt_record("tok-1", alice.clone(), 7_500, vec![]));
        let got = img.delegation_token_by_id("tok-1").expect("token present");
        assert2::assert!(got.expiry_timestamp_ms == 7_500);
        assert2::assert!(img.all_delegation_tokens().count() == 1);
    }
    ...
    fn apply_delete_delegation_token_removes_from_image() {
        ...
        assert2::assert!(img.delegation_token_by_id("tok-1").is_some());
        ...
        assert2::assert!(img.delegation_token_by_id("tok-1").is_none());
        assert2::assert!(img.all_delegation_tokens().count() == 0);
    }
    ...
    fn delegation_tokens_by_owner_filters_correctly() {
        ...
        assert2::assert!(alice_tokens.len() == 2);
        assert2::assert!(alice_tokens.iter().all(|token| token.owner == alice));
        ...
        assert2::assert!(bob_visible.len() == 2);
    ...
    fn applies_voters_and_version() {
        ...
        assert2::assert!(image.kraft_version() == 1);
        assert2::assert!(image.voters().contains(NodeId(1)));
    }
    ...
    fn scram_credentials_for_user_returns_pairs() {
        ...
        assert2::assert!(pairs == vec![(SaslMechanism::ScramSha512, 8192)]);
        assert2::assert!(img.scram_credentials_for_user("ghost").is_empty());
    }
    ...
    fn finalized_metadata_version_reads_feature_map() {
        ...
        let mut m = img();
        assert2::assert!(m.finalized_metadata_version() == None);
        m.apply(&MetadataRecord::V1FeatureLevel(FeatureLevelRecord {
            name: "metadata.version".into(),
            level: 19,
        }));
        assert2::assert!(m.finalized_metadata_version() == Some(19));
    }
    ...
    fn min_required_metadata_version_baseline_is_min() {
        ...
        let m = img();
        assert2::assert!(m.min_required_metadata_version() == METADATA_VERSION_MIN);
    }
    ...
    fn client_metrics_config_apply_and_clear() {
        ...
        assert2::assert!(img.client_metrics_subscriptions().count() == 1);
        img.apply(&MetadataRecord::V1ClientMetricsConfig(
            ClientMetricsConfigRecord {
                name: "sub-a".into(),
        ...
        assert2::assert!(img.client_metrics_config("sub-a").is_none());
        assert2::assert!(img.client_metrics_subscriptions().count() == 0);
    }

    #[test]
    fn group_config_apply_snapshot_and_clear() {
        let mut image = MetadataImage::new(Uuid::nil());
        let record = MetadataRecord::V1GroupConfig(GroupConfigRecord {
        ...
        assert2::assert!(
            image
                .group_config("streams-app")
                .and_then(|configs| configs.get("streams.num.standby.replicas"))
                .map(String::as_str)
                == Some("1")
        );
        assert2::assert!(image.to_records().iter().any(|item| item == &record));

        image.apply(&MetadataRecord::V1GroupConfig(GroupConfigRecord {
            group_id: "streams-app".into(),
            configs: BTreeMap::new(),
        }));
        assert2::assert!(image.group_config("streams-app").is_none());
    }

    #[test]
    fn producer_id_boundary_is_monotonic_and_survives_snapshot() {
        let mut image = MetadataImage::new(Uuid::nil());
        image.apply(&MetadataRecord::V1ProducerIds(ProducerIdsRecord {
            broker_id: NodeId(1),
            broker_epoch: 4,
        ...
        assert2::assert!(image.next_producer_id() == 2_000);
    ...
    fn min_required_metadata_version_rises_with_scram_and_tokens() {
        ...
        assert2::assert!(m.min_required_metadata_version() == SCRAM_MIN_LEVEL);
        ...
        assert2::assert!(m.min_required_metadata_version() == DELEGATION_TOKEN_MIN_LEVEL);
    }
    ...
    fn topic_by_id_resolves_and_drops_on_delete() {
        ...
        assert2::assert!(img.topic_by_id(&id).map(|t| t.name.as_str()) == Some("orders"));
        assert2::assert!(img.topic_name_by_id(&id) == Some("orders"));
        ...
        assert2::assert!(img.topic_by_id(&id).is_none());
        assert2::assert!(img.topic_name_by_id(&id).is_none());
    }
    ...
    fn broker_epoch_reads_back_registered_epoch() {
        ...
        assert2::assert!(image.broker_epoch(NodeId(404)) == None);
    }
    ...
    fn fresh_image_accessors() {
        ...
        assert2::assert!(image.cluster_id() == id);
        assert2::assert!(image.kraft_version() == 0);
    }
    ...
    fn broker_lookup_returns_registered_record() {
        ...
        assert2::assert!((b.node_id, image.broker(NodeId(404)).is_none()) == (NodeId(7), true));
    }
    ...
    fn reapplying_topic_with_new_id_drops_stale_id_index() {
        ...
        assert2::assert!(image.topic_by_id(&old_id).is_some());
        ...
        assert2::assert!(image.topic_by_id(&new_id).is_some());
        assert2::assert!(image.topic_by_id(&old_id).is_none());
    }
