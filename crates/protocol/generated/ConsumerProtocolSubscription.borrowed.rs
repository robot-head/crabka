...
impl ConsumerProtocolSubscription<'_> {
    pub fn to_owned(&self) -> crate::owned::consumer_protocol_subscription::ConsumerProtocolSubscription {
        crate::owned::consumer_protocol_subscription::ConsumerProtocolSubscription {
            topics: (self.topics).iter().map(std::string::ToString::to_string).collect(),
            user_data: (self.user_data).map(Bytes::copy_from_slice),
            owned_partitions: (self.owned_partitions).iter().map(TopicPartition::to_owned).collect(),
...
impl Encode for ConsumerProtocolSubscription<'_> {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        ...
        if version >= 0 {
            if flex { put_compact_nullable_bytes(buf, self.user_data) } else { put_nullable_bytes(buf, self.user_data) }
        }
        if version >= 1 {
            {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += {
                ...
                let body: usize = (self.topics).iter().map(|it| if flex { compact_string_len(it) } else { string_len(it) }).sum();
                prefix + body
            };
        }
        if version >= 0 {
        ...
        if version >= 1 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.owned_partitions).len(), flex);
                let body: usize = (self.owned_partitions).iter().map(|it| it.encoded_len(version)).sum();
                prefix + body
            };
        }
        if version >= 2 {
...
impl<'de> DecodeBorrow<'de> for ConsumerProtocolSubscription<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 {
            out.user_data = if flex { get_compact_nullable_bytes_borrowed(buf)? } else { get_nullable_bytes_borrowed(buf)? };
        }
        if version >= 1 {
