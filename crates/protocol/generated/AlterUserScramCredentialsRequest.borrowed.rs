...
impl AlterUserScramCredentialsRequest<'_> {
    ...
    pub fn to_owned(&self) -> crate::owned::alter_user_scram_credentials_request::AlterUserScramCredentialsRequest {
        crate::owned::alter_user_scram_credentials_request::AlterUserScramCredentialsRequest {
            deletions: (self.deletions).iter().map(ScramCredentialDeletion::to_owned).collect(),
            upsertions: (self.upsertions).iter().map(ScramCredentialUpsertion::to_owned).collect(),
...
impl Encode for AlterUserScramCredentialsRequest<'_> {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.deletions).len(), flex);
                let body: usize = (self.deletions).iter().map(|it| it.encoded_len(version)).sum();
                prefix + body
            };
        }
        if version >= 0 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.upsertions).len(), flex);
                let body: usize = (self.upsertions).iter().map(|it| it.encoded_len(version)).sum();
                prefix + body
            };
        }
        if flex {
...
impl Encode for ScramCredentialUpsertion<'_> {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        ...
        if version >= 0 {
            if flex { put_compact_bytes(buf, self.salt) } else { put_bytes(buf, self.salt) }
        }
        if version >= 0 {
            if flex { put_compact_bytes(buf, self.salted_password) } else { put_bytes(buf, self.salted_password) }
        }
        if flex {
            let tagged = WriteTaggedFields::new();
            tagged.write(buf, &self.unknown_tagged_fields);
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += if flex {
                crate::primitives::varint::uvarint_len(u32::try_from((self.salt).len() + 1).unwrap()) + (self.salt).len()
            } else {
                4 + (self.salt).len()
            };
        }
        if version >= 0 {
            n += if flex {
                crate::primitives::varint::uvarint_len(u32::try_from((self.salted_password).len() + 1).unwrap()) + (self.salted_password).len()
            } else {
...
impl<'de> DecodeBorrow<'de> for ScramCredentialUpsertion<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 {
            out.salt = if flex { get_compact_bytes_borrowed(buf)? } else { get_bytes_borrowed(buf)? };
        }
        if version >= 0 {
            out.salted_password = if flex { get_compact_bytes_borrowed(buf)? } else { get_bytes_borrowed(buf)? };
        }
        if flex {
            out.unknown_tagged_fields = read_tagged_fields(buf, |_tag, _payload| Ok(false))?;
        }
