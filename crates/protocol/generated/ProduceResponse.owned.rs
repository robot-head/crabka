...
impl ProduceResponse {
    ...
    pub fn populated(version: i16) -> Self {
        ...
        if version >= 10 { m.node_endpoints = vec![NodeEndpoint::populated(version)]; }
        m
    }
}
...
impl TopicProduceResponse {
    ...
    pub fn populated(version: i16) -> Self {
        let mut m = Self::default();
        if version >= 0 && version <= 12 { m.name = "x".to_string(); }
...
impl Encode for NodeEndpoint {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        ...
        if version >= 10 { if flex { put_compact_nullable_string(buf, self.rack.as_deref()) } else { put_nullable_string(buf, self.rack.as_deref()) } }
        if flex {
            let tagged = WriteTaggedFields::new();
            tagged.write(buf, &self.unknown_tagged_fields);
        }
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 10 { n += 4; }
        if version >= 10 { n += if flex { compact_nullable_string_len(self.rack.as_deref()) } else { nullable_string_len(self.rack.as_deref()) }; }
        if flex {
            let known_pairs: Vec<(u32, usize)> = Vec::new();
            n += tagged_fields_len(&known_pairs, &self.unknown_tagged_fields);
        }
...
impl<'de> Decode<'de> for NodeEndpoint {
    fn decode<B: Buf>(buf: &mut B, version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 10 { out.host = if flex { get_compact_string_owned(buf)? } else { get_string_owned(buf)? }; }
        if version >= 10 { out.port = get_i32(buf)?; }
        if version >= 10 { out.rack = if flex { get_compact_nullable_string_owned(buf)? } else { get_nullable_string_owned(buf)? }; }
        if flex {
            out.unknown_tagged_fields = read_tagged_fields(buf, |_tag, _payload| {
                Ok(false)
            })?;
...
impl NodeEndpoint {
    ...
    pub fn populated(version: i16) -> Self {
        ...
        if version >= 10 { m.host = "x".to_string(); }
        if version >= 10 { m.port = 1i32; }
        if version >= 10 { m.rack = Some("x".to_string()); }
        m
    }
}
