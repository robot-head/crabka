...
impl Encode for DescribeQuorumResponse<'_> {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 2 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.nodes).len(), flex);
...
impl<'de> DecodeBorrow<'de> for TopicData<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        let mut out = Self::default();
        if version >= 0 {
            out.topic_name = if flex { get_compact_string_borrowed(buf)? } else { get_string_borrowed(buf)? };
        }
        if version >= 0 {
            out.partitions = {
                let n = crate::primitives::array::get_array_len(buf, flex)?;
...
impl<'a> PartitionData<'a> {
    ...
    pub fn to_owned(&self) -> crate::owned::describe_quorum_response::PartitionData {
        crate::owned::describe_quorum_response::PartitionData {
            ...
            current_voters: (self.current_voters).iter().map(super::common::describe_quorum_response::replica_state::ReplicaState::to_owned).collect(),
            observers: (self.observers).iter().map(super::common::describe_quorum_response::replica_state::ReplicaState::to_owned).collect(),
            unknown_tagged_fields: self.unknown_tagged_fields.clone(),
        }
    }
...
impl Encode for PartitionData<'_> {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.current_voters).len(), flex);
                let body: usize = (self.current_voters).iter().map(|it| it.encoded_len(version)).sum();
                prefix + body
            };
        }
        if version >= 0 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.observers).len(), flex);
                let body: usize = (self.observers).iter().map(|it| it.encoded_len(version)).sum();
                prefix + body
            };
        }
        if flex {
...
impl PartitionData<'_> {
    ...
    pub fn populated(version: i16) -> Self {
        ...
        if version >= 0 {
            m.current_voters = vec![super::common::describe_quorum_response::replica_state::ReplicaState::populated(version)];
        }
        if version >= 0 {
            m.observers = vec![super::common::describe_quorum_response::replica_state::ReplicaState::populated(version)];
        }
        m
    }
}
