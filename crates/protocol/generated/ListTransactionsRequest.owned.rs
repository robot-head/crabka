...
impl Encode for ListTransactionsRequest {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 2 {
            n += if flex {
                compact_nullable_string_len(self.transactional_id_pattern.as_deref())
            } else {
                nullable_string_len(self.transactional_id_pattern.as_deref())
            };
        }
        if flex {
...
impl ListTransactionsRequest {
    ...
    pub fn populated(version: i16) -> Self {
        ...
        if version >= 0 {
            m.state_filters = vec!["x".to_string()];
        }
        if version >= 0 {
        ...
        if version >= 2 {
            m.transactional_id_pattern = Some("x".to_string());
        }
        m
    }
}
