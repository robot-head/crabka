...
impl<'a> ElectLeadersResponse<'a> {
    pub fn to_owned(&self) -> crate::owned::elect_leaders_response::ElectLeadersResponse {
        crate::owned::elect_leaders_response::ElectLeadersResponse {
            ...
            replica_election_results: (self.replica_election_results).iter().map(|it| it.to_owned()).collect(),
...
impl<'de> DecodeBorrow<'de> for ElectLeadersResponse<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 { out.replica_election_results = { let n = crate::primitives::array::get_array_len(buf, flex)?; let mut v = Vec::with_capacity(n); for _ in 0..n { v.push(ReplicaElectionResult::decode_borrow(buf, version)?); } v }; }
        if flex {
...
impl<'a> ReplicaElectionResult<'a> {
    pub fn to_owned(&self) -> crate::owned::elect_leaders_response::ReplicaElectionResult {
        crate::owned::elect_leaders_response::ReplicaElectionResult {
            ...
            partition_result: (self.partition_result).iter().map(|it| it.to_owned()).collect(),
            unknown_tagged_fields: self.unknown_tagged_fields.clone(),
        }
    }
}
...
impl<'de> DecodeBorrow<'de> for ReplicaElectionResult<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 { out.partition_result = { let n = crate::primitives::array::get_array_len(buf, flex)?; let mut v = Vec::with_capacity(n); for _ in 0..n { v.push(PartitionResult::decode_borrow(buf, version)?); } v }; }
        if flex {
            out.unknown_tagged_fields = read_tagged_fields(buf, |_tag, _payload| {
                Ok(false)
            })?;
