...
impl SyncGroupRequest<'_> {
    ...
    pub fn to_owned(&self) -> crate::owned::sync_group_request::SyncGroupRequest {
        crate::owned::sync_group_request::SyncGroupRequest {
            ...
            assignments: (self.assignments).iter().map(SyncGroupRequestAssignment::to_owned).collect(),
...
impl Encode for SyncGroupRequest<'_> {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 5 {
            n += if flex { compact_nullable_string_len(self.protocol_name) } else { nullable_string_len(self.protocol_name) };
        }
        if version >= 0 {
            n += {
...
impl<'de> DecodeBorrow<'de> for SyncGroupRequest<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 5 {
            out.protocol_name = if flex { get_compact_nullable_string_borrowed(buf)? } else { get_nullable_string_borrowed(buf)? };
        }
        if version >= 0 {
            out.assignments = {
...
impl Encode for SyncGroupRequestAssignment<'_> {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        ...
        if version >= 0 {
            if flex { put_compact_bytes(buf, self.assignment) } else { put_bytes(buf, self.assignment) }
        }
        if flex {
            let tagged = WriteTaggedFields::new();
            tagged.write(buf, &self.unknown_tagged_fields);
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += if flex {
                crate::primitives::varint::uvarint_len(u32::try_from((self.assignment).len() + 1).unwrap()) + (self.assignment).len()
            } else {
...
impl<'de> DecodeBorrow<'de> for SyncGroupRequestAssignment<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 {
            out.assignment = if flex { get_compact_bytes_borrowed(buf)? } else { get_bytes_borrowed(buf)? };
        }
        if flex {
            out.unknown_tagged_fields = read_tagged_fields(buf, |_tag, _payload| Ok(false))?;
        }
