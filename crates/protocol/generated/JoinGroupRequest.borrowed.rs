...
impl<'a> JoinGroupRequest<'a> {
    ...
    pub fn to_owned(&self) -> crate::owned::join_group_request::JoinGroupRequest {
        crate::owned::join_group_request::JoinGroupRequest {
            ...
            protocols: (self.protocols).iter().map(JoinGroupRequestProtocol::to_owned).collect(),
    ...
    fn encode_field_7<B: BufMut>(&self, buf: &mut B, version: i16, flex: bool) {
        if version >= 8 {
            if flex { put_compact_nullable_string(buf, self.reason) } else { put_nullable_string(buf, self.reason) }
        }
    }
    ...
    fn decode_field_6(out: &mut Self, buf: &mut &'a [u8], version: i16, flex: bool) -> Result<(), ProtocolError> {
        if version >= 0 {
            out.protocols = {
                ...
                for _ in 0..n {
                    v.push(JoinGroupRequestProtocol::decode_borrow(buf, version)?);
                }
                v
            };
        }
    ...
    fn decode_field_7(out: &mut Self, buf: &mut &'a [u8], version: i16, flex: bool) -> Result<(), ProtocolError> {
        if version >= 8 {
            out.reason = if flex { get_compact_nullable_string_borrowed(buf)? } else { get_nullable_string_borrowed(buf)? };
        }
        Ok(())
    }
...
impl Encode for JoinGroupRequest<'_> {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.protocols).len(), flex);
                let body: usize = (self.protocols).iter().map(|it| it.encoded_len(version)).sum();
                prefix + body
            };
        }
        if version >= 8 {
            n += if flex { compact_nullable_string_len(self.reason) } else { nullable_string_len(self.reason) };
        }
        if flex {
