...
impl ShareFetchRequest {
    ...
    fn decode_field_0<B: Buf>(out: &mut Self, buf: &mut B, version: i16, flex: bool) -> Result<(), ProtocolError> {
        if version >= 0 {
            out.group_id = if flex { get_compact_nullable_string_owned(buf)? } else { get_nullable_string_owned(buf)? };
        }
        Ok(())
    }
...
impl Encode for ShareFetchRequest {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        let mut n: usize = 0;
        if version >= 0 {
            n += if flex { compact_nullable_string_len(self.group_id.as_deref()) } else { nullable_string_len(self.group_id.as_deref()) };
        }
        if version >= 0 {
            n += if flex {
