...
use crate::primitives::fixed::{get_i8, get_i16, get_i32, get_i64, put_i8, put_i16, put_i32, put_i64};
    ...
    fn decode<B: Buf>(buf: &mut B, version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 {
            out.topics = {
                ...
                for _ in 0..n {
                    v.push(WriteStateData::decode(buf, version)?);
                }
                v
            };
        }
...
impl Encode for PartitionData {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.state_batches).len(), flex);
                let body: usize = (self.state_batches).iter().map(|it| it.encoded_len(version)).sum();
                prefix + body
            };
        }
        if flex {
