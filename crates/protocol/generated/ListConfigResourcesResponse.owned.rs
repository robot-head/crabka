...
impl Encode for ListConfigResourcesResponse {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.config_resources).len(), flex);
                let body: usize = (self.config_resources).iter().map(|it| it.encoded_len(version)).sum();
                prefix + body
            };
        }
        if flex {
