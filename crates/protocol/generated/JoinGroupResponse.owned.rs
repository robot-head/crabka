...
impl JoinGroupResponse {
    ...
    fn decode_field_3<B: Buf>(out: &mut Self, buf: &mut B, version: i16, flex: bool) -> Result<(), ProtocolError> {
        if version >= 7 {
            out.protocol_type = if flex { get_compact_nullable_string_owned(buf)? } else { get_nullable_string_owned(buf)? };
        }
        Ok(())
    }
    ...
    fn decode_field_5<B: Buf>(out: &mut Self, buf: &mut B, version: i16, flex: bool) -> Result<(), ProtocolError> {
        if version >= 0 {
            out.leader = if flex { get_compact_string_owned(buf)? } else { get_string_owned(buf)? };
        }
        Ok(())
    }
...
impl Encode for JoinGroupResponseMember {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += if flex { compact_bytes_len(&self.metadata) } else { bytes_len(&self.metadata) };
        }
        if flex {
            let known_pairs: Vec<(u32, usize)> = Vec::new();
            n += tagged_fields_len(&known_pairs, &self.unknown_tagged_fields);
...
impl Decode<'_> for JoinGroupResponseMember {
    fn decode<B: Buf>(buf: &mut B, version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 {
            out.metadata = if flex { get_compact_bytes_owned(buf)? } else { get_bytes_owned(buf)? };
        }
        if flex {
            out.unknown_tagged_fields = read_tagged_fields(buf, |_tag, _payload| Ok(false))?;
        }
