...
impl BrokerHeartbeatRequest {
    #[must_use]
    pub fn to_owned(&self) -> crate::owned::broker_heartbeat_request::BrokerHeartbeatRequest {
        crate::owned::broker_heartbeat_request::BrokerHeartbeatRequest {
            broker_id: (self.broker_id),
            broker_epoch: (self.broker_epoch),
            current_metadata_offset: (self.current_metadata_offset),
            want_fence: (self.want_fence),
            want_shut_down: (self.want_shut_down),
            offline_log_dirs: (self.offline_log_dirs).clone(),
        ...
        if flex {
            ...
            if !(crate::codegen_helpers::is_default(&self.offline_log_dirs)) {
                let payload = encode_to_bytes(
                    {
                        let prefix = crate::primitives::array::array_len_prefix_len((self.offline_log_dirs).len(), flex);
                        let body: usize = (self.offline_log_dirs).iter().map(|_| 16).sum();
                        prefix + body
                    },
                    |b| {
                        {
                            crate::primitives::array::put_array_len(b, (self.offline_log_dirs).len(), flex);
