...
impl OffsetFetchRequest<'_> {
    ...
    pub fn to_owned(&self) -> crate::owned::offset_fetch_request::OffsetFetchRequest {
        crate::owned::offset_fetch_request::OffsetFetchRequest {
            ...
            topics: (self.topics).as_ref().map(|v| v.iter().map(OffsetFetchRequestTopic::to_owned).collect()),
            groups: (self.groups).iter().map(OffsetFetchRequestGroup::to_owned).collect(),
...
impl OffsetFetchRequestGroup<'_> {
    ...
    pub fn to_owned(&self) -> crate::owned::offset_fetch_request::OffsetFetchRequestGroup {
        crate::owned::offset_fetch_request::OffsetFetchRequestGroup {
            ...
            topics: (self.topics).as_ref().map(|v| v.iter().map(OffsetFetchRequestTopics::to_owned).collect()),
            unknown_tagged_fields: self.unknown_tagged_fields.clone(),
        }
    }
}
impl Encode for OffsetFetchRequestGroup<'_> {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        ...
        if version >= 9 {
            if flex { put_compact_nullable_string(buf, self.member_id) } else { put_nullable_string(buf, self.member_id) }
        }
        if version >= 9 {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 9 {
            n += if flex { compact_nullable_string_len(self.member_id) } else { nullable_string_len(self.member_id) };
        }
        if version >= 9 {
            n += 4;
        }
...
impl<'de> DecodeBorrow<'de> for OffsetFetchRequestGroup<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 9 {
            out.member_id = if flex { get_compact_nullable_string_borrowed(buf)? } else { get_nullable_string_borrowed(buf)? };
        }
        if version >= 9 {
