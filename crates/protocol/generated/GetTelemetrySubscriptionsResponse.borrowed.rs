...
impl<'a> GetTelemetrySubscriptionsResponse<'a> {
    #[must_use]
    pub fn to_owned(&self) -> crate::owned::get_telemetry_subscriptions_response::GetTelemetrySubscriptionsResponse {
        crate::owned::get_telemetry_subscriptions_response::GetTelemetrySubscriptionsResponse {
            ...
            requested_metrics: (self.requested_metrics).iter().map(std::string::ToString::to_string).collect(),
    ...
    fn encode_field_4<B: BufMut>(&self, buf: &mut B, version: i16, flex: bool) {
        if version >= 0 {
            {
                crate::primitives::array::put_array_len(buf, (self.accepted_compression_types).len(), flex);
...
impl Encode for GetTelemetrySubscriptionsResponse<'_> {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.accepted_compression_types).len(), flex);
        ...
        if version >= 0 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.requested_metrics).len(), flex);
                let body: usize = (self.requested_metrics).iter().map(|it| if flex { compact_string_len(it) } else { string_len(it) }).sum();
                prefix + body
            };
        }
        if flex {
