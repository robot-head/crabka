...
impl<'a> GetTelemetrySubscriptionsResponse<'a> {
    pub fn to_owned(&self) -> crate::owned::get_telemetry_subscriptions_response::GetTelemetrySubscriptionsResponse {
        crate::owned::get_telemetry_subscriptions_response::GetTelemetrySubscriptionsResponse {
            ...
            requested_metrics: (self.requested_metrics).iter().map(|s| s.to_string()).collect(),
...
impl<'de> DecodeBorrow<'de> for GetTelemetrySubscriptionsResponse<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 { out.requested_metrics = { let n = crate::primitives::array::get_array_len(buf, flex)?; let mut v = Vec::with_capacity(n); for _ in 0..n { v.push(if flex { get_compact_string_borrowed(buf)? } else { get_string_borrowed(buf)? }); } v }; }
        if flex {
