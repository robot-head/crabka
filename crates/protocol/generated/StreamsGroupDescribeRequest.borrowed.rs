...
impl StreamsGroupDescribeRequest<'_> {
    #[must_use]
    pub fn to_owned(&self) -> crate::owned::streams_group_describe_request::StreamsGroupDescribeRequest {
        crate::owned::streams_group_describe_request::StreamsGroupDescribeRequest {
            group_ids: (self.group_ids).iter().map(std::string::ToString::to_string).collect(),
            include_authorized_operations: (self.include_authorized_operations),
            unknown_tagged_fields: self.unknown_tagged_fields.clone(),
        }
    }
}
impl Encode for StreamsGroupDescribeRequest<'_> {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.group_ids).len(), flex);
                let body: usize = (self.group_ids).iter().map(|it| if flex { compact_string_len(it) } else { string_len(it) }).sum();
                prefix + body
            };
        }
        if version >= 0 {
