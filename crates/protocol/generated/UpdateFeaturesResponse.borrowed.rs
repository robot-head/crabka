...
impl UpdateFeaturesResponse<'_> {
    ...
    pub fn to_owned(&self) -> crate::owned::update_features_response::UpdateFeaturesResponse {
        crate::owned::update_features_response::UpdateFeaturesResponse {
            ...
            results: (self.results).iter().map(UpdatableFeatureResult::to_owned).collect(),
...
impl Encode for UpdatableFeatureResult<'_> {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        let flex = version >= 0;
        if version >= 0 {
            if flex { put_compact_string(buf, self.feature) } else { put_string(buf, self.feature) }
        }
        if version >= 0 {
            put_i16(buf, self.error_code);
        }
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        let mut n: usize = 0;
        if version >= 0 {
            n += if flex { compact_string_len(self.feature) } else { string_len(self.feature) };
        }
        if version >= 0 {
            n += 2;
        }
...
impl<'de> DecodeBorrow<'de> for UpdatableFeatureResult<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        let mut out = Self::default();
        if version >= 0 {
            out.feature = if flex { get_compact_string_borrowed(buf)? } else { get_string_borrowed(buf)? };
        }
        if version >= 0 {
            out.error_code = get_i16(buf)?;
        }
