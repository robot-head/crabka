...
impl CreateDelegationTokenRequest<'_> {
    ...
    pub fn to_owned(&self) -> crate::owned::create_delegation_token_request::CreateDelegationTokenRequest {
        crate::owned::create_delegation_token_request::CreateDelegationTokenRequest {
            ...
            renewers: (self.renewers).iter().map(CreatableRenewers::to_owned).collect(),
...
impl Encode for CreateDelegationTokenRequest<'_> {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 3 {
            n += if flex {
                compact_nullable_string_len(self.owner_principal_type)
            } else {
                nullable_string_len(self.owner_principal_type)
            };
        }
        if version >= 3 {
            n += if flex {
                compact_nullable_string_len(self.owner_principal_name)
            } else {
                nullable_string_len(self.owner_principal_name)
            };
        }
        if version >= 0 {
            n += {
...
impl<'de> DecodeBorrow<'de> for CreateDelegationTokenRequest<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 3 {
            out.owner_principal_type = if flex { get_compact_nullable_string_borrowed(buf)? } else { get_nullable_string_borrowed(buf)? };
        }
        if version >= 3 {
            out.owner_principal_name = if flex { get_compact_nullable_string_borrowed(buf)? } else { get_nullable_string_borrowed(buf)? };
        }
        if version >= 0 {
            out.renewers = {
                ...
                for _ in 0..n {
                    v.push(CreatableRenewers::decode_borrow(buf, version)?);
                }
                v
            };
        }
...
impl CreateDelegationTokenRequest<'_> {
    ...
    pub fn populated(version: i16) -> Self {
        ...
        if version >= 3 {
            m.owner_principal_type = Some("x");
        }
        if version >= 3 {
            m.owner_principal_name = Some("x");
        }
        if version >= 0 {
