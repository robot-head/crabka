...
impl Encode for ConsumerProtocolAssignment {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        ...
        if version >= 0 {
            {
                crate::primitives::array::put_array_len(buf, (self.assigned_partitions).len(), flex);
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.assigned_partitions).len(), flex);
                let body: usize = (self.assigned_partitions).iter().map(|it| it.encoded_len(version)).sum();
                prefix + body
            };
        }
        if version >= 0 {
            n += if flex { compact_nullable_bytes_len(self.user_data.as_deref()) } else { nullable_bytes_len(self.user_data.as_deref()) };
        }
        n
    }
}
impl Decode<'_> for ConsumerProtocolAssignment {
    fn decode<B: Buf>(buf: &mut B, version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 {
            out.user_data = if flex { get_compact_nullable_bytes_owned(buf)? } else { get_nullable_bytes_owned(buf)? };
        }
        Ok(out)
    }
}
