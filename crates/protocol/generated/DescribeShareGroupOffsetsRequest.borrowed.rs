...
impl DescribeShareGroupOffsetsRequest<'_> {
    ...
    pub fn to_owned(&self) -> crate::owned::describe_share_group_offsets_request::DescribeShareGroupOffsetsRequest {
        crate::owned::describe_share_group_offsets_request::DescribeShareGroupOffsetsRequest {
            groups: (self.groups).iter().map(DescribeShareGroupOffsetsRequestGroup::to_owned).collect(),
    ...
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 {
            out.groups = {
                ...
                for _ in 0..n {
                    v.push(DescribeShareGroupOffsetsRequestGroup::decode_borrow(buf, version)?);
                }
                v
            };
        }
...
impl DescribeShareGroupOffsetsRequestGroup<'_> {
    ...
    pub fn to_owned(&self) -> crate::owned::describe_share_group_offsets_request::DescribeShareGroupOffsetsRequestGroup {
        crate::owned::describe_share_group_offsets_request::DescribeShareGroupOffsetsRequestGroup {
            ...
            topics: (self.topics).as_ref().map(|v| v.iter().map(DescribeShareGroupOffsetsRequestTopic::to_owned).collect()),
            unknown_tagged_fields: self.unknown_tagged_fields.clone(),
        }
    }
}
            ...
            out.topics = {
                ...
                match opt {
                    ...
                    Some(n) => {
                        let mut v = Vec::with_capacity(n);
                        for _ in 0..n {
                            v.push(DescribeShareGroupOffsetsRequestTopic::decode_borrow(buf, version)?);
                        }
                        Some(v)
                    }
                }
...
impl DescribeShareGroupOffsetsRequestGroup<'_> {
    ...
    pub fn populated(version: i16) -> Self {
        ...
        if version >= 0 {
            m.topics = Some(vec![DescribeShareGroupOffsetsRequestTopic::populated(version)]);
        }
        m
    }
}
