...
impl Encode for RequestHeader<'_> {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        ...
        if version >= 1 {
            {
                let flex = false;
                if flex { put_compact_nullable_string(buf, self.client_id) } else { put_nullable_string(buf, self.client_id) }
            }
        }
        if flex {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 1 {
            n += {
                let flex = false;
                if flex { compact_nullable_string_len(self.client_id) } else { nullable_string_len(self.client_id) }
            };
        }
        if flex {
