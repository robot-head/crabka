...
impl Encode for DefaultPrincipalData<'_> {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        ...
        if version >= 0 {
            if flex { put_compact_string(buf, self.type_) } else { put_string(buf, self.type_) }
        }
        if version >= 0 {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        let mut n: usize = 0;
        if version >= 0 {
            n += if flex { compact_string_len(self.type_) } else { string_len(self.type_) };
        }
        if version >= 0 {
...
impl<'de> DecodeBorrow<'de> for DefaultPrincipalData<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 {
            out.type_ = if flex { get_compact_string_borrowed(buf)? } else { get_string_borrowed(buf)? };
        }
        if version >= 0 {
