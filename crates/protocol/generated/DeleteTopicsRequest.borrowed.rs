...
impl DeleteTopicsRequest<'_> {
    ...
    pub fn to_owned(&self) -> crate::owned::delete_topics_request::DeleteTopicsRequest {
        crate::owned::delete_topics_request::DeleteTopicsRequest {
            topics: (self.topics).iter().map(DeleteTopicState::to_owned).collect(),
            topic_names: (self.topic_names).iter().map(std::string::ToString::to_string).collect(),
...
impl Encode for DeleteTopicState<'_> {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        let mut n: usize = 0;
        if version >= 6 {
            n += if flex { compact_nullable_string_len(self.name) } else { nullable_string_len(self.name) };
        }
        if version >= 6 {
            n += 16;
        }
...
impl<'de> DecodeBorrow<'de> for DeleteTopicState<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        let mut out = Self::default();
        if version >= 6 {
            out.name = if flex { get_compact_nullable_string_borrowed(buf)? } else { get_nullable_string_borrowed(buf)? };
        }
        if version >= 6 {
