...
impl Encode for DeleteGroupsRequest {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.groups_names).len(), flex);
                let body: usize = (self.groups_names).iter().map(|it| if flex { compact_string_len(it) } else { string_len(it) }).sum();
                prefix + body
            };
        }
        if flex {
