...
impl AlterReplicaLogDirsRequest<'_> {
    ...
    pub fn to_owned(&self) -> crate::owned::alter_replica_log_dirs_request::AlterReplicaLogDirsRequest {
        crate::owned::alter_replica_log_dirs_request::AlterReplicaLogDirsRequest {
            dirs: (self.dirs).iter().map(AlterReplicaLogDir::to_owned).collect(),
...
impl Encode for AlterReplicaLogDirsRequest<'_> {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.dirs).len(), flex);
...
impl AlterReplicaLogDir<'_> {
    ...
    pub fn to_owned(&self) -> crate::owned::alter_replica_log_dirs_request::AlterReplicaLogDir {
        crate::owned::alter_replica_log_dirs_request::AlterReplicaLogDir {
            ...
            topics: (self.topics).iter().map(AlterReplicaLogDirTopic::to_owned).collect(),
            unknown_tagged_fields: self.unknown_tagged_fields.clone(),
        }
    }
}
impl Encode for AlterReplicaLogDir<'_> {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        let flex = version >= 2;
        if version >= 0 {
            if flex { put_compact_string(buf, self.path) } else { put_string(buf, self.path) }
        }
        if version >= 0 {
            {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        let mut n: usize = 0;
        if version >= 0 {
            n += if flex { compact_string_len(self.path) } else { string_len(self.path) };
        }
        if version >= 0 {
            n += {
...
impl<'de> DecodeBorrow<'de> for AlterReplicaLogDir<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        let mut out = Self::default();
        if version >= 0 {
            out.path = if flex { get_compact_string_borrowed(buf)? } else { get_string_borrowed(buf)? };
        }
        if version >= 0 {
            out.topics = {
                let n = crate::primitives::array::get_array_len(buf, flex)?;
