...
impl Encode for EndTxnResponse {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        ...
        if version >= 5 { put_i64(buf, self.producer_id) }
        if version >= 5 { put_i16(buf, self.producer_epoch) }
        if flex {
...
impl<'de> DecodeBorrow<'de> for EndTxnResponse {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 5 { out.producer_id = get_i64(buf)?; }
        if version >= 5 { out.producer_epoch = get_i16(buf)?; }
        if flex {
