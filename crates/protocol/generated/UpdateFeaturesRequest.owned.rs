...
impl Encode for UpdateFeaturesRequest {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.feature_updates).len(), flex);
                let body: usize = (self.feature_updates).iter().map(|it| it.encoded_len(version)).sum();
                prefix + body
            };
        }
        if version >= 1 {
...
impl Decode<'_> for FeatureUpdateKey {
    fn decode<B: Buf>(buf: &mut B, version: i16) -> Result<Self, ProtocolError> {
        ...
        let mut out = Self::default();
        if version >= 0 {
            out.feature = if flex { get_compact_string_owned(buf)? } else { get_string_owned(buf)? };
        }
        if version >= 0 {
