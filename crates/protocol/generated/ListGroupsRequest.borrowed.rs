...
impl ListGroupsRequest<'_> {
    ...
    pub fn to_owned(&self) -> crate::owned::list_groups_request::ListGroupsRequest {
        crate::owned::list_groups_request::ListGroupsRequest {
            states_filter: (self.states_filter).iter().map(std::string::ToString::to_string).collect(),
            types_filter: (self.types_filter).iter().map(std::string::ToString::to_string).collect(),
...
impl Encode for ListGroupsRequest<'_> {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 4 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.states_filter).len(), flex);
                let body: usize = (self.states_filter).iter().map(|it| if flex { compact_string_len(it) } else { string_len(it) }).sum();
                prefix + body
            };
        }
        if version >= 5 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.types_filter).len(), flex);
                let body: usize = (self.types_filter).iter().map(|it| if flex { compact_string_len(it) } else { string_len(it) }).sum();
                prefix + body
            };
        }
        if flex {
