...
impl Encode for BeginTransactionRecord {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        if !(MIN_VERSION..=MAX_VERSION).contains(&version) {
            return Err(ProtocolError::SchemaMismatch("BeginTransactionRecord version out of range"));
        }
        let flex = is_flexible(version);
        if flex {
            let mut tagged = WriteTaggedFields::new();
            if self.name.is_some() {
                let payload = encode_to_bytes(if flex { compact_nullable_string_len(self.name.as_deref()) } else { nullable_string_len(self.name.as_deref()) }, |b| {
                    if flex {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if flex {
            ...
            if self.name.is_some() {
                known_pairs.push((0, if flex { compact_nullable_string_len(self.name.as_deref()) } else { nullable_string_len(self.name.as_deref()) }));
            }
