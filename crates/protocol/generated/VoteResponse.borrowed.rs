...
pub struct VoteResponse<'a> {
    pub error_code: i16,
    pub topics: Vec<TopicData<'a>>,
    pub node_endpoints: Vec<crate::owned::vote_response::NodeEndpoint>,
    pub unknown_tagged_fields: UnknownTaggedFields,
}

impl<'a> Default for VoteResponse<'a> {
    fn default() -> Self {
        Self {
            error_code: 0i16,
            topics: Vec::new(),
...
impl<'a> Encode for VoteResponse<'a> {
    ...
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if flex {
            ...
            out.unknown_tagged_fields = read_tagged_fields(buf, |tag, payload| {
                match tag {
                0 => { tag_node_endpoints = Some({ let b: &mut &[u8] = payload; { let n = crate::primitives::array::get_array_len(b, flex)?; let mut v = Vec::with_capacity(n); for _ in 0..n { v.push(crate::owned::vote_response::NodeEndpoint::decode(b, version)?); } v } }); Ok(true) }
                    _ => Ok(false),
                }
            })?;
...
impl Encode for PartitionData {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        ...
        if version >= 0 { put_bool(buf, self.vote_granted) }
        if flex {
            let tagged = WriteTaggedFields::new();
            tagged.write(buf, &self.unknown_tagged_fields);
        }
...
impl<'de> DecodeBorrow<'de> for PartitionData {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 { out.vote_granted = get_bool(buf)?; }
        if flex {
            out.unknown_tagged_fields = read_tagged_fields(buf, |_tag, _payload| {
                Ok(false)
            })?;
