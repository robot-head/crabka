...
impl TopicInfo<'_> {
    #[must_use]
    pub fn to_owned(&self) -> crate::owned::common::streams_group_describe_response::topic_info::TopicInfo {
        crate::owned::common::streams_group_describe_response::topic_info::TopicInfo {
            name: (self.name).to_string(),
            partitions: (self.partitions),
            replication_factor: (self.replication_factor),
            topic_configs: (self.topic_configs).iter().map(super::key_value::KeyValue::to_owned).collect(),
...
impl Encode for TopicInfo<'_> {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.topic_configs).len(), flex);
                let body: usize = (self.topic_configs).iter().map(|it| it.encoded_len(version)).sum();
                prefix + body
            };
        }
        if flex {
