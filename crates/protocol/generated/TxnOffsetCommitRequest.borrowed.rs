...
impl<'a> TxnOffsetCommitRequest<'a> {
    ...
    pub fn to_owned(&self) -> crate::owned::txn_offset_commit_request::TxnOffsetCommitRequest {
        crate::owned::txn_offset_commit_request::TxnOffsetCommitRequest {
            ...
            topics: (self.topics).iter().map(TxnOffsetCommitRequestTopic::to_owned).collect(),
            unknown_tagged_fields: self.unknown_tagged_fields.clone(),
        }
    }
    fn encode_field_0<B: BufMut>(&self, buf: &mut B, version: i16, flex: bool) {
        if version >= 0 {
            if flex { put_compact_string(buf, self.transactional_id) } else { put_string(buf, self.transactional_id) }
        }
    }
    ...
    fn decode_field_0(out: &mut Self, buf: &mut &'a [u8], version: i16, flex: bool) -> Result<(), ProtocolError> {
        if version >= 0 {
            out.transactional_id = if flex { get_compact_string_borrowed(buf)? } else { get_string_borrowed(buf)? };
        }
        Ok(())
    }
    ...
    fn decode_field_7(out: &mut Self, buf: &mut &'a [u8], version: i16, flex: bool) -> Result<(), ProtocolError> {
        if version >= 0 {
            out.topics = {
                ...
                for _ in 0..n {
                    v.push(TxnOffsetCommitRequestTopic::decode_borrow(buf, version)?);
                }
                v
            };
        }
...
impl Encode for TxnOffsetCommitRequest<'_> {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        let mut n: usize = 0;
        if version >= 0 {
            n += if flex { compact_string_len(self.transactional_id) } else { string_len(self.transactional_id) };
        }
        if version >= 0 {
...
impl TxnOffsetCommitRequestTopic<'_> {
    ...
    pub fn to_owned(&self) -> crate::owned::txn_offset_commit_request::TxnOffsetCommitRequestTopic {
        crate::owned::txn_offset_commit_request::TxnOffsetCommitRequestTopic {
            ...
            partitions: (self.partitions).iter().map(TxnOffsetCommitRequestPartition::to_owned).collect(),
            unknown_tagged_fields: self.unknown_tagged_fields.clone(),
        }
    }
}
    ...
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 {
            out.partitions = {
                ...
                let mut v = Vec::with_capacity(n);
                for _ in 0..n {
                    v.push(TxnOffsetCommitRequestPartition::decode_borrow(buf, version)?);
                }
                v
            };
        }
...
impl Encode for TxnOffsetCommitRequestPartition<'_> {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += if flex { compact_nullable_string_len(self.committed_metadata) } else { nullable_string_len(self.committed_metadata) };
        }
        if flex {
            let known_pairs: Vec<(u32, usize)> = Vec::new();
            n += tagged_fields_len(&known_pairs, &self.unknown_tagged_fields);
...
impl<'de> DecodeBorrow<'de> for TxnOffsetCommitRequestPartition<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 {
            out.committed_metadata = if flex { get_compact_nullable_string_borrowed(buf)? } else { get_nullable_string_borrowed(buf)? };
        }
        if flex {
            out.unknown_tagged_fields = read_tagged_fields(buf, |_tag, _payload| Ok(false))?;
        }
