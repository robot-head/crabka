...
impl Decode<'_> for SyncGroupRequest {
    fn decode<B: Buf>(buf: &mut B, version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 5 {
            out.protocol_name = if flex { get_compact_nullable_string_owned(buf)? } else { get_nullable_string_owned(buf)? };
        }
        if version >= 0 {
            out.assignments = {
...
impl Encode for SyncGroupRequestAssignment {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += if flex { compact_bytes_len(&self.assignment) } else { bytes_len(&self.assignment) };
        }
        if flex {
            let known_pairs: Vec<(u32, usize)> = Vec::new();
            n += tagged_fields_len(&known_pairs, &self.unknown_tagged_fields);
...
impl Decode<'_> for SyncGroupRequestAssignment {
    fn decode<B: Buf>(buf: &mut B, version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 {
            out.assignment = if flex { get_compact_bytes_owned(buf)? } else { get_bytes_owned(buf)? };
        }
        if flex {
            out.unknown_tagged_fields = read_tagged_fields(buf, |_tag, _payload| Ok(false))?;
        }
...
pub fn default_json(version: i16) -> ::serde_json::Value {
    ...
    obj.insert("assignments".to_string(), ::serde_json::Value::Array(vec![]));
