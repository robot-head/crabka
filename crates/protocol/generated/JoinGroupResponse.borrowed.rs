...
impl<'a> JoinGroupResponse<'a> {
    ...
    pub fn to_owned(&self) -> crate::owned::join_group_response::JoinGroupResponse {
        crate::owned::join_group_response::JoinGroupResponse {
            ...
            members: (self.members).iter().map(JoinGroupResponseMember::to_owned).collect(),
    ...
    fn encode_field_3<B: BufMut>(&self, buf: &mut B, version: i16, flex: bool) {
        if version >= 7 {
            if flex { put_compact_nullable_string(buf, self.protocol_type) } else { put_nullable_string(buf, self.protocol_type) }
        }
    }
    fn encode_field_4<B: BufMut>(&self, buf: &mut B, version: i16, flex: bool) {
        if version >= 0 {
            if version >= 7 {
                if flex { put_compact_nullable_string(buf, self.protocol_name) } else { put_nullable_string(buf, self.protocol_name) }
            } else {
                if flex {
    ...
    fn encode_field_5<B: BufMut>(&self, buf: &mut B, version: i16, flex: bool) {
        if version >= 0 {
            if flex { put_compact_string(buf, self.leader) } else { put_string(buf, self.leader) }
        }
    }
    ...
    fn decode_field_3(out: &mut Self, buf: &mut &'a [u8], version: i16, flex: bool) -> Result<(), ProtocolError> {
        if version >= 7 {
            out.protocol_type = if flex { get_compact_nullable_string_borrowed(buf)? } else { get_nullable_string_borrowed(buf)? };
        }
        Ok(())
    }
    fn decode_field_4(out: &mut Self, buf: &mut &'a [u8], version: i16, flex: bool) -> Result<(), ProtocolError> {
        if version >= 0 {
            out.protocol_name = if version >= 7 {
                if flex { get_compact_nullable_string_borrowed(buf)? } else { get_nullable_string_borrowed(buf)? }
            } else {
                Some(if flex { get_compact_string_borrowed(buf)? } else { get_string_borrowed(buf)? })
            };
        }
        Ok(())
    }
    fn decode_field_5(out: &mut Self, buf: &mut &'a [u8], version: i16, flex: bool) -> Result<(), ProtocolError> {
        if version >= 0 {
            out.leader = if flex { get_compact_string_borrowed(buf)? } else { get_string_borrowed(buf)? };
        }
        Ok(())
    }
...
impl Encode for JoinGroupResponse<'_> {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 7 {
            n += if flex { compact_nullable_string_len(self.protocol_type) } else { nullable_string_len(self.protocol_type) };
        }
        if version >= 0 {
            n += if version >= 7 {
                if flex { compact_nullable_string_len(self.protocol_name) } else { nullable_string_len(self.protocol_name) }
            } else {
                if flex {
        ...
        if version >= 0 {
            n += if flex { compact_string_len(self.leader) } else { string_len(self.leader) };
        }
        if version >= 9 {
            n += 1;
        }
...
impl Encode for JoinGroupResponseMember<'_> {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        ...
        if version >= 0 {
            if flex { put_compact_bytes(buf, self.metadata) } else { put_bytes(buf, self.metadata) }
        }
        if flex {
            let tagged = WriteTaggedFields::new();
            tagged.write(buf, &self.unknown_tagged_fields);
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += if flex {
                crate::primitives::varint::uvarint_len(u32::try_from((self.metadata).len() + 1).unwrap()) + (self.metadata).len()
            } else {
                4 + (self.metadata).len()
            };
        }
...
impl<'de> DecodeBorrow<'de> for JoinGroupResponseMember<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 {
            out.metadata = if flex { get_compact_bytes_borrowed(buf)? } else { get_bytes_borrowed(buf)? };
        }
        if flex {
            out.unknown_tagged_fields = read_tagged_fields(buf, |_tag, _payload| Ok(false))?;
        }
