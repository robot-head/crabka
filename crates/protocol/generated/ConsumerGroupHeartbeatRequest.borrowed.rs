...
impl<'a> ConsumerGroupHeartbeatRequest<'a> {
    ...
    pub fn to_owned(&self) -> crate::owned::consumer_group_heartbeat_request::ConsumerGroupHeartbeatRequest {
        crate::owned::consumer_group_heartbeat_request::ConsumerGroupHeartbeatRequest {
            ...
            subscribed_topic_names: (self.subscribed_topic_names).as_ref().map(|v| v.iter().map(std::string::ToString::to_string).collect()),
    ...
    fn decode_field_8(out: &mut Self, buf: &mut &'a [u8], version: i16, flex: bool) -> Result<(), ProtocolError> {
        if version >= 0 {
            out.server_assignor = if flex { get_compact_nullable_string_borrowed(buf)? } else { get_nullable_string_borrowed(buf)? };
        }
        Ok(())
    }
...
impl Encode for ConsumerGroupHeartbeatRequest<'_> {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += if flex { compact_nullable_string_len(self.server_assignor) } else { nullable_string_len(self.server_assignor) };
        }
        if version >= 0 {
            n += {
