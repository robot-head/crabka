...
impl<'a> ShareFetchRequest<'a> {
    ...
    fn encode_field_0<B: BufMut>(&self, buf: &mut B, version: i16, flex: bool) {
        if version >= 0 {
            if flex { put_compact_nullable_string(buf, self.group_id) } else { put_nullable_string(buf, self.group_id) }
        }
    }
    ...
    fn decode_field_0(out: &mut Self, buf: &mut &'a [u8], version: i16, flex: bool) -> Result<(), ProtocolError> {
        if version >= 0 {
            out.group_id = if flex { get_compact_nullable_string_borrowed(buf)? } else { get_nullable_string_borrowed(buf)? };
        }
        Ok(())
    }
...
impl Encode for ShareFetchRequest<'_> {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        let mut n: usize = 0;
        if version >= 0 {
            n += if flex { compact_nullable_string_len(self.group_id) } else { nullable_string_len(self.group_id) };
        }
        if version >= 0 {
...
impl FetchPartition {
    ...
    pub fn to_owned(&self) -> crate::owned::share_fetch_request::FetchPartition {
        crate::owned::share_fetch_request::FetchPartition {
            ...
            acknowledgement_batches: (self.acknowledgement_batches).iter().map(AcknowledgementBatch::to_owned).collect(),
            unknown_tagged_fields: self.unknown_tagged_fields.clone(),
        }
    }
}
impl Encode for FetchPartition {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        ...
        if version >= 0 {
            {
                crate::primitives::array::put_array_len(buf, (self.acknowledgement_batches).len(), flex);
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.acknowledgement_batches).len(), flex);
                let body: usize = (self.acknowledgement_batches).iter().map(|it| it.encoded_len(version)).sum();
                prefix + body
            };
        }
        if flex {
...
impl Encode for AcknowledgementBatch {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.acknowledge_types).len(), flex);
