...
impl<'a> DescribedGroup<'a> {
    ...
    pub fn to_owned(&self) -> crate::owned::describe_groups_response::DescribedGroup {
        crate::owned::describe_groups_response::DescribedGroup {
            ...
            members: (self.members).iter().map(DescribedGroupMember::to_owned).collect(),
    ...
    fn encode_field_4<B: BufMut>(&self, buf: &mut B, version: i16, flex: bool) {
        if version >= 0 {
            if flex { put_compact_string(buf, self.protocol_type) } else { put_string(buf, self.protocol_type) }
        }
    }
    fn encode_field_5<B: BufMut>(&self, buf: &mut B, version: i16, flex: bool) {
        if version >= 0 {
            if flex { put_compact_string(buf, self.protocol_data) } else { put_string(buf, self.protocol_data) }
        }
    }
    ...
    fn decode_field_4(out: &mut Self, buf: &mut &'a [u8], version: i16, flex: bool) -> Result<(), ProtocolError> {
        if version >= 0 {
            out.protocol_type = if flex { get_compact_string_borrowed(buf)? } else { get_string_borrowed(buf)? };
        }
        Ok(())
    }
    fn decode_field_5(out: &mut Self, buf: &mut &'a [u8], version: i16, flex: bool) -> Result<(), ProtocolError> {
        if version >= 0 {
            out.protocol_data = if flex { get_compact_string_borrowed(buf)? } else { get_string_borrowed(buf)? };
        }
        Ok(())
    }
    ...
    fn decode_field_7(out: &mut Self, buf: &mut &'a [u8], version: i16, _flex: bool) -> Result<(), ProtocolError> {
        if version >= 3 {
...
impl Encode for DescribedGroup<'_> {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += if flex { compact_string_len(self.protocol_type) } else { string_len(self.protocol_type) };
        }
        if version >= 0 {
            n += if flex { compact_string_len(self.protocol_data) } else { string_len(self.protocol_data) };
        }
        if version >= 0 {
            n += {
...
impl Encode for DescribedGroupMember<'_> {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        ...
        if version >= 0 {
            if flex { put_compact_bytes(buf, self.member_metadata) } else { put_bytes(buf, self.member_metadata) }
        }
        if version >= 0 {
            if flex { put_compact_bytes(buf, self.member_assignment) } else { put_bytes(buf, self.member_assignment) }
        }
        if flex {
            let tagged = WriteTaggedFields::new();
            tagged.write(buf, &self.unknown_tagged_fields);
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 4 {
            n += if flex { compact_nullable_string_len(self.group_instance_id) } else { nullable_string_len(self.group_instance_id) };
        }
        if version >= 0 {
        ...
        if version >= 0 {
            n += if flex {
                crate::primitives::varint::uvarint_len(u32::try_from((self.member_metadata).len() + 1).unwrap()) + (self.member_metadata).len()
            } else {
        ...
        if version >= 0 {
            n += if flex {
                crate::primitives::varint::uvarint_len(u32::try_from((self.member_assignment).len() + 1).unwrap()) + (self.member_assignment).len()
            } else {
...
impl<'de> DecodeBorrow<'de> for DescribedGroupMember<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 4 {
            out.group_instance_id = if flex { get_compact_nullable_string_borrowed(buf)? } else { get_nullable_string_borrowed(buf)? };
        }
        if version >= 0 {
        ...
        if version >= 0 {
            out.member_metadata = if flex { get_compact_bytes_borrowed(buf)? } else { get_bytes_borrowed(buf)? };
        }
        if version >= 0 {
            out.member_assignment = if flex { get_compact_bytes_borrowed(buf)? } else { get_bytes_borrowed(buf)? };
        }
        if flex {
            out.unknown_tagged_fields = read_tagged_fields(buf, |_tag, _payload| Ok(false))?;
        }
