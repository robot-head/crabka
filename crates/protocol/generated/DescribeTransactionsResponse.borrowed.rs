...
impl DescribeTransactionsResponse<'_> {
    ...
    pub fn to_owned(&self) -> crate::owned::describe_transactions_response::DescribeTransactionsResponse {
        crate::owned::describe_transactions_response::DescribeTransactionsResponse {
            ...
            transaction_states: (self.transaction_states).iter().map(TransactionState::to_owned).collect(),
...
impl<'a> TransactionState<'a> {
    ...
    fn decode_field_2(out: &mut Self, buf: &mut &'a [u8], version: i16, flex: bool) -> Result<(), ProtocolError> {
        if version >= 0 {
            out.transaction_state = if flex { get_compact_string_borrowed(buf)? } else { get_string_borrowed(buf)? };
        }
        Ok(())
    }
