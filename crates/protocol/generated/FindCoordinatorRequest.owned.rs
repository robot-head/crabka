...
impl Encode for FindCoordinatorRequest {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 4 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.coordinator_keys).len(), flex);
                let body: usize = (self.coordinator_keys).iter().map(|it| if flex { compact_string_len(it) } else { string_len(it) }).sum();
                prefix + body
            };
        }
        if flex {
...
impl FindCoordinatorRequest {
    ...
    pub fn populated(version: i16) -> Self {
        ...
        if version >= 4 {
            m.coordinator_keys = vec!["x".to_string()];
        }
        m
    }
}
