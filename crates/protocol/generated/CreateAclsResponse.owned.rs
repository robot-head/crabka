...
impl<'de> Decode<'de> for CreateAclsResponse {
    fn decode<B: Buf>(buf: &mut B, version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 { out.results = { let n = crate::primitives::array::get_array_len(buf, flex)?; let mut v = Vec::with_capacity(n); for _ in 0..n { v.push(AclCreationResult::decode(buf, version)?); } v }; }
        if flex {
...
impl Encode for AclCreationResult {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        let flex = version >= 2;
...
pub fn default_json() -> ::serde_json::Value {
    ::serde_json::json!({"ThrottleTimeMs": 0, "Results": []})
}
