...
impl Encode for AddPartitionsToTxnRequest<'_> {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        ...
        if (0..=3).contains(&version) {
            {
                crate::primitives::array::put_array_len(buf, (self.v3_and_below_topics).len(), flex);
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 4 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.transactions).len(), flex);
                let body: usize = (self.transactions).iter().map(|it| it.encoded_len(version)).sum();
                prefix + body
            };
        }
        if (0..=3).contains(&version) {
        ...
        if (0..=3).contains(&version) {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.v3_and_below_topics).len(), flex);
                let body: usize = (self.v3_and_below_topics).iter().map(|it| it.encoded_len(version)).sum();
                prefix + body
            };
        }
        if flex {
...
impl<'de> DecodeBorrow<'de> for AddPartitionsToTxnRequest<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if (0..=3).contains(&version) {
            out.v3_and_below_transactional_id = if flex { get_compact_string_borrowed(buf)? } else { get_string_borrowed(buf)? };
        }
        if (0..=3).contains(&version) {
