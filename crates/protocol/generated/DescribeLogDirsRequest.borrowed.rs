...
impl<'de> DecodeBorrow<'de> for DescribeLogDirsRequest<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 { out.topics = { let opt = crate::primitives::array::get_nullable_array_len(buf, flex)?; match opt { None => None, Some(n) => { let mut v = Vec::with_capacity(n); for _ in 0..n { v.push(DescribableLogDirTopic::decode_borrow(buf, version)?); } Some(v) } } }; }
        if flex {
