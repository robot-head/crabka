...
impl<'a> ListConfigResourcesResponse<'a> {
    pub fn to_owned(&self) -> crate::owned::list_config_resources_response::ListConfigResourcesResponse {
        crate::owned::list_config_resources_response::ListConfigResourcesResponse {
            throttle_time_ms: (self.throttle_time_ms),
            error_code: (self.error_code),
            config_resources: (self.config_resources).iter().map(|it| it.to_owned()).collect(),
...
impl<'de> DecodeBorrow<'de> for ListConfigResourcesResponse<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 { out.config_resources = { let n = crate::primitives::array::get_array_len(buf, flex)?; let mut v = Vec::with_capacity(n); for _ in 0..n { v.push(ConfigResource::decode_borrow(buf, version)?); } v }; }
        if flex {
