...
impl<'de> Decode<'de> for DescribeUserScramCredentialsRequest {
    fn decode<B: Buf>(buf: &mut B, version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 { out.users = { let opt = crate::primitives::array::get_nullable_array_len(buf, flex)?; match opt { None => None, Some(n) => { let mut v = Vec::with_capacity(n); for _ in 0..n { v.push(UserName::decode(buf, version)?); } Some(v) } } }; }
        if flex {
...
impl crate::ProtocolRequest for DescribeUserScramCredentialsRequest {
    ...
    type Response = super::describe_user_scram_credentials_response::DescribeUserScramCredentialsResponse;
}
