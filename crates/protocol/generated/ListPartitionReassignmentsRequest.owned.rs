...
impl<'de> Decode<'de> for ListPartitionReassignmentsRequest {
    fn decode<B: Buf>(buf: &mut B, version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 { out.topics = { let opt = crate::primitives::array::get_nullable_array_len(buf, flex)?; match opt { None => None, Some(n) => { let mut v = Vec::with_capacity(n); for _ in 0..n { v.push(ListPartitionReassignmentsTopics::decode(buf, version)?); } Some(v) } } }; }
        if flex {
...
impl crate::ProtocolRequest for ListPartitionReassignmentsRequest {
    ...
    type Response = super::list_partition_reassignments_response::ListPartitionReassignmentsResponse;
}
