...
impl Encode for SaslHandshakeRequest {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        ...
        if version >= 0 { if flex { put_compact_string(buf, &self.mechanism) } else { put_string(buf, &self.mechanism) } }
        Ok(())
    }
    fn encoded_len(&self, version: i16) -> usize {
        let flex = is_flexible(version);
        let mut n: usize = 0;
        if version >= 0 { n += if flex { compact_string_len(&self.mechanism) } else { string_len(&self.mechanism) }; }
        n
    }
}
...
impl<'de> Decode<'de> for SaslHandshakeRequest {
    fn decode<B: Buf>(buf: &mut B, version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 { out.mechanism = if flex { get_compact_string_owned(buf)? } else { get_string_owned(buf)? }; }
        Ok(out)
    }
}
