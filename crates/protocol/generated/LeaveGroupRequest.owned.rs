...
impl Encode for MemberIdentity {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 5 {
            n += if flex { compact_nullable_string_len(self.reason.as_deref()) } else { nullable_string_len(self.reason.as_deref()) };
        }
        if flex {
            let known_pairs: Vec<(u32, usize)> = Vec::new();
            n += tagged_fields_len(&known_pairs, &self.unknown_tagged_fields);
...
impl Decode<'_> for MemberIdentity {
    fn decode<B: Buf>(buf: &mut B, version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 5 {
            out.reason = if flex { get_compact_nullable_string_owned(buf)? } else { get_nullable_string_owned(buf)? };
        }
        if flex {
            out.unknown_tagged_fields = read_tagged_fields(buf, |_tag, _payload| Ok(false))?;
        }
