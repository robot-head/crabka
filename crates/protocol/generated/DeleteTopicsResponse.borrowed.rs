...
impl<'de> DecodeBorrow<'de> for DeleteTopicsResponse<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 { out.responses = { let n = crate::primitives::array::get_array_len(buf, flex)?; let mut v = Vec::with_capacity(n); for _ in 0..n { v.push(DeletableTopicResult::decode_borrow(buf, version)?); } v }; }
        if flex {
...
impl<'a> Encode for DeletableTopicResult<'a> {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        let flex = version >= 4;
        if version >= 0 { if version >= 6 { if flex { put_compact_nullable_string(buf, self.name) } else { put_nullable_string(buf, self.name) } } else { if flex { put_compact_string(buf, (self.name).unwrap_or("")) } else { put_string(buf, (self.name).unwrap_or("")) } } }
        if version >= 6 { crate::primitives::uuid::put_uuid(buf, self.topic_id) }
        if version >= 0 { put_i16(buf, self.error_code) }
        if version >= 5 { if flex { put_compact_nullable_string(buf, self.error_message) } else { put_nullable_string(buf, self.error_message) } }
        if flex {
            let tagged = WriteTaggedFields::new();
            tagged.write(buf, &self.unknown_tagged_fields);
        }
    ...
    fn encoded_len(&self, version: i16) -> usize {
        let flex = version >= 4;
        let mut n: usize = 0;
        if version >= 0 { n += if version >= 6 { if flex { compact_nullable_string_len(self.name) } else { nullable_string_len(self.name) } } else { if flex { compact_string_len((self.name).unwrap_or("")) } else { string_len((self.name).unwrap_or("")) } }; }
        if version >= 6 { n += 16; }
        if version >= 0 { n += 2; }
        if version >= 5 { n += if flex { compact_nullable_string_len(self.error_message) } else { nullable_string_len(self.error_message) }; }
        if flex {
            let known_pairs: Vec<(u32, usize)> = Vec::new();
            n += tagged_fields_len(&known_pairs, &self.unknown_tagged_fields);
        }
...
impl<'de> DecodeBorrow<'de> for DeletableTopicResult<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        let flex = version >= 4;
        let mut out = Self::default();
        if version >= 0 { out.name = if version >= 6 { if flex { get_compact_nullable_string_borrowed(buf)? } else { get_nullable_string_borrowed(buf)? } } else { Some(if flex { get_compact_string_borrowed(buf)? } else { get_string_borrowed(buf)? }) }; }
        if version >= 6 { out.topic_id = crate::primitives::uuid::get_uuid(buf)?; }
        if version >= 0 { out.error_code = get_i16(buf)?; }
        if version >= 5 { out.error_message = if flex { get_compact_nullable_string_borrowed(buf)? } else { get_nullable_string_borrowed(buf)? }; }
        if flex {
            out.unknown_tagged_fields = read_tagged_fields(buf, |_tag, _payload| {
                Ok(false)
            })?;
