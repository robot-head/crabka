...
impl Encode for RegisterBrokerRecord {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        if !(MIN_VERSION..=MAX_VERSION).contains(&version) {
            return Err(ProtocolError::SchemaMismatch("RegisterBrokerRecord version out of range"));
        }
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.end_points).len(), flex);
                let body: usize = (self.end_points).iter().map(|it| it.encoded_len(version)).sum();
                prefix + body
            };
        }
        if version >= 0 {
...
pub fn default_json(version: i16) -> ::serde_json::Value {
    ...
    if version >= 1 {
        obj.insert("inControlledShutdown".to_string(), ::serde_json::Value::Bool(false));
    }
    if version >= 3 {
