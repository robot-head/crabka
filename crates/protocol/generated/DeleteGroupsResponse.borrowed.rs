...
impl<'de> DecodeBorrow<'de> for DeleteGroupsResponse<'de> {
    fn decode_borrow(buf: &mut &'de [u8], version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 { out.results = { let n = crate::primitives::array::get_array_len(buf, flex)?; let mut v = Vec::with_capacity(n); for _ in 0..n { v.push(DeletableGroupResult::decode_borrow(buf, version)?); } v }; }
        if flex {
