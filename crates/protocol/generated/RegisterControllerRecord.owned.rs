...
impl Encode for RegisterControllerRecord {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        if !(MIN_VERSION..=MAX_VERSION).contains(&version) {
            return Err(ProtocolError::SchemaMismatch("RegisterControllerRecord version out of range"));
        }
    ...
    fn decode<B: Buf>(buf: &mut B, version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 {
            out.end_points = {
                ...
                for _ in 0..n {
                    v.push(ControllerEndpoint::decode(buf, version)?);
                }
                v
            };
        }
        if version >= 0 {
            out.features = {
                ...
                let mut v = Vec::with_capacity(n);
                for _ in 0..n {
                    v.push(ControllerFeature::decode(buf, version)?);
                }
                v
            };
        }
