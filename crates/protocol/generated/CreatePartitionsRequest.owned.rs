...
impl<'de> Decode<'de> for CreatePartitionsRequest {
    fn decode<B: Buf>(buf: &mut B, version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 { out.topics = { let n = crate::primitives::array::get_array_len(buf, flex)?; let mut v = Vec::with_capacity(n); for _ in 0..n { v.push(CreatePartitionsTopic::decode(buf, version)?); } v }; }
...
impl<'de> Decode<'de> for CreatePartitionsTopic {
    fn decode<B: Buf>(buf: &mut B, version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 { out.assignments = { let opt = crate::primitives::array::get_nullable_array_len(buf, flex)?; match opt { None => None, Some(n) => { let mut v = Vec::with_capacity(n); for _ in 0..n { v.push(CreatePartitionsAssignment::decode(buf, version)?); } Some(v) } } }; }
        if flex {
            out.unknown_tagged_fields = read_tagged_fields(buf, |_tag, _payload| {
                Ok(false)
            })?;
...
pub fn default_json() -> ::serde_json::Value {
    ::serde_json::json!({"Topics": [], "TimeoutMs": 0, "ValidateOnly": false})
}
