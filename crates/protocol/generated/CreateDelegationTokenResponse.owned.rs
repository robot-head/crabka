...
impl CreateDelegationTokenResponse {
    ...
    fn decode_field_3<B: Buf>(out: &mut Self, buf: &mut B, version: i16, flex: bool) -> Result<(), ProtocolError> {
        if version >= 3 {
            out.token_requester_principal_type = if flex { get_compact_string_owned(buf)? } else { get_string_owned(buf)? };
        }
        Ok(())
    }
    fn decode_field_4<B: Buf>(out: &mut Self, buf: &mut B, version: i16, flex: bool) -> Result<(), ProtocolError> {
        if version >= 3 {
            out.token_requester_principal_name = if flex { get_compact_string_owned(buf)? } else { get_string_owned(buf)? };
        }
        Ok(())
    }
    ...
    fn decode_field_8<B: Buf>(out: &mut Self, buf: &mut B, version: i16, flex: bool) -> Result<(), ProtocolError> {
        if version >= 0 {
            out.token_id = if flex { get_compact_string_owned(buf)? } else { get_string_owned(buf)? };
        }
        Ok(())
    }
    fn decode_field_9<B: Buf>(out: &mut Self, buf: &mut B, version: i16, flex: bool) -> Result<(), ProtocolError> {
        if version >= 0 {
            out.hmac = if flex { get_compact_bytes_owned(buf)? } else { get_bytes_owned(buf)? };
        }
        Ok(())
    }
...
impl Encode for CreateDelegationTokenResponse {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += if flex { compact_bytes_len(&self.hmac) } else { bytes_len(&self.hmac) };
        }
        if version >= 0 {
            n += 4;
        }
