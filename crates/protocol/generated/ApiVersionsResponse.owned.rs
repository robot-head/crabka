...
impl<'de> Decode<'de> for ApiVersionsResponse {
    fn decode<B: Buf>(buf: &mut B, version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 { out.api_keys = { let n = crate::primitives::array::get_array_len(buf, flex)?; let mut v = Vec::with_capacity(n); for _ in 0..n { v.push(ApiVersion::decode(buf, version)?); } v }; }
        ...
        if flex {
            ...
            out.unknown_tagged_fields = read_tagged_fields(buf, |tag, payload| {
                match tag {
                0 => { tag_supported_features = Some({ let b: &mut &[u8] = payload; { let n = crate::primitives::array::get_array_len(b, flex)?; let mut v = Vec::with_capacity(n); for _ in 0..n { v.push(SupportedFeatureKey::decode(b, version)?); } v } }); Ok(true) }
                1 => { tag_finalized_features_epoch = Some({ let b: &mut &[u8] = payload; get_i64(b)? }); Ok(true) }
                2 => { tag_finalized_features = Some({ let b: &mut &[u8] = payload; { let n = crate::primitives::array::get_array_len(b, flex)?; let mut v = Vec::with_capacity(n); for _ in 0..n { v.push(FinalizedFeatureKey::decode(b, version)?); } v } }); Ok(true) }
...
impl<'de> Decode<'de> for SupportedFeatureKey {
    fn decode<B: Buf>(buf: &mut B, version: i16) -> Result<Self, ProtocolError> {
        let flex = version >= 3;
        let mut out = Self::default();
        if version >= 3 { out.name = if flex { get_compact_string_owned(buf)? } else { get_string_owned(buf)? }; }
...
pub fn default_json() -> ::serde_json::Value {
    ::serde_json::json!({"ErrorCode": 0, "ApiKeys": [], "ThrottleTimeMs": 0, "SupportedFeatures": [], "FinalizedFeaturesEpoch": -1, "FinalizedFeatures": [], "ZkMigrationReady": false})
}
