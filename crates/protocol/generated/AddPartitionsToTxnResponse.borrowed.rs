...
impl Encode for AddPartitionsToTxnResponse<'_> {
    fn encode<B: BufMut>(&self, buf: &mut B, version: i16) -> Result<(), ProtocolError> {
        ...
        if version >= 4 {
            {
                crate::primitives::array::put_array_len(buf, (self.results_by_transaction).len(), flex);
        ...
        if (0..=3).contains(&version) {
            {
                crate::primitives::array::put_array_len(buf, (self.results_by_topic_v3_and_below).len(), flex);
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 4 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.results_by_transaction).len(), flex);
                let body: usize = (self.results_by_transaction).iter().map(|it| it.encoded_len(version)).sum();
                prefix + body
            };
        }
        if (0..=3).contains(&version) {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.results_by_topic_v3_and_below).len(), flex);
                let body: usize = (self.results_by_topic_v3_and_below).iter().map(|it| it.encoded_len(version)).sum();
                prefix + body
            };
        }
        if flex {
...
impl Encode for AddPartitionsToTxnResult<'_> {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 4 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.topic_results).len(), flex);
                let body: usize = (self.topic_results).iter().map(|it| it.encoded_len(version)).sum();
                prefix + body
            };
        }
        if flex {
