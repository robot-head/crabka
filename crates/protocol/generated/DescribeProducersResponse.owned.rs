...
impl Encode for PartitionResponse {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.active_producers).len(), flex);
                let body: usize = (self.active_producers).iter().map(|it| it.encoded_len(version)).sum();
                prefix + body
            };
        }
        if flex {
