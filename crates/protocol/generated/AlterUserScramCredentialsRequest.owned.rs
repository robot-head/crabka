...
impl Encode for ScramCredentialUpsertion {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += if flex { compact_bytes_len(&self.salted_password) } else { bytes_len(&self.salted_password) };
        }
        if flex {
            let known_pairs: Vec<(u32, usize)> = Vec::new();
            n += tagged_fields_len(&known_pairs, &self.unknown_tagged_fields);
...
impl Decode<'_> for ScramCredentialUpsertion {
    fn decode<B: Buf>(buf: &mut B, version: i16) -> Result<Self, ProtocolError> {
        ...
        if version >= 0 {
            out.salted_password = if flex { get_compact_bytes_owned(buf)? } else { get_bytes_owned(buf)? };
        }
        if flex {
            out.unknown_tagged_fields = read_tagged_fields(buf, |_tag, _payload| Ok(false))?;
        }
