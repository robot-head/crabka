...
impl DeleteAclsResponse<'_> {
    ...
    pub fn to_owned(&self) -> crate::owned::delete_acls_response::DeleteAclsResponse {
        crate::owned::delete_acls_response::DeleteAclsResponse {
            ...
            filter_results: (self.filter_results).iter().map(DeleteAclsFilterResult::to_owned).collect(),
...
impl DeleteAclsFilterResult<'_> {
    ...
    pub fn to_owned(&self) -> crate::owned::delete_acls_response::DeleteAclsFilterResult {
        crate::owned::delete_acls_response::DeleteAclsFilterResult {
            ...
            matching_acls: (self.matching_acls).iter().map(DeleteAclsMatchingAcl::to_owned).collect(),
            unknown_tagged_fields: self.unknown_tagged_fields.clone(),
        }
    }
}
...
impl<'a> DeleteAclsMatchingAcl<'a> {
    ...
    fn decode_field_5(out: &mut Self, buf: &mut &'a [u8], version: i16, flex: bool) -> Result<(), ProtocolError> {
        if version >= 0 {
            out.principal = if flex { get_compact_string_borrowed(buf)? } else { get_string_borrowed(buf)? };
        }
        Ok(())
    }
