...
impl SaslHandshakeResponse<'_> {
    ...
    pub fn to_owned(&self) -> crate::owned::sasl_handshake_response::SaslHandshakeResponse {
        crate::owned::sasl_handshake_response::SaslHandshakeResponse {
            ...
            mechanisms: (self.mechanisms).iter().map(std::string::ToString::to_string).collect(),
...
impl Encode for SaslHandshakeResponse<'_> {
    ...
    fn encoded_len(&self, version: i16) -> usize {
        ...
        if version >= 0 {
            n += {
                let prefix = crate::primitives::array::array_len_prefix_len((self.mechanisms).len(), flex);
                let body: usize = (self.mechanisms).iter().map(|it| if flex { compact_string_len(it) } else { string_len(it) }).sum();
                prefix + body
            };
        }
        n
