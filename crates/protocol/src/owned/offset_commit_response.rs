...
include!(concat!(
    ...
    "/generated/OffsetCommitResponse.owned.rs"
));
...
mod tests {
    ...
    fn min_version_roundtrips() {
        ...
        let mut cur = &buf[..];
        let decoded = OffsetCommitResponse::decode(&mut cur, MIN_VERSION).unwrap();
    ...
    fn max_version_roundtrips() {
        ...
        let mut cur = &buf[..];
        assert_eq!(
            OffsetCommitResponse::decode(&mut cur, MAX_VERSION).unwrap(),
            resp
        );
        assert!(cur.is_empty());
    }
