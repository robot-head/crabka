...
include!(concat!(
    ...
    "/generated/IncrementalAlterConfigsRequest.owned.rs"
));
...
mod tests {
    ...
    fn min_version_roundtrips() {
        let v = MIN_VERSION;
        let msg = IncrementalAlterConfigsRequest::default();
        let mut buf = BytesMut::new();
        msg.encode(&mut buf, v).unwrap();
        assert_eq!(msg.encoded_len(v), buf.len());
        let mut cur = &buf[..];
        let _decoded = IncrementalAlterConfigsRequest::decode(&mut cur, v).unwrap();
