...
include!(concat!(
    ...
    "/generated/UpdateRaftVoterRequest.owned.rs"
));
...
mod tests {
    ...
    fn min_version_roundtrips() {
        let v = MIN_VERSION;
        let msg = UpdateRaftVoterRequest::default();
        let mut buf = BytesMut::new();
        msg.encode(&mut buf, v).unwrap();
        assert_eq!(msg.encoded_len(v), buf.len());
        let mut cur = &buf[..];
        let _decoded = UpdateRaftVoterRequest::decode(&mut cur, v).unwrap();
    ...
    fn max_version_roundtrips() {
        ...
        assert_eq!(msg.encoded_len(v), buf.len());
        let mut cur = &buf[..];
        let decoded = UpdateRaftVoterRequest::decode(&mut cur, v).unwrap();
