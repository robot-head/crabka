...
mod tests {
    ...
    fn roundtrip(case_name: &str, msg: &NoOpRecord, v: i16) {
        ...
        let mut cur = &bytes[..];
        let decoded = NoOpRecord::decode(&mut cur, v).unwrap();
        assert!(
            cur.is_empty(),
    ...
    fn roundtrips_all_versions() {
        type TestCase1<'a> = (&'a str, fn(i16) -> NoOpRecord);
        let cases: [TestCase1<'_>; 2] = [
            ("default", |_| NoOpRecord::default()),
            ("populated", NoOpRecord::populated),
        ];
