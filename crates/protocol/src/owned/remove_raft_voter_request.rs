...
include!(concat!(
    ...
    "/generated/RemoveRaftVoterRequest.owned.rs"
));
...
mod tests {
    ...
    fn min_version_roundtrips() {
        let v = MIN_VERSION;
        let msg = RemoveRaftVoterRequest::default();
        let mut buf = BytesMut::new();
        msg.encode(&mut buf, v).unwrap();
        assert_eq!(msg.encoded_len(v), buf.len());
        let mut cur = &buf[..];
        let _decoded = RemoveRaftVoterRequest::decode(&mut cur, v).unwrap();
    ...
    fn max_version_roundtrips() {
        ...
        assert_eq!(msg.encoded_len(v), buf.len());
        let mut cur = &buf[..];
        let decoded = RemoveRaftVoterRequest::decode(&mut cur, v).unwrap();
