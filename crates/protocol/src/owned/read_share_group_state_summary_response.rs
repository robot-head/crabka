...
include!(concat!(
    ...
    "/generated/ReadShareGroupStateSummaryResponse.owned.rs"
));
#[cfg(test)]
mod tests {
    ...
    fn roundtrip(case_name: &str, msg: &ReadShareGroupStateSummaryResponse, v: i16) {
        let mut buf = BytesMut::new();
        msg.encode(&mut buf, v).unwrap();
        assert2::assert!(msg.encoded_len(v) == buf.len());
        let bytes = buf.freeze();
        let mut cur = &bytes[..];
        let decoded = ReadShareGroupStateSummaryResponse::decode(&mut cur, v).unwrap();
    ...
    fn roundtrips_all_versions() {
        type TestCase1<'a> = (&'a str, fn(i16) -> ReadShareGroupStateSummaryResponse);
        let cases: [TestCase1<'_>; 2] = [
            ("default", |_| ReadShareGroupStateSummaryResponse::default()),
            ("populated", ReadShareGroupStateSummaryResponse::populated),
        ];
