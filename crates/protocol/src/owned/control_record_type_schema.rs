...
include!(concat!(
    ...
    "/generated/ControlRecordTypeSchema.owned.rs"
));
#[cfg(test)]
mod tests {
    ...
    fn roundtrip(case_name: &str, msg: &ControlRecordTypeSchema, v: i16) {
        let mut buf = BytesMut::new();
        msg.encode(&mut buf, v).unwrap();
        assert2::assert!(msg.encoded_len(v) == buf.len());
        let bytes = buf.freeze();
        let mut cur = &bytes[..];
        let decoded = ControlRecordTypeSchema::decode(&mut cur, v).unwrap();
    ...
    fn roundtrips_all_versions() {
        type TestCase1<'a> = (&'a str, fn(i16) -> ControlRecordTypeSchema);
        let cases: [TestCase1<'_>; 2] = [
            ("default", |_| ControlRecordTypeSchema::default()),
            ("populated", ControlRecordTypeSchema::populated),
        ];
