...
mod tests {
    ...
    use assert2::assert;
    use bytes::BytesMut;
    fn roundtrip(msg: &StreamsGroupDescribeResponse, v: i16) {
    ...
    fn default_roundtrips_all_versions() {
        for v in MIN_VERSION..=MAX_VERSION {
            roundtrip(&StreamsGroupDescribeResponse::default(), v);
        }
    }
    #[test]
    fn populated_roundtrips_all_versions() {
        for v in MIN_VERSION..=MAX_VERSION {
            roundtrip(&StreamsGroupDescribeResponse::populated(v), v);
        }
    }
}
