...
mod tests {
    ...
    use assert2::assert;
    use bytes::BytesMut;
    fn roundtrip(msg: &RemotePartitionDeleteMetadataRecord, v: i16) {
    ...
    fn default_roundtrips_all_versions() {
        for v in MIN_VERSION..=MAX_VERSION {
            roundtrip(&RemotePartitionDeleteMetadataRecord::default(), v);
        }
    }
    #[test]
    fn populated_roundtrips_all_versions() {
        for v in MIN_VERSION..=MAX_VERSION {
            roundtrip(&RemotePartitionDeleteMetadataRecord::populated(v), v);
        }
    }
}
