...
include!(concat!(
    ...
    "/generated/DescribeUserScramCredentialsResponse.owned.rs"
));
...
mod tests {
    ...
    fn min_version_roundtrips() {
        let v = MIN_VERSION;
        let msg = DescribeUserScramCredentialsResponse::default();
        let mut buf = BytesMut::new();
        msg.encode(&mut buf, v).unwrap();
        assert_eq!(msg.encoded_len(v), buf.len());
        let mut cur = &buf[..];
        let _decoded = DescribeUserScramCredentialsResponse::decode(&mut cur, v).unwrap();
