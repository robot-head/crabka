...
include!(concat!(
    ...
    "/generated/RenewDelegationTokenRequest.owned.rs"
));
#[cfg(test)]
mod tests {
    use assert2::assert;
    ...
    fn roundtrip(case_name: &str, msg: &RenewDelegationTokenRequest, v: i16) {
        ...
        let mut cur = &bytes[..];
        let decoded = RenewDelegationTokenRequest::decode(&mut cur, v).unwrap();
        assert!(
            cur.is_empty(),
    ...
    fn roundtrips_all_versions() {
        type TestCase1<'a> = (&'a str, fn(i16) -> RenewDelegationTokenRequest);
        let cases: [TestCase1<'_>; 2] = [
            ("default", |_| RenewDelegationTokenRequest::default()),
            ("populated", RenewDelegationTokenRequest::populated),
        ];
