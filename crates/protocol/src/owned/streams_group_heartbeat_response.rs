...
include!(concat!(
    ...
    "/generated/StreamsGroupHeartbeatResponse.owned.rs"
));
#[cfg(test)]
mod tests {
    ...
    fn roundtrip(case_name: &str, msg: &StreamsGroupHeartbeatResponse, v: i16) {
        let mut buf = BytesMut::new();
        msg.encode(&mut buf, v).unwrap();
        assert2::assert!(msg.encoded_len(v) == buf.len());
        let bytes = buf.freeze();
        let mut cur = &bytes[..];
        let decoded = StreamsGroupHeartbeatResponse::decode(&mut cur, v).unwrap();
    ...
    fn roundtrips_all_versions() {
        type TestCase1<'a> = (&'a str, fn(i16) -> StreamsGroupHeartbeatResponse);
        let cases: [TestCase1<'_>; 2] = [
            ("default", |_| StreamsGroupHeartbeatResponse::default()),
            ("populated", StreamsGroupHeartbeatResponse::populated),
        ];
