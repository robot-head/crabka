...
include!(concat!(
    ...
    "/generated/RegisterBrokerRecord.owned.rs"
));
#[cfg(test)]
mod tests {
    use assert2::assert;
    ...
    fn roundtrip(case_name: &str, msg: &RegisterBrokerRecord, v: i16) {
        ...
        let mut cur = &bytes[..];
        let decoded = RegisterBrokerRecord::decode(&mut cur, v).unwrap();
        assert!(
            cur.is_empty(),
    ...
    fn roundtrips_all_versions() {
        type TestCase1<'a> = (&'a str, fn(i16) -> RegisterBrokerRecord);
        let cases: [TestCase1<'_>; 2] = [
            ("default", |_| RegisterBrokerRecord::default()),
            ("populated", RegisterBrokerRecord::populated),
        ];
