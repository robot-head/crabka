...
include!(concat!(
    ...
    "/generated/AllocateProducerIdsRequest.owned.rs"
));
#[cfg(test)]
mod tests {
    use assert2::assert;
    ...
    fn roundtrip(case_name: &str, msg: &AllocateProducerIdsRequest, v: i16) {
        ...
        let mut cur = &bytes[..];
        let decoded = AllocateProducerIdsRequest::decode(&mut cur, v).unwrap();
        assert!(
            cur.is_empty(),
    ...
    fn roundtrips_all_versions() {
        type TestCase1<'a> = (&'a str, fn(i16) -> AllocateProducerIdsRequest);
        let cases: [TestCase1<'_>; 2] = [
            ("default", |_| AllocateProducerIdsRequest::default()),
            ("populated", AllocateProducerIdsRequest::populated),
        ];
