...
include!(concat!(
    ...
    "/generated/RemoteLogSegmentMetadataRecord.borrowed.rs"
));
#[cfg(test)]
mod tests {
    use bytes::BytesMut;
    ...
    fn check(case: &str, msg_bytes: &bytes::Bytes, v: i16) {
        ...
        let decoded = RemoteLogSegmentMetadataRecord::decode_borrow(&mut cur, v).unwrap();
    ...
    fn roundtrip_cases_all_versions() {
        for v in MIN_VERSION..=MAX_VERSION {
            for (case, msg) in [
                ("default", RemoteLogSegmentMetadataRecord::default()),
                ("populated", RemoteLogSegmentMetadataRecord::populated(v)),
            ] {
