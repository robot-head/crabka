...
include!(concat!(
    ...
    "/generated/RemoveDelegationTokenRecord.borrowed.rs"
));
#[cfg(test)]
mod tests {
    use bytes::BytesMut;
    ...
    fn check(case: &str, msg_bytes: &bytes::Bytes, v: i16) {
        ...
        let decoded = RemoveDelegationTokenRecord::decode_borrow(&mut cur, v).unwrap();
    ...
    fn roundtrip_cases_all_versions() {
        for v in MIN_VERSION..=MAX_VERSION {
            for (case, msg) in [
                ("default", RemoveDelegationTokenRecord::default()),
                ("populated", RemoveDelegationTokenRecord::populated(v)),
            ] {
