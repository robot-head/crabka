...
include!(concat!(
    ...
    "/generated/RemoveAccessControlEntryRecord.borrowed.rs"
));
#[cfg(test)]
mod tests {
    use bytes::BytesMut;
    ...
    fn check(case: &str, msg_bytes: &bytes::Bytes, v: i16) {
        ...
        let decoded = RemoveAccessControlEntryRecord::decode_borrow(&mut cur, v).unwrap();
    ...
    fn roundtrip_cases_all_versions() {
        for v in MIN_VERSION..=MAX_VERSION {
            for (case, msg) in [
                ("default", RemoveAccessControlEntryRecord::default()),
                ("populated", RemoveAccessControlEntryRecord::populated(v)),
            ] {
