...
include!(concat!(
    ...
    "/generated/OffsetCommitResponse.borrowed.rs"
));
...
mod tests {
    ...
    fn min_version_roundtrips() {
        ...
        let mut cur = &frozen[..];
        let decoded = OffsetCommitResponse::decode_borrow(&mut cur, MIN_VERSION).unwrap();
    ...
    fn max_version_roundtrips() {
        ...
        let frozen = buf.freeze();
        let mut cur = &frozen[..];
        let decoded = OffsetCommitResponse::decode_borrow(&mut cur, MAX_VERSION).unwrap();
        assert!(cur.is_empty());
