...
mod tests {
    ...
    fn roundtrips_all_versions() {
        for v in MIN_VERSION..=MAX_VERSION {
            let cases = [
                ("default", OffsetFetchRequest::default()),
                ("populated", OffsetFetchRequest::populated(v)),
            ];

            for (case, mut expected) in cases {
                if v < 2 && expected.topics.is_none() {
                    expected.topics = Some(Vec::new());
                }
                ...
                let decoded = OffsetFetchRequest::decode_borrow(&mut cur, v).unwrap();
