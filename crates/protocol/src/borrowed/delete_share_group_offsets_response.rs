...
include!(concat!(
    ...
    "/generated/DeleteShareGroupOffsetsResponse.borrowed.rs"
));
...
mod tests {
    ...
    fn min_version_roundtrips() {
        let v = MIN_VERSION;
        let msg = DeleteShareGroupOffsetsResponse::default();
        let mut buf = BytesMut::new();
        msg.encode(&mut buf, v).unwrap();
        assert_eq!(msg.encoded_len(v), buf.len());
        let frozen = buf.freeze();
        let mut cur: &[u8] = &frozen;
        let _decoded = DeleteShareGroupOffsetsResponse::decode_borrow(&mut cur, v).unwrap();
    }
