...
include!(concat!(
    ...
    "/generated/DescribeTransactionsRequest.borrowed.rs"
));
...
mod tests {
    ...
    fn min_version_roundtrips() {
        let v = MIN_VERSION;
        let msg = DescribeTransactionsRequest::default();
        let mut buf = BytesMut::new();
        msg.encode(&mut buf, v).unwrap();
        assert_eq!(msg.encoded_len(v), buf.len());
        let frozen = buf.freeze();
        let mut cur: &[u8] = &frozen;
        let _decoded = DescribeTransactionsRequest::decode_borrow(&mut cur, v).unwrap();
    }
