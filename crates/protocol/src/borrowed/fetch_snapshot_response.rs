...
include!(concat!(
    ...
    "/generated/FetchSnapshotResponse.borrowed.rs"
));
#[cfg(test)]
mod tests {
    ...
    fn roundtrips_all_versions() {
        for v in MIN_VERSION..=MAX_VERSION {
            let cases = [
                ("default", FetchSnapshotResponse::default()),
                ("populated", FetchSnapshotResponse::populated(v)),
            ];

            for (case, mut expected) in cases {
                for topic in &mut expected.topics {
                    for partition in &mut topic.partitions {
                        partition.unaligned_records =
                            crate::records::RecordsPayloadBorrowed::Legacy(&[]);
                    }
                }
                ...
                let decoded = FetchSnapshotResponse::decode_borrow(&mut cur, v).unwrap();
