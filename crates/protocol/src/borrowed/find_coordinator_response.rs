...
include!(concat!(
    ...
    "/generated/FindCoordinatorResponse.borrowed.rs"
));
...
mod tests {
    ...
    fn min_version_roundtrips() {
        let v = MIN_VERSION;
        let msg = FindCoordinatorResponse::default();
        let mut buf = BytesMut::new();
        msg.encode(&mut buf, v).unwrap();
        assert_eq!(msg.encoded_len(v), buf.len());
        let frozen = buf.freeze();
        let mut cur: &[u8] = &frozen;
        let _decoded = FindCoordinatorResponse::decode_borrow(&mut cur, v).unwrap();
    }
