...
mod tests {
    ...
    fn check(msg_bytes: &bytes::Bytes, v: i16) {
        ...
        let decoded = OffsetCommitRequest::decode_borrow(&mut cur, v).unwrap();
    ...
    fn default_roundtrips_all_versions() {
        for v in MIN_VERSION..=MAX_VERSION {
            ...
            OffsetCommitRequest::default().encode(&mut buf, v).unwrap();
            check(&buf.freeze(), v);
        }
    }
    ...
    fn populated_roundtrips_all_versions() {
        for v in MIN_VERSION..=MAX_VERSION {
            let mut buf = BytesMut::new();
            OffsetCommitRequest::populated(v).encode(&mut buf, v).unwrap();
            check(&buf.freeze(), v);
        }
    }
}
