...
include!(concat!(
    ...
    "/generated/RemoteLogSegmentMetadataSnapshotRecord.borrowed.rs"
));
#[cfg(test)]
mod tests {
    use super::*;
    ...
    fn check(msg_bytes: &bytes::Bytes, v: i16) {
        ...
        let decoded = RemoteLogSegmentMetadataSnapshotRecord::decode_borrow(&mut cur, v).unwrap();
        assert!(cur.is_empty());
    ...
    fn default_roundtrips_all_versions() {
        for v in MIN_VERSION..=MAX_VERSION {
            let msg = RemoteLogSegmentMetadataSnapshotRecord::default();
    ...
    fn populated_roundtrips_all_versions() {
        for v in MIN_VERSION..=MAX_VERSION {
            let msg = RemoteLogSegmentMetadataSnapshotRecord::populated(v);
            let mut buf = BytesMut::new();
            msg.encode(&mut buf, v).unwrap();
            check(&buf.freeze(), v);
        }
