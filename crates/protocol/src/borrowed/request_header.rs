...
mod tests {
    ...
    fn check(msg_bytes: &bytes::Bytes, v: i16) {
        ...
        let decoded = RequestHeader::decode_borrow(&mut cur, v).unwrap();
    ...
    fn default_roundtrips_all_versions() {
        for v in MIN_VERSION..=MAX_VERSION {
            ...
            RequestHeader::default().encode(&mut buf, v).unwrap();
            check(&buf.freeze(), v);
        }
    }
    ...
    fn populated_roundtrips_all_versions() {
        for v in MIN_VERSION..=MAX_VERSION {
            let mut buf = BytesMut::new();
            RequestHeader::populated(v).encode(&mut buf, v).unwrap();
            check(&buf.freeze(), v);
        }
    }
}
