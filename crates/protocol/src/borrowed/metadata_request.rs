...
mod tests {
    ...
    fn roundtrips_all_versions() {
        for v in MIN_VERSION..=MAX_VERSION {
            let cases = [
                ("default", MetadataRequest::default()),
                ("populated", MetadataRequest::populated(v)),
            ];

            for (case, mut expected) in cases {
                if v == 0 && expected.topics.is_none() {
                ...
                let decoded = MetadataRequest::decode_borrow(&mut cur, v).unwrap();
