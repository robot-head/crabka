    ...
    S: ProfileStore,
{
    ...
    let step = step_from_secs(req.step).map_err(connect_error)?;
    let time_buckets =
        heatmap_time_buckets(StartMs(req.start), EndMs(req.end), step).map_err(connect_error)?;
    let span_exemplars = match req.exemplar_type {
        exemplar_type if exemplar_type == pb::querier::v1::ExemplarType::Span as i32 => state
...
",
    ...
    fn heatmap_time_buckets_ceil_from_step_seconds() {
        check!(heatmap_time_buckets(StartMs(0), EndMs(21_000), secs(10)).unwrap() == 3);
        check!(heatmap_time_buckets(StartMs(0), EndMs(1), <Time as TimeExt>::ZERO).is_err());
        check!(heatmap_time_buckets(StartMs(1), EndMs(1), secs(1)).is_err());
    }
    ...
    fn heatmap_time_buckets_span_uses_nonzero_start() {
        ...
        check!(heatmap_time_buckets(StartMs(30_000), EndMs(50_000), secs(10)).unwrap() == 2);
    }
    ...
    fn heatmap_time_buckets_rejects_sub_millisecond_steps() {
        for step_secs in [0.0001, 0.0005, 0.000_999_9] {
            ...
            check!(
                heatmap_time_buckets(StartMs(0), EndMs(1), step).is_err(),
                "{step_secs}"
            );
        }
        check!(heatmap_time_buckets(StartMs(0), EndMs(1), millis(1)).unwrap() == 1);
    }

    #[test]
    fn heatmap_time_buckets_caps_large_ranges() {
        assert!(
            heatmap_time_buckets(StartMs(0), EndMs(i64::MAX), secs(10)).unwrap()
