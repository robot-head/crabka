...
#[test]
fn cogroup_sliding_matches_jvm() {
    ...
    assert_matches_fixture(&wire, "cogroup_sliding");
}

#[test]
fn cogroup_sliding_matches_jvm_behavior() {
    ...
    let golden: Vec<Row> = serde_json::from_str(
        &std::fs::read_to_string(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/tests/testdata/cogroup/behavior_sliding.json"
        ))
        .unwrap(),
    )
    .unwrap();
