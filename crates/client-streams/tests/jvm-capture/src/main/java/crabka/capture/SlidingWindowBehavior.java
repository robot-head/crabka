...
/**
 * Capture-first behavior pin for KIP-450 sliding-window count: runs the JVM
 * {@link TopologyTestDriver} with a sliding-window count topology, drives a
 * fixed out-of-order script, and writes the exact emission sequence to
 * {@code behavior.json}. This pins the JVM KStreamSlidingWindowAggregate
 * algorithm (processInOrder + out-of-order path) as ground truth for the Rust
 * port.
 */
public final class SlidingWindowBehavior {
    public static void main(String[] args) throws Exception {
        ...
        try (TopologyTestDriver driver = new TopologyTestDriver(b.build(), props, Instant.ofEpochMilli(0))) {
            ...
            System.out.println("sliding window behavior:\n" + sb);
        }
    }
}
