...
mod tests {
    ...
    fn session_key_layout_end_first() {
        ...
        assert2::assert!(k.len() == 17); // "k"(1) ‖ end:8 ‖ start:8
        assert2::assert!(&k[1..9] == &9i64.to_be_bytes()); // end first
        assert2::assert!(&k[9..17] == &5i64.to_be_bytes()); // start second
        assert2::assert!(session_end_of(&k) == 9);
        assert2::assert!(session_start_of(&k) == 5);
        assert2::assert!(session_key_bytes_of(&k) == b"k");
    }
