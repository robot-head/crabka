...
impl StreamsBuilder {
        ...
        VS: Serde<V> + Clone,
    {
        ...
        let id = g
            .graph
            .add(name, GraphNodeKind::StreamSource { topics }, vec![]);
        // Store the source `Consumed` (boxed) so Task 5's lowering can call
        // `add_source` with these serdes. Consumed<KS,VS>: Send + 'static.
        g.graph.nodes[id].aux = Some(Box::new(consumed) as Box<dyn std::any::Any + Send>);
        drop(g);
