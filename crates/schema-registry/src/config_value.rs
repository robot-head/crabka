...
mod tests {
    ...
    use super::{PositiveI32, PositiveMillis};

    #[test]
    fn config_value_from_str_checks_refined_boundaries() {
        assert!(PositiveMillis::from_str("1").is_ok());
        assert!(PositiveMillis::from_str("0").is_err());
        assert!(PositiveI32::from_str("1").is_ok());
        assert!(PositiveI32::from_str("0").is_err());
    }
}
