    ...
    let payload = LokiProtoPushRequest {
        streams: vec![LokiProtoStream {
            ...
            entries: vec![LokiProtoEntry {
                ...
                line: "invalid protobuf label".to_string(),
    ...
    let payload = LokiProtoPushRequest {
        streams: vec![LokiProtoStream {
            ...
            entries: vec![LokiProtoEntry {
                ...
                line: "empty string protobuf stream labels".to_string(),
                structured_metadata: vec![],
                parsed: vec![],
            }],
            hash: 0,
    ...
    let payload = LokiProtoPushRequest {
        streams: vec![LokiProtoStream {
            ...
            entries: vec![LokiProtoEntry {
                ...
                line: "duplicate protobuf metadata".to_string(),
                structured_metadata: vec![
                    LokiProtoLabelPair {
    ...
    let payload = LokiProtoPushRequest {
        streams: vec![LokiProtoStream {
            ...
            entries: vec![LokiProtoEntry {
                ...
                line: "empty protobuf metadata name".to_string(),
                structured_metadata: vec![LokiProtoLabelPair {
                    name: String::new(),
                    value: "metadata".to_string(),
                }],
...
async fn real_loki_and_crabka_return_same_invalid_structured_metadata_name_push_error() {
    ...
    let (broker, bootstrap, _broker_dir) = boot_crabka().await;
    let topic = "__crabka_observability_loki_invalid_metadata_name_differential";
    create_topic(&bootstrap, topic).await;
    broker.wait_until_partition_present(topic, 0).await;
    let data_root = TempDir::new().expect("data root");
    let distributor_config = service_config(Role::Distributor, &bootstrap, topic, &data_root);
        ...
        "streams": [
            {
                ...
                "values": [[current_unix_second_ns().to_string(), "invalid metadata name", {"9bad": "metadata"}]]
            }
        ]
    });
