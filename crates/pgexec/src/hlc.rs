...
//!
//! A Hybrid Logical Clock timestamp is one `u64`. The physical component, in
//! milliseconds since a Crabka epoch, occupies the high [`PHYSICAL_BITS`], and a
//! logical counter occupies the low [`LOGICAL_BITS`]. This single packing makes
//! numeric order, big-endian byte order, and HLC happens-before order coincide.
//! So the mode leaves the existing MVCC key/tuple encodings and the
//! `commit_ts <= read_ts` visibility check untouched. See the timestamp-source
//! seam design. A stamp whose physical component is zero is a purely logical
//! timestamp, and it sorts below every stamp with a real physical component.
//! That is what lets a solo tenant be promoted onto the HLC monotonically.
//!
//! The caller *injects* wall-clock time into [`HybridLogicalClock::now`] and
//! [`HybridLogicalClock::observe`] rather than reading it from `SystemTime`, so
//! the clock is deterministic under test.
...
/// Largest representable logical counter, that is `2^LOGICAL_BITS - 1`, or about
/// 4M causally related events per physical millisecond.
pub const MAX_LOGICAL: u32 = (1 << LOGICAL_BITS) - 1;

/// Largest representable physical millisecond component, that is
/// `2^PHYSICAL_BITS - 1`, or over a century from a Crabka epoch.
...
/// `logical`, which matches the numeric order of the packed `u64` exactly.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub struct Hlc {
    /// Physical component in milliseconds. It is zero for a purely logical
    /// stamp.
    pub physical_ms: u64,
    /// Logical counter that tells apart events within one physical millisecond.
    pub logical: u32,
}

/// Pack a physical millisecond value and a logical counter into one stamp.
///
/// This function masks each component to its field width, that is
/// [`MAX_PHYSICAL_MS`] and [`MAX_LOGICAL`]. A sane millisecond clock stays
/// inside [`PHYSICAL_BITS`] for over a century, and the clock rules below never
/// let the logical counter exceed [`MAX_LOGICAL`].
#[must_use]
...
/// Increment the logical component of `packed`, and roll into the physical
/// component when the counter is already saturated.
///
/// The roll into physical is deliberate, in place of a failure. The clock never
/// blocks and never returns an error, and a stamp that borrows a millisecond
/// from the future stays within the skew budget the mode already tolerates.
/// `(physical + 1, 0)` strictly dominates `(physical, MAX_LOGICAL)`, so the
/// clock keeps its monotonicity.
...
/// component is the max of all three inputs, and this function bumps the logical
/// component past whichever inputs supplied that max.
...
///
/// The last-issued stamp lives in an [`AtomicU64`] that a compare-exchange loop
/// updates, rather than behind a [`std::sync::Mutex`]. Every timestamp
/// allocation in HLC mode hits this clock. The critical section is a handful of
/// branch-free integer operations, with no allocation and no `.await`. A
/// lock-free retry avoids both lock contention on that hot path and the
...
impl HybridLogicalClock {
    /// Create a clock whose last-issued stamp is zero. That is a purely logical
    /// stamp below every real-physical stamp.
    #[must_use]
    pub fn new() -> Self {
        Self::default()
    }
    ...
    ///
    /// Node startup folds the local durable horizon in this way before it serves
    /// its first stamp. That discharges the seam's durable-horizon obligation.
    #[must_use]
    ...
    /// `(wall_ms, 0)`. Otherwise the physical component holds and the logical
    ...
    /// Atomically reserve a contiguous run of `count` stamps. Returns the first
    /// stamp of the run.
    ///
    /// The run starts at `max(last + 1, pack(wall_ms, 0))`. It is wall-anchored
    /// when the injected wall clock has moved past the last-issued stamp, and
    /// densely packed right behind the previous reservation otherwise. A packed
    /// stamp is one integer, so `last + 1` is exactly the HLC logical bump,
    /// including the carry into the physical field when the logical counter is
    /// saturated. So a run that crosses the logical-field boundary borrows a
    /// future millisecond, the same bounded drift a stalled wall clock already
    /// produces. A reservation of one is identical to [`Self::now`].
    ///
