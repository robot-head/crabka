...
use crabka_pgtypes::{
    ColumnType, Datum, EnumValue, RecordValue,
...
/// The domain's `DEFAULT` source text, when it has one.
#[must_use]
pub fn domain_default(ty: ColumnType) -> Option<String> {
    let ColumnType::Domain(domain_ref) = ty else {
        return None;
    };
    usertype::lookup_oid(domain_ref.oid)?
        .domain()?
        .default
        .clone()
}

/// The composite type `ty` refers to, or 42704 when it is no longer registered.
///
/// # Errors
///
/// 42704 when the oid names no registered type.
pub fn composite_definition(
    named: crabka_pgtypes::usertype::UserTypeRef,
) -> Result<UserType, ExecError> {
    usertype::lookup_oid(named.oid).ok_or_else(|| {
        ExecError::UndefinedObject(format!("type \"{}\" does not exist", named.name))
    })
}

/// Build a value of composite type `named` from positional field values,
/// coercing each to its declared type.
///
/// # Errors
///
/// 42P16 when the field count does not match, plus any coercion error.
pub fn build_composite(
    named: crabka_pgtypes::usertype::UserTypeRef,
    values: Vec<Datum>,
    coerce: &mut dyn FnMut(Datum, ColumnType) -> Result<Datum, ExecError>,
) -> Result<Datum, ExecError> {
    let ty = composite_definition(named)?;
    let fields = ty.fields().unwrap_or(&[]);
    if values.len() != fields.len() {
        return Err(ExecError::Remote(crabka_pgwire::error::PgError::error(
            "42P16",
            format!(
                "cannot cast type record to {}\nDETAIL:  Input has too {} columns.",
                named.name,
                if values.len() < fields.len() {
                    "few"
                } else {
    ...
    for (value, field) in values.into_iter().zip(fields) {
        coerced.push(coerce(value, field.ty)?);
    }
...
/// The field names of a composite, shared so a per-row record does not clone
/// them.
#[must_use]
pub fn field_names(fields: &[CompositeField]) -> std::sync::Arc<[String]> {
    fields
        .iter()
...
/// `record_in` for a named composite: split the literal and convert each field
/// with `parse_field`.
///
/// # Errors
///
/// 22P02 for a malformed literal or a wrong column count, plus any per-field
/// input error.
pub fn parse_composite_literal(
    named: crabka_pgtypes::usertype::UserTypeRef,
    literal: &str,
    parse_field: &mut dyn FnMut(&str, ColumnType) -> Result<Datum, ExecError>,
) -> Result<Datum, ExecError> {
    let ty = composite_definition(named)?;
    let fields = ty.fields().unwrap_or(&[]);
    let mut parts = crabka_pgtypes::composite::record_fields(literal)?;
    // `()` splits as one NULL field; against a zero-attribute composite that is
    // the empty record, which the literal alone cannot express.
    if fields.is_empty() && parts.len() == 1 && parts[0].is_none() {
        parts.clear();
    }
    if parts.len() != fields.len() {
        return Err(crabka_pgtypes::composite::malformed(
            literal,
            if parts.len() < fields.len() {
                "Too few columns."
            } else {
    ...
    for (part, field) in parts.into_iter().zip(fields) {
        values.push(match part {
            None => Datum::Null,
            Some(text) => parse_field(&text, field.ty)?,
        });
    }
    Ok(Datum::Record(RecordValue::named(
        Some(named),
...
/// `enum_in`: the label must be one of the type's, or 22P02.
///
/// # Errors
///
/// 22P02 when `label` is not a label of the type, 42704 when the type is gone.
pub fn parse_enum_label(
    named: crabka_pgtypes::usertype::UserTypeRef,
    label: &str,
) -> Result<Datum, ExecError> {
    let ty = usertype::lookup_oid(named.oid).ok_or_else(|| {
        ExecError::UndefinedObject(format!("type \"{}\" does not exist", named.name))
    })?;
    // PostgreSQL's `enum_in` trims surrounding whitespace from the literal.
    let trimmed = label.trim();
    if ty
        .labels()
        .is_some_and(|labels| labels.iter().any(|existing| existing == trimmed))
    {
        return Ok(Datum::Enum(EnumValue {
            ty: named,
    ...
    Err(ExecError::Remote(crabka_pgwire::error::PgError::error(
        "22P02",
        format!("invalid input value for enum {}: \"{label}\"", named.name),
    )))
}
...
mod tests {
    ...
    #[test]
    fn a_composite_literal_needs_exactly_the_declared_columns() {
        let ty = usertype::register(
            "ut_exec_literal",
        ...
        let named = ty.type_ref();
        let mut parse = |text: &str, _: ColumnType| -> Result<Datum, ExecError> {
            Ok(Datum::Text(text.into()))
        };
        let value = parse_composite_literal(named, "(1,x)", &mut parse).expect("two columns");
        let Datum::Record(record) = value else {
            panic!("a composite literal builds a record");
        };
        assert!(record.values == vec![Datum::Text("1".into()), Datum::Text("x".into())]);
        assert!(record.field("b") == Some(&Datum::Text("x".into())));
        for bad in ["(1)", "(1,x,y)"] {
            let err = parse_composite_literal(named, bad, &mut parse).expect_err("column count");
            assert!(err.into_pg().code == "22P02", "{bad}");
        }
        usertype::unregister("ut_exec_literal");
    }

    #[test]
    fn an_enum_accepts_only_its_own_labels() {
        let ty = usertype::register(
            "ut_exec_enum",
            UserTypeBody::Enum(vec!["red".into(), "green".into()]),
        );
        let named = ty.type_ref();
        assert!(
            parse_enum_label(named, "red").expect("a declared label")
                == Datum::Enum(EnumValue {
                    ty: named,
                    label: "red".into()
                })
        );
        // `enum_in` trims the literal before matching.
        assert!(parse_enum_label(named, "  green  ").is_ok());
        let err = parse_enum_label(named, "blue").expect_err("not a label");
        assert!(err.into_pg().code == "22P02");
        usertype::unregister("ut_exec_enum");
    }
}
