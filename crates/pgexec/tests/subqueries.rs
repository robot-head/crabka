...
/// An aggregate written inside a sub-select belongs to the query level of the
/// innermost variable its arguments read, ignoring variables local to
/// sub-selects *within* those arguments — `PostgreSQL`'s `check_agg_arguments`.
///
/// Every case here reads the same three relations, so what separates them is
/// only where the aggregate's arguments point. The ones that point at the
/// statement's own row make the statement a grouping query, which this engine
/// does not run yet; deferring them into a per-row hidden column instead would
/// answer one row per source row where `PostgreSQL` folds them to one, so they
/// are refused rather than answered wrongly.
#[tokio::test]
async fn an_aggregate_is_levelled_by_the_variables_it_reads() {
    use assert2::assert;

    let client = connect(spawn().await).await;
    seed_correlated(&client).await;

    // Aggregates that belong to their own sub-select: answered, one row per
    // source row, exactly as PostgreSQL 18.4 answers them.
    let levelled_inward: &[(&str, &[&[Option<&str>]])] = &[
        // Reads only its own level.
        (
            "SELECT x.a, (SELECT max(z.d) FROM u3 z WHERE z.c = x.a) FROM u1 x ORDER BY x.a",
            &[
                &[Some("1"), Some("11")],
        ...
        // Reads nothing at all, so it belongs where it is written.
        (
            "SELECT x.a, (SELECT count(*) FROM u3 z WHERE z.c = x.a) FROM u1 x ORDER BY x.a",
            &[
                &[Some("1"), Some("2")],
                &[Some("2"), Some("0")],
                &[Some("3"), Some("1")],
        ...
        // Reads BOTH levels: the innermost one wins, so it still belongs to the
        // sub-select and still answers per source row.
        (
            "SELECT x.a, (SELECT max(z.d + x.a) FROM u3 z WHERE z.c = x.a) FROM u1 x ORDER BY x.a",
            &[
                &[Some("1"), Some("12")],
                &[Some("2"), None],
                &[Some("3"), Some("33")],
        ...
        // The variable it reads is local to a sub-select of its own argument, so
        // that variable is ignored and the aggregate stays where it is written.
        (
            "SELECT x.a, (SELECT max((SELECT z.d FROM u3 z WHERE z.c = 3))) FROM u1 x ORDER BY x.a",
            &[
                &[Some("1"), Some("30")],
                &[Some("2"), Some("30")],
                &[Some("3"), Some("30")],
    ...
    for (sql, expected) in levelled_inward {
        let expected: Vec<Vec<Option<String>>> = expected
            .iter()
            .map(|row| {
                row.iter()
    ...
    // Aggregates that belong to the statement's own level. PostgreSQL 18.4 folds
    // each of these to a single row holding `3`; this engine reports the outer
    // reference it cannot resolve instead, which is what it reported before
    // select-list correlation existed. Answering them needs the outer-level
    // aggregate itself, which is a separate feature — what matters here is that
    // recognising the shape keeps deferral from answering three rows.
    for sql in [
        // Reads the statement's row directly, one level out.
        "SELECT (SELECT max(x.a)) FROM u1 x",
        // Reads it through a sub-select of its own argument, whose own
        // variables are ignored — the shape the upstream `aggregates` test uses.
        "SELECT (SELECT max((SELECT y.b FROM u2 y WHERE y.b = x.a))) FROM u1 x",
        // Two levels out.
        "SELECT (SELECT (SELECT max(x.a))) FROM u1 x",
    ] {
        assert!(err_code(&client, sql).await == "42P01", "{sql}");
    }
}

/// Two deferred select items are bound against their own FROM descriptions.
///
/// Each deferred expression walks its own query blocks, so the cache that keeps
/// a block's column names across source rows is per expression: sharing one
/// would describe the second item's sub-select with the first item's FROM, and
/// an unqualified outer reference the second one makes would look shadowed by a
/// column only the first one's FROM has.
#[tokio::test]
async fn each_deferred_select_item_is_described_by_its_own_from() {
    use assert2::assert;

    let client = connect(spawn().await).await;
    for sql in [
        "CREATE TABLE w1 (n int4)",
        "CREATE TABLE t1 (n int4)",
        "CREATE TABLE t2 (m int4)",
    ...
    // The first item's sub-select has a column named `n`; the second one's does
    // not, so its bare `n` is the outer reference and must be bound as one.
    let sql = "SELECT (SELECT count(*) FROM t1 WHERE t1.n = w1.n), \
                      (SELECT max(t2.m) FROM t2 WHERE t2.m = n) \
                 FROM w1 ORDER BY 1";
    assert!(
        rows(&client, sql).await
            == vec![
