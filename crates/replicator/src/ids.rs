...
//! [`crate::mm2::OffsetSync`] as
//! `{ upstream: downstream_val, downstream: upstream_val }`, or write the
//! `translate()` math as `up + (committed - down)`. Such code compiles cleanly
//! when every value is a bare `i64`, and it silently corrupts offset
//! translation. A separate type for each meaning turns those swaps into compile
//! errors.
//!
...
/// crate-local. They encode source-cluster and target-cluster distinctions that
/// are specific to MirrorMaker-2 and have no meaning outside this crate.
...
/// A consumer group's committed **source** offset that translation moves to the
/// target. It lives in the same address space as [`UpstreamOffset`], because
/// both are source-cluster offsets. It is a distinct concept at the translation
/// call site, so it gets its own type.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord, Display, From, Into)]
