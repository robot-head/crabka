...
/// How far on either side of an applied fault timeline rows are always
/// rendered.
const FAULT_WINDOW: Time = secs(5);

/// Maximum number of timeline rows rendered before eliding the rest.
const TIMELINE_ROW_CAP: usize = 60;

/// How far a second's committed count may sit from the run's median before
/// the timeline calls it interesting.
const DEVIATION_THRESHOLD: Ratio = percent(30);
...
/// [`DEVIATION_THRESHOLD`] from the run's median).
#[must_use]
...
/// ±[`FAULT_WINDOW`] of an applied fault, plus any second whose committed
/// count deviates more than [`DEVIATION_THRESHOLD`] from the run's median, in
/// timeline order.
    ...
    report
        ...
        .filter(|sample| {
            let near_fault = report
                .faults
                .iter()
                .any(|fault| (fault.at - sample.t).abs() <= FAULT_WINDOW);
...
/// True when `committed` deviates strictly more than [`DEVIATION_THRESHOLD`]
...
fn deviates_from_median(committed: u64, median: u64) -> bool {
    fraction(u64_as_f64(committed.abs_diff(median)) / u64_as_f64(median)) > DEVIATION_THRESHOLD
}
