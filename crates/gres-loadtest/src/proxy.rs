...
/// is.
const MIN_BURST: ByteSize = kibibytes(64);

/// The window of traffic a throttled direction may burst: one tenth of a
/// second at the configured rate, floored at [`MIN_BURST`].
const BURST_WINDOW: Time = millis(100);
...
fn burst_for(limit: ByteRate) -> ByteSize {
    let window: ByteSize = (limit * BURST_WINDOW).into();
    window.max(MIN_BURST)
}
