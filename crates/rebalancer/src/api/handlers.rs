...
pub async fn cancel_execution(
    ...
    let deadline = cancel_poll_deadline(std::time::Instant::now(), CANCEL_DRAIN_TIMEOUT);
    loop {
        ...
        tokio::time::sleep(CANCEL_DRAIN_POLL.to_std()).await;
    }
}

/// How long `CancelExecution` waits for the executor to release its slot
/// before returning the still-`Executing` proposal.
const CANCEL_DRAIN_TIMEOUT: Time = secs(5);

/// Poll cadence while waiting out [`CANCEL_DRAIN_TIMEOUT`].
const CANCEL_DRAIN_POLL: Time = millis(25);
