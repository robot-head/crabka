//! Hard goal: enforce a per-broker `network_out_bytes_per_sec` limit. It uses
//! the scraped `UsageStore::bytes_out_rate`, summed across the broker's hosted
...
impl NetworkOutCapacity {
    ...
    fn totals(
        ...
        crate::goals::replica_totals(partitions, broker_ids, |broker, topic, partition| {
            ctx.broker_usages
                .bytes_out_rate(broker, topic, partition, Window::FiveMin, now_ms)
...
mod tests {
    ...
    fn empty_usage_no_op() {
        ...
        assert2::assert!(
            (
                NetworkOutCapacity.propose(&s, &ctx).is_empty(),
                NetworkOutCapacity.is_satisfied_with_ctx(&s, &ctx)
            ) == (true, true)
        );
    }
    ...
    fn is_satisfied_with_ctx_returns_false_when_over() {
        ...
        let store = store_with_counter_pair(samples);
        let ctx = ctx_with(caps(1, bytes_per_sec(500_000)), store);
        assert2::assert!(!NetworkOutCapacity.is_satisfied_with_ctx(&s, &ctx));
    }
    ...
    fn network_out_capacity_stops_when_all_destinations_are_replicas() {
        ...
        assert2::assert!(NetworkOutCapacity.propose(&s, &ctx).is_empty());
    }
