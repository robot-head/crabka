...
//! `UsageStore::cpu_micros_rate(broker, topic, partition, FiveMin)`.
//! The rate units (micros/sec) are immaterial for a balance check —
//! the ratio across brokers is what matters. Greedy hot->cold swap,
//! threshold-driven via `GoalContext.imbalance_threshold_pct`.
    ...
    fn totals(
        ...
        for p in partitions {
            for replica in &p.replicas {
                if let Some(rate) = ctx.broker_usages.cpu_micros_rate(
                    *replica,
                    &p.topic,
                    p.partition,
                    Window::FiveMin,
