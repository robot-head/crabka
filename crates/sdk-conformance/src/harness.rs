...
impl Harness {
    ...
    async fn run_vector(
        ...
        match hello {
            Response::Hello { contract_major, .. } if contract_major == CONTRACT_MAJOR => {}
            actual => {
...
impl LiveSubstrate {
    async fn boot(topic_names: &[String]) -> Result<Self, HarnessError> {
        ...
        let broker = Broker::start(BrokerConfig::for_tests(data_dir.path().to_path_buf()))
            .await
