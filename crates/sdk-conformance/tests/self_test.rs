...
const LIVE_ONLY_VECTOR_ID: &str = "queue_v1_1_live_error_mapping";
const TOTAL_VECTOR_COUNT: usize = 17;
const V1_0_APPLICABLE_VECTOR_COUNT: usize = 9;
const V1_1_APPLICABLE_VECTOR_COUNT: usize = 16;
const LIVE_COMPATIBLE_VECTOR_COUNT: usize = 12;
const EXTERNAL_LIVE_COMPATIBLE_VECTOR_COUNT: usize = 11;
...
async fn mock_adapter_passes_contract_vectors() {
    ...
    check!(summary.skipped.len() == QUEUE_V1_1_VECTOR_IDS.len() + 1);
...
async fn mock_adapter_v1_1_passes_queue_vectors_without_stub_queue() {
    ...
    check!(summary.failed.is_empty());
    check!(summary.passed == V1_1_APPLICABLE_VECTOR_COUNT - 1);
    check!(summary.skipped.len() == 1);
    check!(summary.skipped[0].vector_id == LIVE_ONLY_VECTOR_ID);
    check!(summary.skipped[0].reason == "requires live substrate");
}
...
fn queue_v1_1_vectors_are_versioned_and_loaded_by_mock_contract() {
    ...
    let live_only = vectors
        .iter()
        .find(|vector| vector.id == LIVE_ONLY_VECTOR_ID)
        .expect("live error vector is loaded");
    check!(live_only.live_only);
    check!(live_only.since == ContractVersion::new(1, 1));
}
...
async fn live_compatible_mode_runs_queue_v1_1_vectors_when_adapter_supports_them() {
    ...
    check!(summary.failed.is_empty());
    check!(summary.passed == EXTERNAL_LIVE_COMPATIBLE_VECTOR_COUNT);
    check!(
        summary.skipped.len()
            == V1_1_APPLICABLE_VECTOR_COUNT - EXTERNAL_LIVE_COMPATIBLE_VECTOR_COUNT
    );
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn configure_mismatch_reports_before_vector_steps() {
    let _guard = adapter_harness_test_guard().await;
    let harness = Harness::new(HarnessConfig {
        adapter: conformance_bin(),
        adapter_args: vec![
    ...
    check!(summary.failed.len() == 1);
    check!(summary.failed[0].vector_id == "messaging_roundtrip");
    check!(summary.failed[0].step == "configure");
    check!(summary.skipped.is_empty());
}
...
async fn rust_adapter_live_substrate_passes_supported_live_vectors() {
    ...
    check!(summary.skipped.len() == V1_1_APPLICABLE_VECTOR_COUNT - LIVE_COMPATIBLE_VECTOR_COUNT);
    check!(summary.passed + summary.skipped.len() == V1_1_APPLICABLE_VECTOR_COUNT);
    check!(
        summary
            .skipped
