...
CREATE TABLE fp_m (g int4, x double precision);
INSERT INTO fp_m VALUES (1, 1.5), (1, 2.5), (2, 2.0), (2, 4.0), (3, NULL);
...
SELECT g, x FROM fp_m ORDER BY g, x;
SELECT x FROM fp_m WHERE x IS NOT NULL ORDER BY x DESC;

-- whole-table aggregates over a float8 column (sum/avg/min/max in float8)
SELECT sum(x), avg(x), min(x), max(x), count(x), count(*) FROM fp_m;

-- grouped aggregates; the all-NULL group's sum/avg are NULL and count(x) is 0
SELECT g, sum(x), avg(x), count(x) FROM fp_m GROUP BY g ORDER BY g;

-- count(DISTINCT float)
SELECT count(DISTINCT x) FROM fp_m;

-- float arithmetic through the column (float8 ⊕ int → float8, real division)
SELECT x + 1 FROM fp_m WHERE g = 2 ORDER BY x;
SELECT x / 2 FROM fp_m WHERE g = 2 ORDER BY x;
SELECT x * 2 FROM fp_m WHERE g = 1 ORDER BY x;

-- float comparison in WHERE and a projected float8 = literal predicate
SELECT x FROM fp_m WHERE x > 2 ORDER BY x;
SELECT x = 2.5 FROM fp_m WHERE g = 1 ORDER BY x;
...
SELECT x / 0 FROM fp_m WHERE g = 1;
-- an unknown function (42883)
SELECT frobnicate(x) FROM fp_m;
